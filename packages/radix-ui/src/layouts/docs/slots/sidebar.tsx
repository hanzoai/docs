...
function SidebarFolderContent({
  ...
  return (
    <Base.SidebarFolderContent
      ...
      {...props}
    >
      <div className="grid grid-cols-[minmax(0,1fr)] gap-0.5 pt-0.5">{children}</div>
    </Base.SidebarFolderContent>
  );
}
...
function SidebarTabsDropdown({
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="grid grid-cols-[minmax(0,1fr)] gap-1 w-(--radix-popover-trigger-width) p-1 fd-scroll-container">
        {tabs.map((item) => {
