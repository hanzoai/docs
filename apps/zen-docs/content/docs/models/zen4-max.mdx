---
title: zen4-max
description: Trillion-parameter frontier MoE model. 1.04T (32B active) with 163K context.
---

# zen4-max

**Maximum Scale**

Trillion-parameter frontier model with deep reasoning capabilities. 1.04T total parameters with 32B active per token -- the largest model in the Zen family.

## Specifications

| Property | Value |
|----------|-------|
| **Model ID** | `zen4-max` |
| **Parameters** | 1.04T (32B active) |
| **Architecture** | MoE |
| **Context Window** | 163K tokens |
| **Tier** | ultra max |
| **Input Price** | $9.00 / 1M tokens |
| **Output Price** | $9.00 / 1M tokens |

## Capabilities

- Frontier-scale reasoning with 1.04T parameters
- Deep thinking with chain-of-thought capabilities
- 163K context window for massive document processing
- Strong performance on math, code, and complex reasoning
- Efficient MoE inference with only 32B active parameters

## API Usage

```bash
curl https://api.hanzo.ai/v1/chat/completions \
  -H "Authorization: Bearer $HANZO_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "zen4-max",
    "messages": [{"role": "user", "content": "Summarize this research paper..."}]
  }'
```

```python
from hanzoai import Hanzo

client = Hanzo(api_key="sk-your-api-key")

response = client.chat.completions.create(
    model="zen4-max",
    messages=[{"role": "user", "content": "Summarize this research paper..."}],
)
print(response.choices[0].message.content)
```

## See Also

- [zen4](/docs/models/zen4) -- Flagship (744B MoE)
- [zen4-ultra](/docs/models/zen4-ultra) -- Maximum reasoning with deep thinking
- [Pricing](/docs/api/pricing) -- Full pricing table
