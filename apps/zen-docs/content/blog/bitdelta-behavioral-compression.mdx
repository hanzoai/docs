...
[BITDELTA PAPER](https://arxiv.org/abs/2402.10193) [MONOSOUP PAPER](https://arxiv.org/abs/2602.09689) [K-MERGE PAPER](https://arxiv.org/abs/2510.13537) [ZEN MODELS](https://huggingface.co/zenlm)
...
The architecture keeps one base model loaded per GPU cluster. All variants share it. The delta cache fits in KV (NVMe-backed), loading on demand in under 50ms. In practice, our top-5 variants stay hot in KV memory; the remaining variants load from NVMe on first request.
