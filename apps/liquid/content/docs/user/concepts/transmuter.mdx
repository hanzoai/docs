...
<img src="/img/transmuter.png" alt="Transmuter" class="banner-spacing" />

The Transmuter lets you redeem lAssets (LUSD, LETH) 1:1 for their underlying asset after a known waiting period. Purchase below face value, and receive the full value on the maturity date.
...
| Flow     | Context                                                                                                                                                                        |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Deposit  | Send LUSD or LETH to the Transmuter contract.                                                                                                                                |
...
| Maturity | You receive 1 asset-worth of MYT from borrowers for every 1 lAsset.                                                                                                           |

All redeemed lAssets are burned, contracting their supply.

## Why Discounts Exist

Borrowers often sell newly minted lAssets for working capital, pushing market price slightly below par. The spread between that market price and the Transmuter’s guaranteed 1:1 accounting creates a fixed-rate opportunity for buyers.

Inside Liquid, 1 LUSD always offsets 1 USD worth of debt, regardless of its external market price.

### Fixed-Rate Yield Example

**Market**: LUSD = 0.96USDC

**Term**: 90 days

| Action                  | Outcome                                    |
| ----------------------- | ------------------------------------------ |
| Buy LUSD               | Spend 10,000 USDC → receive \~10,416 LUSD |
| Deposit into Transmuter | Locks the 10,416 LUSD for 90 days.        |
...
| ------------------------------------ | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Bad debt in Liquid (exploit, etc) | Redemption pays pro-rata (EG: 0.97:1) until debt is restored.                                                         | Claim now and take a haircut, or leave unclaimed. Once debt clears, you may redeem full 1:1.                   |
...
- **Arbitrage & peg maintenance** - capture fixed yield while pulling lAssets back to parity.

- **LP protection** - LPs can move lAssets from liquidity pools into the Transmuter to erase impermanent loss if the peg widens.
