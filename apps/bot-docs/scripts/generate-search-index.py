#!/usr/bin/env python3
"""
Generate Meilisearch index documents from bot-docs MDX source files.
...
endpoint and the Meilisearch admin-bot-docs-docs index.

Usage:
    python3 scripts/generate-search-index.py > /tmp/bot-docs-index.json

Then index via kubectl exec into the search pod:
    kubectl exec -n hanzo deploy/search -- curl -X POST \
        'http://localhost:7700/indexes/admin-bot-docs-docs/documents' \
        -H 'Authorization: Bearer <MASTER_KEY>' \
        -H 'Content-Type: application/json' \
        --data-binary @- < /tmp/bot-docs-index.json
"""
...
BASE_URL = "https://docs.hanzo.bot/docs"
TAG = "bot-docs-hanzo-bot"
...
def strip_mdx_components(text: str) -> str:
    ...
    # Remove HTML tags (p, strong, br, a, img, etc.)
...
def file_to_url_path(filepath: Path) -> str:
    """Convert a file path to a docs.hanzo.bot URL path."""
...
def sanitize_id(text: str) -> str:
    """Ensure IDs contain only alphanumeric, hyphens, and underscores (Meilisearch requirement)."""
...
def main():
    ...
    mdx_files = [
        ...
        if not any(p.startswith(".") for p in f.relative_to(CONTENT_ROOT).parts)
    ]
