...
ListItems extend Stack views, inheriting all the [Gui standard props](/docs/intro/props), plus:
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'boolean',
      description:
        'Removes all default Hanzo GUI styles (padding, background, etc.). Default is `false` unless `HANZO_GUI_HEADLESS` is set.',
    },
    {
      name: '// Styling Text',
      type: '---',
