...
Text in Hanzo GUI matches Text in react-native-web, with the added [Gui props](/docs/intro/props).
...
SizableText adds a `size` property that is defined using a [spread variant](/docs/core/styled#spread-variants) which looks for a matching key on each of these properties (using `@hanzogui/get-font-sized`):
...
[Source code for SizableText](https://github.com/hanzoai/gui/blob/main/code/ui/text/src/SizableText.tsx).
