...
directly. Hanzo GUI allows nesting themes - both the definition and at runtime. At
runtime Hanzo GUI resolves theme values upwards, ultimately all the way back to
tokens.
...
If you want to style `bento` or `@hanzo/gui` components: see
[Styling `@hanzo/gui` UI components](#styling-gui-components).
...
One of the unique powers of Hanzo GUI is theme nesting. Define a theme with a
name in the form of `parentName_subName` and Hanzo GUI will let you nest themes,
...
Every Hanzo GUI `styled()` component looks for its own specific theme if you pass
it the `name` property. For example:

```tsx
import { View, styled } from '@hanzo/gui' // or '@hanzogui/core'
...
by Hanzo GUI to check for a sub-theme that ends with `_Circle`.
...
### Styling Hanzo GUI components

Hanzo GUI comes in two parts: a core library and a full UI kit. The core library
(`@hanzogui/core`) is flexible and doesn't have many rules. But the full UI kit
(`@hanzo/gui`) has some standard ways of doing things. This helps make everything
work well together.

In the `@hanzo/gui` UI kit, all components use these main theme keys:
...
This means that you can easily re-theme `@hanzo/gui`'s UI kit and your own
...
```tsx
import { GetProps, View, Text, styled } from '@hanzo/gui' // or '@hanzogui/core'
...
```tsx
import { GuiProvider, createTokens, createGui, View, Theme } from '@hanzo/gui'
...
theme that use those variables. Hanzo GUI will handle defining:
...
runtime. Hanzo GUI exports three helpers for this in the package `@hanzogui/theme`
...
You can configure Hanzo GUI to not send any themes JS to the client side, so long
...
you'd typically pass to `createGui` for the client bundle. Note this is a
...
// The `@hanzogui/next-plugin` sets HANZO_GUI_IS_SERVER automatically.
// If you pass an empty themes object Hanzo GUI will try to hydrate by scanning CSS in browser environments.
// It typically takes low single-digit ms to scan and can save significantly on JS size.

const themes =
  process.env.HANZO_GUI_IS_SERVER || process.env.NODE_ENV !== 'production'
    ? themesIn
    : ({} as typeof themesIn)
