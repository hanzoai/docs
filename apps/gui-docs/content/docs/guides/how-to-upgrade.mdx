---
title: Upgrading to Hanzo GUI 8
description: A comprehensive guide for migrating a v1-era Hanzo GUI project onto the current 8.x line
---

Hanzo GUI 8 aligns with modern web standards, improves performance, and simplifies APIs. The migration is moderate -- mostly removing deprecated APIs, renaming props, and updating your config. This guide walks through every change.

## Prerequisites

Hanzo GUI 8 requires:
...
Bump all `@hanzogui/*` and `@hanzo/gui` packages to `^8.0.0`:
...
cause some issues. It's easier to migrate to Hanzo GUI 8 first, then Config v5 after if you do choose to.
...
No special Metro configuration is needed for Hanzo GUI 8. A standard Expo Metro config works out of the box:
...
- [ ] Bump all `@hanzogui/*` packages to `^8.0.0`
