...
description: Learn how to create a powerful yet simple API for a button with Hanzo GUI and
  compound components
...
Hanzo GUI allows for building a button with "compound components", a concept
...
Hanzo GUI components off the shelf and use them with inline styles. But sometimes
...
<Notice>
  ...
  simple, ready to use Button, you can use the [Gui Button](/docs/components/button)
  itself directly.
</Notice>
...
In v2, `color` is not a direct prop on the Button frame. Instead, style your
text through `Button.Text`. For interactive color changes on hover or press, use
Hanzo GUI's `group` prop to coordinate styles between parent and child:
...
for interactive styles using `$group-btn-hover`, `$group-btn-press`, and so on.
...
```tsx
import { createGui, createTokens } from '@hanzogui/core'
...
export default createGui({
  ...
  // ... the rest of your gui.config.ts
})
```

If you'd like to see a more complete work-up of a Hanzo GUI config, check out
[the Simple Web starter repo source code](https://github.com/hanzoai/gui/blob/main/code/starters/simple-web/src/gui.config.ts)
or go ahead and create that starter using `npm create hanzo-gui@latest`.
...
We set the `name` property to `"Button"`, which tells Hanzo GUI to check for a
sub-theme `Button` that extends the current one. So since we have a theme
`light` and we have a sub-theme `light_Button`, Hanzo GUI finds the theme
...
use the handy Hanzo GUI [Spread Variants](/docs/core/variants#spread-variants):
...
awkward. Since Hanzo GUI uses this pattern internally for many components, we've
made a small helper:

```tsx
import { withStaticProperties } from '@hanzogui/core'
...
Not to mention by staying with the `styled` world, the Hanzo GUI optimizing
...
import { SizeTokens, GetProps, withStaticProperties } from '@hanzogui/core'
import { Button } from './OurButton'
...
and pseudo queries is quite complex, and if Hanzo GUI doesn't know that there is a
...
```tsx
import { View, styled, createStyledContext } from '@hanzogui/core'
...
functional component. It will still work nicely with the Hanzo GUI themes and
sizes, showing how you can use them together with external React components.

```tsx
import { getTokens, useTheme } from '@hanzogui/core'
...
We hope this has been helpful in explaining a variety of Hanzo GUI features and
...
With the new Hanzo GUI APIs, we look forward to never having to carefully add a
