...
Configuration in Hanzo GUI controls things like tokens, themes, media queries,
animations, shorthands, and settings.

First, create a `gui.config.ts` file.

<Notice theme="blue">
  We recommend starting with [`@hanzogui/config/v5`](/docs/core/config-v5), our preset
...
<Notice theme="green">
  If using the compiler, your gui.config.ts is parsed at build-time. For this reason,
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      required: false,
      type: 'boolean',
      description: `By default Hanzo GUI inserts CSS with a useInsertionEffect on load. But if you're setting up SSR you'll want to use getCSS() on the server instead and then turn on this setting.`,
    },
    {
      name: 'insets',
      required: false,
...
Native to recognize them. Hanzo GUI doesn't really touch this area, instead you'll
use Expo or
...
Hanzo GUI components in general expect a set of theme keys to be defined like the
...
One of the more powerful features in Hanzo GUI is nesting themes, you just define
them like so:
...
For more on themes, see the [themes docs](/docs/intro/themes), and more
...
- [`@hanzogui/animations-css`](/docs/core/animations-css) - CSS transition based
  animations (lightest)
- [`@hanzogui/animations-react-native`](/docs/core/animations-react-native) -
  React Native Animated
- [`@hanzogui/animations-reanimated`](/docs/core/animations-reanimated) -
  Reanimated for advanced animations
- [`@hanzogui/animations-motion`](/docs/core/animations-motion) - Motion with
  WAAPI for web
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      default: false,
      description: `If you want to style your <body> tag to use theme CSS variables on web, you must place the theme className  onto the body element or above. This will do so. If disabled, Hanzo GUI will place the className onto the element rendered by the GuiProvider`,
    },
    {
      ...
      type: 'boolean',
      description:
        'On web, GuiProvider will add your top level theme className to the root of your document, the html tag. This lets you do things like style your body tag using Hanzo GUI CSS variables that are generated by themes. If disabled, the theme className will instead by applied to the span inside the GuiProvider directly.',
    },
    {
    ...
    },
    {
      ...
      type: 'boolean',
      default: false,
      description: `On iOS, this enables a mode where Hanzo GUI returns color values using DynamicColorIOS significantly speeding up dark/light re-renders. Note: When enabled you must make sure the defaultTheme you set on GuiProvider matches the current system color scheme.`,
    },
    {
      ...
      type: 'string',
      default: 'inline-size',
      description: `Only for web, when using group styles Hanzo GUI uses container queries. When using a group on a containing element, Hanzo GUI defaults to setting "container-type" in CSS to "inline-size".`,
    },
  ]}
/>
...
A few things in Hanzo GUI can be configured via environment variables. Doing it
...
You should be sure to either use the Vite or Webpack Hanzo GUI plugins, or
...
This works because Hanzo GUI generates CSS variables for all theme values. On the
