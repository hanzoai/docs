...
AnimatePresence animates direct children before they unmount. It is inspired by and forked from [Framer Motion](https://github.com/framer/motion), but works with any animation driver in Hanzo GUI.
...
- **`children`** - One or more Hanzo GUI components with unique `key` props
