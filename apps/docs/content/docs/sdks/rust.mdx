...
The `hanzo-ai` crate provides an async Rust client for the Hanzo AI Gateway, with additional support for crypto and decentralized identity (DID) features.
...
let openai = client.chat().completions().create("gpt-5")
    .message("user", "Hello!").send().await?;

let anthropic = client.chat().completions().create("claude-sonnet-4-6")
    .message("user", "Hello!").send().await?;
...
match client.chat().completions().create("gpt-5")
    .message("user", "Hello!")
    .send()
    .await
{
