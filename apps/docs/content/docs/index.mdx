---
title: Documentation
description: The AI cloud you can run yourself. 67 capabilities, one contract at api.hanzo.ai — and the same open-source binary you can run on your own machine, GPU, or cluster.
---
...
one contract at `api.hanzo.ai`. The same binary that answers in production is the
one you run on your own machine, GPU, or cluster.
[**The network is the cloud.**](/docs/network)
...
one domain. Browse the whole platform, category-first.
