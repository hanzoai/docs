...
Good engineering is mostly subtraction. Hanzo **V8 · Open Edition** is a small set
of decisions, applied everywhere, until the whole cloud is simple enough to hold in
your head: one binary, one contract, and a clean wire between languages instead of a
tangle inside one. No consumer ever needs to know or care which part is Go and which
is Rust. This page is the *why* — the design pedagogy. The canonical, normative
version lives in **[HIP-0127](https://hips.hanzo.ai)**.
...
exactly once, in exactly one of the [nine domains](/docs/openapi). The build *fails*
...
consumer speaks. Capabilities are grouped into the nine domains of the
...
Hanzo is Go **and** Rust. The cloud is Go; the [engine](/docs/openapi/engine),
...
```
openapi.yaml          the ONE contract — /v1 over HTTP + ZAP
   │  generate ↓
