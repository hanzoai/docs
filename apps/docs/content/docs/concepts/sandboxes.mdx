...
```bash
curl -X POST https://api.hanzo.ai/v1/sandboxes/lease \
...
`POST /v1/sandboxes/run` narrates into a named session, and its output appends
...
- **[Sandboxes reference](/docs/openapi/sandboxes)** — lease, exec, fs, terminal,
  screen.
