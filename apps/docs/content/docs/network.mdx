...
linked — embeds the whole platform: [every capability](/docs/openapi) across the
nine domains, mounted as subsystems, served behind one contract at
...
Hanzo is Go **and** Rust: the cloud is Go, while the [engine](/docs/openapi/engine),
...
Authenticate with [Hanzo IAM](/docs/openapi/iam) — the same identity works against every
cloud, everywhere.
...
```bash
hanzo "explain the network"          # Intelligence
hanzo projects deploy my-app         # Platform
hanzo kms secrets create             # Identity & trust
```
