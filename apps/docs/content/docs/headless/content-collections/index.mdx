...
description: Use Content Collections for Hanzo Docs
---
...
To integrate with Hanzo Docs, add the following to your `content-collections.ts`.
...
import { transformMDX } from '@hanzo/docs-content-collections/configuration';
...
To use components from other packages like Hanzo Docs UI, pass them to your `<MDXContent />` component.
...
<Callout title='Deep Dive: Why?'>
    ...
    To support importing a package from node modules, Hanzo Docs added a default value to the `cwd` option of MDX Bundler.
