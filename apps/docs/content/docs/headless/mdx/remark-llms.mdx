...
Enable `includeProcessedMarkdown` in your collection config, Hanzo Docs MDX runs `remarkLLMs` internally when this is set.

```ts title="source.config.ts"
import { defineDocs } from '@hanzo/docs-mdx/config';
import { type LLMsOptions } from '@hanzo/docs-core/mdx-plugins/remark-llms';
...
```ts
import { placeholder, type LLMsOptions } from '@hanzo/docs-core/mdx-plugins/remark-llms';
...
```ts
import { renderPlaceholder } from '@hanzo/docs-core/mdx-plugins/remark-llms.runtime';
...
<auto-type-table
  type="import('@hanzo/docs-core/mdx-plugins/remark-llms').LLMsOptions"
/>
