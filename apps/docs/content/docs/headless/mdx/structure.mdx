...
import { remarkStructure } from '@hanzo/docs-core/mdx-plugins';
...
export default defineConfig({
  mdxOptions: {
    // This plugin is enabled by default on Hanzo Docs MDX
    // options:
...
```ts
import { structure } from '@hanzo/docs-core/mdx-plugins';
