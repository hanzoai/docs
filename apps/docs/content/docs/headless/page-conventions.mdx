...
> This guide only applies for content sources that uses `loader()` API, such as **Hanzo Docs MDX**.

## Overview

Hanzo Docs generates **page slugs** and **page tree** (sidebar items) from your content directory using [`loader()`](/docs/headless/source-api),
...
Hanzo Docs detects from the following properties to construct page trees.
...
Page information is supplied by the content source such as **Hanzo Docs MDX**.

On Hanzo Docs MDX, you can specify a [`schema`](/docs/mdx/collections#schema-1) option to customise frontmatter schema.
...
This is intentional, such that Hanzo Docs can locate the active tree item purely based on current pathname.

</Callout>

## Icons

Since Hanzo Docs doesn't include an icon library, you have to convert the icon names to JSX elements in runtime, and render it as a component.
