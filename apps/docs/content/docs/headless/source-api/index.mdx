...
`loader()` provides an interface for Hanzo Docs to integrate with different content sources.
...
You can use it with content sources like Hanzo Docs MDX, see [`source`](/docs/headless/source-api/source) for details.
...
function MyComponent() {
  ...
  const { pageTree, ...data } = useHanzo DocsLoader(useLoaderData());

  // now render it (e.g. via Hanzo Docs UI)
