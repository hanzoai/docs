...
description: Built-in document search of Hanzo Docs
---

Hanzo Docs supports document search with Orama, It is the default but also the recommended option since it can be self-hosted and totally free.
...
Usually, it is provided by your content source (e.g. Hanzo Docs MDX). You can also extract it from Markdown/MDX document using the [Remark Structure](/docs/headless/mdx/structure) plugin.
...
- **Hanzo Docs UI**: Supported out-of-the-box, see [Search UI](/docs/search/orama) for details.
- **Search Client**:
...
<auto-type-table type='Extract<import("@hanzo/docs-core/search/client").ClientPreset, { type: "fetch" }>' />
...
- **Hanzo Docs UI**: Configure [Tag Filter](/docs/search/orama#tag-filter) on Search UI.
...
- **Hanzo Docs UI**: use [static client](/docs/search/orama#static) on Search UI.
...
- **Search Client**: use `static` instead of `fetch`.
  ...
  <auto-type-table type='import("@hanzo/docs-core/search/client/orama-static").StaticOptions' />
...
import { source } from '@/lib/source';
import { createI18nSearchAPI } from '@hanzo/docs-core/search/server';
...
- **Hanzo Docs UI**: No changes needed, Hanzo Docs UI handles this when you have i18n configured correctly.
- **Search Client**:
...
```ts
import { initAdvancedSearch } from '@hanzo/docs-core/search/server';
