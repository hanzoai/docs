...
Hanzo Docs offers the **MDX Remote** package, it is a helper to integrate Markdown/MDX content sources with Hanzo Docs.
You can think it as a `next-mdx-remote` with built-in plugins for Hanzo Docs.
...
```tsx
import { createCompiler } from '@hanzo/docs-mdx-remote';
...
```tsx
import { createCompiler } from '@hanzo/docs-mdx-remote';
import { executeMdxSync } from '@hanzo/docs-mdx-remote/client';
import { DocsBody, DocsPage } from '@hanzo/docs-ui/layouts/docs/page';
import { getPage } from './my-content-source';
