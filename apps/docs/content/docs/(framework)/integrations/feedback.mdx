...
You can integrate a simple feedback system with Hanzo Docs.

## Installation

Install it using **Hanzo Docs CLI**.

```npm
npx @hanzo/docs-cli@latest add feedback
```
...
```tsx
import { DocsPage } from '@hanzo/docs-ui/layout/docs/page';
import { Feedback } from '@/components/feedback/client';
import ha from '@hanzo/insights';
    ...
    <DocsPage>
      ...
      <Feedback
        onSendAction={async (feedback) => {
          ...
          await ha.capture('on_rate_docs', feedback);
        }}
      />
    </DocsPage>
  );
...
For example, to report user feedback as a `on_rate_docs` event on Hanzo Insights.
...
```tsx title="source.config.ts (Hanzo Docs MDX)"
import {
  remarkFeedbackBlock,
  type RemarkFeedbackBlockOptions,
} from '@hanzo/docs-core/mdx-plugins/remark-feedback-block';
