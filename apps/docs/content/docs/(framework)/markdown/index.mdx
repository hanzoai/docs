...
Hanzo Docs provides many useful extensions to MDX, a markup language. Here is a brief introduction to the default MDX syntax of Hanzo Docs.

> MDX is not the only supported format of Hanzo Docs. In fact, you can use any renderer such as headless CMS.
>
...
Hanzo Docs support YAML-based frontmatter by default, `title` represents the page title (`h1`) in Hanzo Docs UI.
...
<Callout title='In Hanzo Docs MDX'>
...
```ts tab="Hanzo Docs MDX" title="source.config.ts"
...
import { remarkCodeTab } from '@hanzo/docs-core/mdx-plugins';
...
> This is only available on **Hanzo Docs MDX**.
