...
Hanzo Docs doesn't have a built-in Mermaid wrapper provided, we recommend using `mermaid` directly.

You can decide the Mermaid renderer to configure:

### Official Renderer

Install the required dependencies, `next-themes` is used with Hanzo Docs to manage the light/dark mode.
...
```tsx tab="Hanzo Docs MDX" title="source.config.ts"
import { remarkMdxMermaid } from '@hanzo/docs-core/mdx-plugins';
