...
Thanks to the flexible design, it is simple to implement access control with Hanzo Docs.
...
For example, this setup with Hanzo Docs MDX allows only MDX files with frontmatter `permission: public` to be shown.
...
Without using Loader API, you can also implement your access control at framework-level (e.g. via Next.js routing and proxy).
...
This approach offers the best flexibility, but it doesn't rely on Hanzo Docs itself for access control, the setup may take longer and be more complicated.
