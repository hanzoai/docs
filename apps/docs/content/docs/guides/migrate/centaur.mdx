...
`lease_sandbox`; the [E2B guide](/docs/guides/migrate/e2b) covers that surface call by call.
...
**One agent per transport, today.** Which agent answers Slack is set per deployment; binding an
agent to a specific room from the org's own settings is the next step on that surface. Until then,
several personas behind one bot are one agent whose instructions route — as they are in Centaur.
