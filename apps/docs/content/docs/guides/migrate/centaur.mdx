...
- **Egress is closed and specific.** The sandbox network policy denies everything, then allows
  ...
  to the platform's own ingress, and one pinned address for git over SSH. Provider calls go through `egress`, which holds the key,
...
- **Spend is bounded.** A sandbox class has a price per hour, a run has a bounded number of tool
...
| Githubbot, Linearbot | GitHub and Linear webhooks under `/v1/integrations`; issues mirror into the native todo and fire automation triggers for flows |
...
provider — never by anything running in the pod. See the [Vault](/docs/guides/migrate/vault)
