...
| `/v1/metrics` | The ingest doors — metric, log and trace writes, plus the reads over that bounded ring | 11 |
...
CPU and heap profiles have no ingest door. `POST /v1/o11y/traces/{traceId}/flamegraph`
...
**Two ingest doors, both counting in nanoseconds.** `/v1/metrics/logs/write` and
...
signal at `POST /v1/o11y/settings/ttl`. Choose the door deliberately. Note also that
Prometheus remote write sends milliseconds while both doors here read nanoseconds, and
