...
| **`hanzoai/cloud`** | `openapi.yaml` — the one description of the API, emitted from the handlers. Source of truth for **all** API reference. | Hand-written prose reference. |
...
truth stays with the code, at `hanzoai/cloud`. A `hanzo-docs/<team>` repo (any
...
- **API reference** ← `hanzoai/cloud` `openapi.yaml`, at the release
  `openapi-specs/.spec-lock` pins. `scripts/sync-openapi.sh` fetches that one
  file — digest-checked, from the forge or a sibling checkout, else the
  committed snapshot — and `scripts/gen-openapi-pages.ts` renders one page per
  product tag into `content/docs/openapi/<product>.mdx`, cross-linked to the
  matching prose guide. Every count on those pages is the document's own, and
  the reference used to render a projection of it published on its own clock:
  two descriptions of one API, disagreeing by nineteen releases.
...
API? Don't — the sentence lives next to the handler in `hanzoai/cloud`, and the
page regenerates from it. Documenting an SDK?
