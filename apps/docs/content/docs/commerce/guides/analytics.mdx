---
title: Analytics
description: Datastore-powered analytics with real-time dashboards
icon: ChartBar
---

Hanzo Commerce includes a powerful analytics system built on Datastore, providing real-time insights into sales, customers, products, and AI-powered recommendations.
...
```
Events → Kafka → Datastore → API → Dashboard
                     ↓
              AI Recommendations
```
...
```bash
DATASTORE_URL=clickhouse://default:@localhost:9000/commerce
```

One DSN carries host, credentials and database — commerce reads `DATASTORE_URL`
and nothing else, so a separate user, password or database variable is ignored.
