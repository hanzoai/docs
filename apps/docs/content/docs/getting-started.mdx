...
The `sk-` prefix identifies it as a Hanzo secret key -- server-side only. See [API Keys](/docs/api-keys) for a full breakdown of key types and prefixes.
...
| Prefix | Type | Purpose |
|--------|------|---------|
| `sk-*` | Secret Key | Server-side API access, and provider-level keys (e.g. upstream model providers). Scoped to a project or an org. Never expose client-side. |
