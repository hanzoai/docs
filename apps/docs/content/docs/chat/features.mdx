...
- **Zen** — `zen5`, `zen5-pro`, `zen5-coder`, `zen5-flash`, plus `zen-vl` for vision and `zen-guard` for safety
- **Carried models** — models from outside labs are available too, picked from the same model menu
...
|-----------|-------------|
| **Model** | Which model to use (e.g., `zen5`, `zen5-coder`) |
...
```bash
# Embeddings come from Hanzo. The RAG API's key/base-URL variables are named
# for the wire format they speak; point them at api.hanzo.ai and use a Hanzo key.
RAG_OPENAI_API_KEY=sk-your-api-key
RAG_OPENAI_BASEURL=https://api.hanzo.ai/v1
EMBEDDINGS_MODEL=zen-embedding
...
- **Supported models** — `zen-vl` for images in, text out; carried vision models work too

## Conversation Management

- **Persistent history** — all conversations stored in DocumentDB
