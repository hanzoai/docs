...
RAG_OPENAI_API_KEY=hk-your-api-key  # a Hanzo key
...
### Pointing embeddings elsewhere

The RAG API's key and base-URL variables are named for the wire format they
speak, not for a particular vendor. Leave them pointed at `api.hanzo.ai` to use
`zen-embedding`, or repoint them at another embeddings service that takes the
same request shape:

```bash
RAG_OPENAI_BASEURL=https://your-embeddings-host/v1
EMBEDDINGS_MODEL=your-model-id
```
