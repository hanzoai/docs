---
title: Workspace
description: Use Hanzo Docs MDX in multiple workspace.
---
...
<Callout title="Good to Know">
  Hanzo Docs MDX Workspace is not limited to the traditional meaning of workspace
  in package managers.
...
To integrate multiple sources in Hanzo Docs, use [`multiple`](/docs/headless/source-api/source#multiple-sources):
...
export const source = loader(
  {
    root: docs.toHanzoDocsSource(),
    'my-workspace': MyWorkspace.docs.toHanzoDocsSource(),
  },
  {
    baseUrl: '/docs',
  },
...
- when a deployment is triggered on $A$:
  - Hanzo Docs MDX handles each $T_n$ as a workspace.
  - each workspace generates its own collection entries, e.g. `collections/{repo}/server`.
  - Hanzo Docs `loader()` integrate multiple sources into one.
