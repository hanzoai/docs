...
The same ML-DSA implementation is used by the [Lux Network](/docs/services/lux) mainnet precompiles for on-chain post-quantum signature verification.
...
ConsensusKV uses the same post-quantum crypto library as the Lux Network:

- **ML-DSA precompile**: `lux/precompile/mldsa/` -- on-chain PQ signature verification
- **Quasar consensus**: `lux/consensus/protocol/quasar/` -- dual BLS+Ringtail threshold finality
- **Shared crypto**: `github.com/luxfi/crypto/mldsa` -- ML-DSA-44/65/87 implementation

The MPC private chain can optionally register as a **Lux subnet** (M-Chain or T-Chain) for network-level integration and cross-chain state anchoring.
