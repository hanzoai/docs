---
title: Hanzo MPC
description: Enterprise-grade multi-party computation for digital asset custody with blockchain-native consensus, dual-certificate post-quantum finality, 8 cryptographic protocols, and configurable t-of-n thresholds.
---
...
MPC nodes form their own **private BFT blockchain** for cluster state management, with **dual-certificate finality** combining Ed25519 (classical) and ML-DSA-65 (post-quantum, FIPS 204) signatures. Think of it as a self-hosted alternative to Fireblocks -- you own the infrastructure, control the key shares, and operate under your own security policies.

## Key Properties

- **No single point of failure**: No individual node ever holds a complete private key
- **Blockchain-native consensus**: MPC nodes are validators on their own private BFT chain
- **Dual-certificate finality**: Ed25519 + ML-DSA-65 (NIST Level 3) -- quantum-safe state management
...
- **KMS-first key management**: Identity and consensus keys stored in KMS/HSM when configured
...
```
                    ...
                    └──────────────┬───────────────────┘
                                   │ NATS JetStream
                    ┌──────────────▼─────────────────────────┐
                    │     MPC Nodes (StatefulSet / Validators) │
                    │                                          │
                    │  ┌────────┐ ┌────────┐  ...  ┌────────┐ │
                    ...
                    │       │          │                │      │
                    │  ┌────▼──────────▼────────────────▼───┐ │
                    │  │  Private BFT Blockchain (ConsensusKV)│ │
                    │  │  Dual-cert: Ed25519 + ML-DSA-65      │ │
                    │  │  t-of-n threshold finality            │ │
                    │  └────────────────────────────────────┘ │
                    │       │                                   │
                    │  ┌────▼──────────────────────────────┐   │
                    │  │    BadgerDB (encrypted)             │   │
...
| MPC Nodes | Go + Rust (crypto) | Key share storage, protocol execution, consensus validators |
| ConsensusKV | Private BFT chain | Cluster state (peers, keys, wallets) with dual-cert finality |
| Message Bus | NATS JetStream | Inter-node messaging + consensus transport |
| Storage | BadgerDB | Encrypted key share persistence |
| Auth | Hanzo IAM (hanzo.id) | Bearer token validation, RBAC |
| Identity | Ed25519 + ML-DSA-65 | Dual post-quantum node identity, stored in KMS/HSM |

### State Management Backends

All cluster state flows through an abstract KV interface. Three backends are available:

| Backend | Config | Transport | Finality | Use Case |
|---------|--------|-----------|----------|----------|
| `consensus` | `kv_backend: consensus` | NATS + Private BFT | Dual-cert threshold (<100ms) | **Production** (default) |
| `nats` | `kv_backend: nats` | NATS JetStream KV | JetStream replication | Development/staging |
| `consul` | `kv_backend: consul` | Consul HTTP | Raft | Legacy/parity testing |

See the [Consensus deep dive](/docs/services/mpc/consensus) for how the private blockchain works.
...
TOKEN=$(curl -s https://hanzo.id/v1/iam/login \
...
| Environment | URL | Backend | Threshold |
|-------------|-----|---------|-----------|
| Production (managed) | `https://mpc.hanzo.ai` | consensus | 3-of-5 |
| Production (decentralized) | `https://mpc.lux.network` | consensus | 3-of-5 |
| Staging | `https://stg.mpc.hanzo.ai` | nats | 2-of-3 |

## Next Steps

<Cards>
  <Card title="Consensus" href="/docs/services/mpc/consensus">
    How the private BFT blockchain and dual-certificate finality work
  </Card>
