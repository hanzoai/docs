...
Agents authenticate to the control plane with the same `sk-*` API key as the rest of the stack. The control plane:
...
- A SQL database (or SQLite for dev)
