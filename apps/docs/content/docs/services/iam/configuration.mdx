...
All integrations use the RFC 6749 / OpenID Connect Core 1.0 standard paths under
`/v1/iam/*`. There is ONE spelling per endpoint — the one OIDC discovery
advertises at `/v1/iam/.well-known/openid-configuration`:
...
Never `/api/*`, and never a second spelling of an endpoint discovery already
names. Resolve the endpoint from the discovery document rather than hardcoding
it, and a path change never becomes your outage.
