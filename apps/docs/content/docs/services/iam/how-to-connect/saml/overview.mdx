...
Hanzo IAM metadata URL format: `<iam-endpoint>/api/saml/metadata?application=admin/<application-name>`

Example: if Hanzo IAM is at `https://iam.hanzo.ai` and the application is `app-built-in`:

```text
https://iam.hanzo.ai/api/saml/metadata?application=admin/app-built-in
...
When integrating Hanzo IAM as a SAML IdP with external identity providers (like Azure AD), the `/api/acs` endpoint receives SAML responses. This endpoint is configured to accept cross-origin POST requests, allowing IdPs from different domains to send authentication data.
