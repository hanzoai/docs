...
description: Use Hanzo IAM as a SAML 2.0 identity provider.
---

Hanzo IAM can act as a **SAML 2.0 IdP**. This page covers SP configuration, Hanzo IAM IdP settings, and SAML attributes.

### Configuration in the SP (service provider)

The SP typically needs **Single Sign-On URL**, **Issuer**, and **Public Certificate**. Most SPs can fill these from the Hanzo IAM **XML Metadata URL** or by uploading the metadata file.

Hanzo IAM metadata URL format: `<iam-endpoint>/v1/iam/saml/metadata?application=admin/<application-name>`

Example: if Hanzo IAM is at `https://iam.hanzo.ai` and the application is `app-built-in`:

```text
https://iam.hanzo.ai/v1/iam/saml/metadata?application=admin/app-built-in
```
...
### Configuration in Hanzo IAM IdP

Hanzo IAM supports both GET and POST `SAMLResponse`. Configure the application in Hanzo IAM to match the `SAMLResponse` binding (GET or POST) that your SP supports.

When integrating Hanzo IAM as a SAML IdP with external identity providers (like Azure AD), the `/v1/iam/acs` endpoint receives SAML responses. It accepts **POST only**, and is configured to accept cross-origin POST requests so IdPs on other domains can post authentication data to it.

:::info

If you fill in the `Reply URL`, Hanzo IAM will send the `SAMLResponse` by **POST** Request. If the Reply URL is empty, Hanzo IAM will use **GET** request. You might wonder how Hanzo IAM knows the `Reply URL` of the SP if the `Reply URL` is empty. Actually, Hanzo IAM can get the URL called `AssertionConsumerServiceURL` by parsing the `SAMLRequest` and send the request with `SAMLResponse` to `AssertionConsumerServiceURL`. The `Reply URL` will overwrite the `AssertionConsumerServiceURL` in `SAMLRequest`.
...
If the service provider only needs NameID and not extra user attributes (Email, Name, DisplayName, Roles), enable **Disable SAML attributes** in the application settings. When enabled, Hanzo IAM will omit these attributes from the SAML response, which can help avoid XML namespace issues with certain SPs that have strict validation requirements.

### Assertion signature control

SAML responses from Hanzo IAM are always signed to ensure authenticity. However, some service providers may not support or require signed assertions within the response itself. Starting from version 2.81.0, Hanzo IAM added assertion signatures following SAML 2.0 best practices, but this caused compatibility issues with certain SPs like Sentry.

If a service provider does not handle signed assertions correctly, disable assertion signing while keeping the response envelope signed. Toggle the **Enable SAML assertion signature** option in your application settings to control this behavior. When disabled, Hanzo IAM will sign only the SAML response envelope, which maintains security while ensuring compatibility with a wider range of service providers.
...
After successfully logging in, the user profile in the returned `SAMLResponse` from Hanzo IAM has three fields. The attributes in the XML and the attributes of the user in Hanzo IAM are mapped as follows:
...
[gosaml2](https://github.com/russellhaering/gosaml2) is a SAML 2.0 implementation for Service Providers based on etree and goxmldsig, a pure Go implementation of XML digital signatures. We use this library to test the SAML 2.0 in Hanzo IAM as shown below.

Example: Hanzo IAM at `http://localhost:7001/`, application `app-built-in` in org `built-in`. Add `http://localhost:6900/acs/example` and `http://localhost:6900/saml/acs/example` to **Redirect URLs** in `app-built-in`.
...
func main() {
    res, err := http.Get("http://localhost:7001/v1/iam/saml/metadata?application=admin/app-built-in")
    if err != nil {
        panic(err)
    }
...
Click the URL to authenticate, and the login page of Hanzo IAM will be displayed.
