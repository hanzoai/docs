...
description: Connect to Hanzo IAM with any OIDC client using discovery endpoints.
---
...
```json
{
  ...
  "userinfo_endpoint": "https://iam.hanzo.ai/oauth/userinfo",
...
The following table illustrates how OIDC UserInfo fields (via the `/oauth/userinfo` API) are mapped from properties of Hanzo IAM's User table:
