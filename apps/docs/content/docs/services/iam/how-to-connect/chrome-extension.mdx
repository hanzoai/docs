...
description: Integrate Hanzo IAM OAuth in a Chrome extension.
---

The [iam-chrome-extension](https://github.com/iam/iam-chrome-extension) repo shows how to integrate Hanzo IAM in a Chrome extension. Summary of steps:

## Step 1: Deploy Hanzo IAM

Deploy Hanzo IAM in [production mode](/docs/basic/server-installation). Confirm the login page at `http://localhost:8000` works (e.g. sign in with `admin` / `123` in dev).

## Step 2: Configure the application

1. In Hanzo IAM, go to **Applications** and create or edit an application.
...
{
  "manifest_version": 3,
  "name": "Hanzo IAM Chrome Extension",
  "version": "1.0.0",
  "description": "Chrome extension integrated with Hanzo IAM",
  "permissions": [
    "identity",
    "storage"
  ],
...
Replace the configuration values with your own Hanzo IAM instance, especially the `client_id` and the host permissions URLs.
...
```javascript
const IAM_ENDPOINT = "http://localhost:8000";
...
function getAuthUrl() {
  ...
  const authUrl = `${IAM_ENDPOINT}/login/oauth/authorize?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=openid%20profile%20email&state=${state}`;
...
async function handleOAuthCallback(redirectUrl) {
  ...
  const tokenResponse = await fetch(`${IAM_ENDPOINT}/api/login/oauth/access_token`, {
    method: 'POST',
    headers: {
...
<body>
  <div id="login-section">
    <h2>Hanzo IAM Login</h2>
    <button id="login-btn">Login with Hanzo IAM</button>
  </div>
...
5. Click the **Login with Hanzo IAM** button to test the authentication flow.
...
async function refreshAccessToken(refreshToken) {
  const response = await fetch(`${IAM_ENDPOINT}/api/login/oauth/refresh_token`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
- [iam-chrome-extension](https://github.com/iam/iam-chrome-extension) - Official example repository
- [Hanzo IAM OAuth Documentation](/docs/how-to-connect/oauth)
- [Chrome Extension Identity API](https://developer.chrome.com/docs/extensions/reference/identity/)
- [Hanzo IAM JavaScript SDK](/docs/how-to-connect/sdk)
