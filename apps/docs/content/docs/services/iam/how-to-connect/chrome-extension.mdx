...
Deploy Hanzo IAM in [production mode](/docs/services/iam/basic/server-installation). Confirm the login page at `http://localhost:8000` works (e.g. sign in with `admin` / `123` in dev).
...
function getAuthUrl() {
  ...
  const authUrl = `${IAM_ENDPOINT}/oauth/authorize?client_id=${CLIENT_ID}&response_type=code&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=openid%20profile%20email&state=${state}`;
...
async function handleOAuthCallback(redirectUrl) {
  ...
  const tokenResponse = await fetch(`${IAM_ENDPOINT}/oauth/token`, {
    method: 'POST',
    headers: {
...
async function refreshAccessToken(refreshToken) {
  const response = await fetch(`${IAM_ENDPOINT}/oauth/token`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
- [Hanzo IAM OAuth Documentation](/docs/services/iam/how-to-connect/oauth)
- [Chrome Extension Identity API](https://developer.chrome.com/docs/extensions/reference/identity/)
- [Hanzo IAM JavaScript SDK](/docs/services/iam/how-to-connect/sdk)
