...
This page covers common problems when using Hanzo IAM’s built-in MCP server or when Hanzo IAM is the OAuth provider for your own MCP server.
...
**Symptom**: Browser console shows CORS errors when MCP client attempts to connect to Hanzo IAM.

**Causes**:

- Hanzo IAM CORS settings don't include the MCP client's origin
- Preflight requests are being blocked
- Wildcard CORS is disabled for security

**Fix**:

1. Log into Hanzo IAM admin panel
...
2. In Hanzo IAM, edit the application configuration
...
- Hanzo IAM configuration doesn't expose discovery endpoints
...
3. **OAuth Protected Resource Metadata** (RFC 9470):
   ...
   ```bash
   curl https://your-iam.com/.well-known/oauth-protected-resource
   ```
...
1. Navigate to your organization settings in Hanzo IAM
...
See [Dynamic client registration](/docs/services/iam/application/dynamic-client-registration) for details.
...
1. Edit your application in Hanzo IAM admin panel
...
2. Request a new token with the required scope:
   ...
   ```bash
   curl -X POST https://your-iam.com/oauth/token \
...
See [Authorization and scopes](/docs/services/iam/how-to-connect/mcp/authorization) for the scope reference.
...
2. **Check WWW-Authenticate header** on unauthorized requests:
   ...
   WWW-Authenticate: Bearer realm="mcp-server",
     authorization_uri="https://your-iam.com/oauth/authorize",
...
curl -s https://your-iam.com/.well-known/oauth-authorization-server | jq

# OIDC Configuration
curl -s https://your-iam.com/.well-known/openid-configuration | jq
...
- `iss` (issuer): Should match Hanzo IAM's authorization server URL
...
Hanzo IAM provides a token introspection endpoint (RFC 7662) to validate and inspect tokens:

```bash
curl -X POST https://your-iam.com/oauth/introspect \
...
# Open in browser:
https://your-iam.com/oauth/authorize?
...
- [Authorization and scopes](/docs/services/iam/how-to-connect/mcp/authorization) — Scope reference
- [Error handling](/docs/services/iam/how-to-connect/mcp/error-handling) — JSON-RPC error codes
- [Authentication](/docs/services/iam/how-to-connect/mcp/authentication) — OAuth and tokens
- [Custom scopes](/docs/services/iam/application/scopes) — Agent application scopes
- [Dynamic client registration](/docs/services/iam/application/dynamic-client-registration) — DCR setup
