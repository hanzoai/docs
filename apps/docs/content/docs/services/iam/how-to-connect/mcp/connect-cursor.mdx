...
description: Connect Cursor IDE to Hanzo IAM’s MCP server with OAuth.
---

Connect Cursor IDE to Hanzo IAM’s MCP server so you can manage applications, users, and resources from the editor.

## Prerequisites

- A running Hanzo IAM instance (HTTPS recommended for production)
...
Create a Hanzo IAM application for Cursor’s OAuth:
...
8. **(Optional) Category**: Set to `MCP` for better organization
   ...
   :::info
   See [Application categories](/docs/services/iam/application/categories) for Category and Type options.
   :::
...
Add your Hanzo IAM MCP server configuration:
...
See [Authorization and Scopes](/docs/services/iam/how-to-connect/mcp/authorization) for the complete list of available scopes.
...
- **Code Review**: Always review AI-generated code that interacts with Hanzo IAM before executing it

## Use Cases

With Cursor connected to Hanzo IAM's MCP server, you can:

- **Generate configuration code**: Ask Cursor to generate application setup code based on existing Hanzo IAM apps
- **Automate user management**: Create scripts to bulk-create users or update permissions
- **Documentation**: Generate documentation for your Hanzo IAM setup automatically
- **Testing**: Create test applications and clean them up programmatically
- **Migration scripts**: Build scripts to migrate configurations between Hanzo IAM instances

## Next Steps

Now that Cursor is connected to Hanzo IAM:

- Explore available [MCP Tools](/docs/services/iam/how-to-connect/mcp/tools) that Cursor can use
- Learn about [Authentication](/docs/services/iam/how-to-connect/mcp/authentication) methods
- Understand [Error Handling](/docs/services/iam/how-to-connect/mcp/error-handling) for better debugging
- Check out the [Integration Example](/docs/services/iam/how-to-connect/mcp/integration) for programmatic access

## Related Resources

- [MCP Server Overview](/docs/services/iam/how-to-connect/mcp/overview)
- [Authorization and Scopes](/docs/services/iam/how-to-connect/mcp/authorization)
- [Application Categories](/docs/services/iam/application/categories)
