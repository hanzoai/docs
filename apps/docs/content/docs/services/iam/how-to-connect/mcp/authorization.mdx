...
When you request a token from Hanzo IAM, include the scopes you need in your authorization request. The MCP server filters available tools based on these granted scopes, ensuring that automated processes can only perform actions they're explicitly authorized for.
...
Hanzo IAM's built-in MCP server supports scopes across multiple resource types. Each scope follows the `resource:action` pattern, where `resource` identifies the entity type and `action` specifies the operation level.
...
When building your own MCP server with Hanzo IAM as the OAuth provider, you can define custom scopes to match your server's capabilities. This allows fine-grained authorization beyond the built-in Hanzo IAM scopes.

To configure custom scopes:

1. Set your application's **Category** to "Agent" in the Hanzo IAM admin panel
...
See [Custom scopes](/docs/services/iam/application/scopes) for configuration.
...
When users authorize an MCP client, Hanzo IAM displays a consent screen showing the requested scopes. The consent screen uses the **Display Name** and **Description** fields you configure for each scope.

To control when the consent screen appears:

1. Navigate to your application configuration in Hanzo IAM
...
For authorization requirements beyond simple scopes, Hanzo IAM integrates with Casbin to provide fine-grained access control. Casbin policies can enforce complex rules based on:
...
1. Define a Casbin model in Hanzo IAM that describes your authorization rules
...
Casbin authorization runs in addition to scope validation. A request must pass both scope checking and Casbin policy enforcement to succeed. See [Permissions](/docs/services/iam/permission/overview) for Casbin integration.
