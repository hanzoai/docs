---
title: OAuth 2.0
description: Obtain, verify, and use access tokens with Hanzo IAM’s OAuth 2.0 endpoints.
---

Hanzo IAM issues **access tokens** for authenticating clients. This page describes how to get a token via the API, verify it, and use it. Alternatively use [Hanzo IAM SDKs](/docs/services/iam/how-to-connect/sdk) to handle the flow.
...
```url
https://<IAM_HOST>/oauth/authorize?
client_id=CLIENT_ID&
...
| email | email address |
| address | address (OIDC object in **JWT-Standard**; see [OIDC address claim](/docs/services/iam/token/overview#oidc-address-claim)) |
| phone | phone number |
...
After the user signs in, Hanzo IAM redirects to:
...
Hanzo IAM supports [PKCE](https://datatracker.ietf.org/doc/html/rfc7636) (Proof Key for Code Exchange) for enhanced security. To enable PKCE, add two parameters when requesting the authorization code:
...
For OAuth providers configured in Hanzo IAM (like Twitter and custom providers with PKCE enabled), Hanzo IAM automatically generates unique code verifiers for each authentication flow, so you don't need to manually implement PKCE.
...
Hanzo IAM supports RFC 8707 Resource Indicators, which lets you specify the intended service when requesting authorization. Add the `resource` parameter with an absolute URI identifying your service:
...
```url
https://<IAM_HOST>/oauth/authorize?client_id=CLIENT_ID&redirect_uri=REDIRECT_URI&response_type=token&scope=openid&state=STATE

After your user has authenticated with Hanzo IAM, Hanzo IAM will redirect them to:

```url
https://REDIRECT_URI/#access_token=ACCESS_TOKEN

Hanzo IAM also supports the [id_token](https://openid.net/specs/oauth-v2-multiple-response-types-1_0.html#id_token) as `response_type`, which is a feature of OpenID.
...
If your application doesn't have a frontend that redirects users to Hanzo IAM, then you may need this.
...
Enable **Client Credentials Grant** on the application and send a POST request to `https://<IAM_HOST>/oauth/token`:
...
Set the **Refresh Token** expiration in the application (default 0 hours), then send a POST request to `https://<IAM_HOST>/oauth/token`
...
To exchange a token, send a POST request to `https://<IAM_HOST>/oauth/token`:
...
Hanzo IAM supports three token types for `subject_token_type`:
...
Hanzo IAM currently supports the [token introspection](https://datatracker.ietf.org/doc/html/rfc7662) endpoint. This endpoint is protected by Basic Authentication (ClientId:ClientSecret).
...
Use the access token to call Hanzo IAM APIs that require authentication.

For example, there are two different ways to request `/oauth/userinfo`.

Type 1: Query parameter

`https://IAM_HOST/oauth/userinfo?accessToken=your_access_token`

Type 2: HTTP Bearer token

`https://IAM_HOST/oauth/userinfo` with the header: "Authorization: Bearer `your_access_token`"

Hanzo IAM will parse the access_token and return corresponding user information according to the `scope`.
...
The token is available through the `/v1/iam/get-account` endpoint:
...
- `/oauth/userinfo`: This API returns user information as part of the OIDC protocol. It provides limited information, including only the basic information defined in OIDC standards. For a list of available scopes supported by Hanzo IAM, see the [Scopes](#scopes) section.

- `/v1/iam/get-account`: This API retrieves the user object for the currently logged-in account. It is a Hanzo IAM-specific API that allows you to obtain all the information of the [user](/docs/services/iam/basic/core-concepts#user) in Hanzo IAM, including the OAuth provider's access token when applicable.
