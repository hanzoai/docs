...
Hanzo IAM issues **access tokens** for authenticating clients. This page describes how to get a token via the API, verify it, and use it. Alternatively use [Hanzo IAM SDKs](/docs/how-to-connect/sdk) to handle the flow.
...
- `/v1/iam/get-account`: This API retrieves the user object for the currently logged-in account. It is a Hanzo IAM-specific API that allows you to obtain all the information of the [user](/docs/basic/core-concepts#user) in Hanzo IAM, including the OAuth provider's access token when applicable.
