...
Deploy [Hanzo IAM](/docs/services/iam/basic/server-installation) and [Zabbix](https://www.zabbix.com/documentation/current/en/manual). Ensure both are running and you can sign in to each.
...
- SSO service URL: Corresponds to `Location="http://localhost:8000/v1/iam/saml/redirect/admin/zabbix"`. This is the URL of the SSO service provided by Hanzo IAM. When a user initiates an SSO request in Zabbix, they will be redirected to this URL for authentication.
