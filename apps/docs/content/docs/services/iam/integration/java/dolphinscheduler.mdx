...
Deploy Hanzo IAM. See [Server installation](/docs/services/iam/basic/server-installation). Ensure the server is running (e.g. `http://localhost:8000`) and you can open the login page (e.g. `http://localhost:7001`) and sign in with `admin` / `123`.
