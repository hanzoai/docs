...
description: Connect iam and Jira using the OIDC protocol as the IDP
---

This tutorial explains how to use [miniOrange](https://marketplace.atlassian.com/apps/1217688/mo-jira-oauth-sso-jira-openid-connect-sso-jira-oidc-sso?hosting=cloud&tab=overview) to connect iam and Jira.

This guide connects [Jira](https://www.atlassian.com/software/jira/guides/getting-started/overview) to [Hanzo IAM](/docs/basic/server-installation) via OIDC using the miniOrange plugin.
...
3. For local `prod` mode, set `IAM_HOSTNAME` to `http://localhost:8000`. See [production mode](/docs/basic/server-installation#production-mode).
...
6. Configure the app as follows: ![Jira Config](/img/integration/java/jira/Jira_Config.png)
   - `Token server URL`: **http://`IAM_HOSTNAME`/api/login/oauth/access_token**
   - `Authorization server URL`: **http://`IAM_HOSTNAME`/login/oauth/authorize**
