---
title: Nginx
description: Using Hanzo IAM with Nginx
---

Enable OpenID Connect-based single sign-on for applications proxied by NGINX Plus using Hanzo IAM as the identity provider (IdP).

This guide explains how to enable single sign-on (SSO) for applications that are being proxied by NGINX Plus. The solution uses OpenID Connect as the authentication mechanism, with [Hanzo IAM](https://hanzo.ai/) as the identity provider (IdP), and NGINX Plus as the relying party.
...
**Note:** The following procedure reflects the Hanzo IAM GUI at the time of publication, but the GUI is subject to change. Use this guide as a reference and adapt to the current Hanzo IAM GUI as necessary.

To create a Hanzo IAM client for NGINX Plus in the Hanzo IAM GUI, follow these steps:

1. Log in to your Hanzo IAM account at **`http://your-iam-url.com/login/`**.
...
6. On the **Add** page that opens, type a value in the **Name** and **Display Name** fields (here it is nginx-iam-role) and click the **Save** button.
...
9. Go back to the **Roles** page and click **Edit** on the **nginx-iam-role** row. In the opened page, in the **Sub users** field, select the username you just created (here it is built-in/user1).
...
3. Retrieve the URLs for the authorization endpoint, token endpoint, and JSON Web Key (JWK) file from the Hanzo IAM configuration. Open a terminal and execute the following `curl` command, piping the output to the indicated `python` command to generate a readable configuration format. For brevity, we have truncated the output to display only the relevant fields. <span id="jump1"></span>

    ```bash
    curl http://<iam-server-address>/.well-known/openid-configuration | python -m json.tool
    {
        "authorization_endpoint": "https://<iam-server-address>/login/oauth/authorize",
        "...":"...",
        "token_endpoint": "http://<iam-server-address>/api/login/oauth/access_token",
        "...":"...",
        "jwks_uri": "http://<iam-server-address>/.well-known/jwks",
        "...":"...",
    }
    ```
...
4. Open **/etc/nginx/conf.d/openid_connect_configuration.conf** using your preferred text editor. Modify the "default" parameter value for each of the following [map](https://nginx.org/en/docs/http/ngx_http_map_module.html#map) directives with the specified values:<span id="jump2"></span>
    - `map $host $oidc_authz_endpoint` – Use the value of the `authorization_endpoint` from [Step 3](#configuring-nginx-plus) (in this guide, `https://<iam-server-address>/login/oauth/authorize`)
    - `map $host $oidc_token_endpoint` – Use the value of the `token_endpoint` from [Step 3](#configuring-nginx-plus) (in this guide, `http://<iam-server-address>/api/login/oauth/access_token`)
    - `map $host $oidc_client` – Use the value in the **Client ID** field from [Step 4 of *Configuring Hanzo IAM*](#configuring-iam)
    - `map $host $oidc_client_secret` – Use the value in the **Client Secret** field from [Step 2 of *Configuring Hanzo IAM*](#configuring-iam)
...
5. Configure the JWK file based on the version of NGINX Plus being used:
    - In NGINX Plus R17 and later, NGINX Plus can directly read the JWK file from the URL specified as `jwks_uri` in [Step 3](#configuring-nginx-plus). Make the following changes to **/etc/nginx/conf.d/frontend.conf**:
        ...
        3. Update the "default" parameter of the `map $host $oidc_jwt_keyfile` directive to the value obtained from the `jwks_uri` field in [Step 3](#configuring-nginx-plus) (in this guide, `http://<iam-server-address>/.well-known/jwks`).
    - In NGINX Plus R16 and earlier, or if preferred, the JWK file must be located on the local disk. Follow these steps:
        1. Copy the JSON contents from the JWK file specified in the `jwks_uri` field in [Step 3](#configuring-nginx-plus) (in this guide, `http://<iam-server-address>/.well-known/jwks`) to a local file (e.g., `/etc/nginx/my_iam_jwk.json`).
