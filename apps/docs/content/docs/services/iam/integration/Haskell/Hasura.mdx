...
metaDescription: "Learn how to integrate Hanzo IAM with Hasura to secure your applications using JWT"
---

Deploy Hanzo IAM first (see [Server installation](/docs/services/iam/basic/server-installation)). Then configure the application and Hasura as below.

## Configure Hanzo IAM application

1. Create or use an existing Hanzo IAM application.
2. Add a redirect URL: `http://IAM_HOSTNAME/login`
   ![Hanzo IAM Application Setting](/img/integration/Haskell/Hasura/cas.png)
3. Copy the client ID; we will need it in the following steps.

## Add a user in Hanzo IAM
...
## Configure Hasura with Hanzo IAM
...
The `HASURA_GRAPHQL_JWT_SECRET` should be in the following format. Remember to change `<Hanzo IAM endpoint>` to your own Hanzo IAM's URL (like `https://iam.hanzo.ai`)
...
HASURA_GRAPHQL_JWT_SECRET: '{"claims_map": {
    ...
    },"jwk_url":"<Hanzo IAM endpoint>/.well-known/jwks"}'
...
```url
http://localhost:8000/oauth/authorize?client_id=<client ID>&response_type=code&redirect_uri=http%3A%2F%2Flocalhost%3A8080%2Flogin&scope=read&state=app-built-in<public certificate>>

Change `client ID` to the ID you copied before and input the public certificate of Hanzo IAM, from the Hanzo IAM **Certs** page.
...
Go back to the Hanzo IAM/Token page.
...
![Picture showing the access token from Hanzo IAM being used in Hasura](/img/integration/Haskell/Hasura/hasura.png)
