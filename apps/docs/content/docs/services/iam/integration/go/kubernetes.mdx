---
title: Kubernetes
description: Using Hanzo IAM for Authentication in Kubernetes
---
...
This article will guide you on how to configure authentication in Kubernetes using Hanzo IAM.
...
- A Hanzo IAM application like this [demo website](https://demo.iam.com/).
- kubectl command tool (optional).

:::note

Kubernetes `oidc-issuer-url` only accepts URLs which use the `https://` prefix.
So your Hanzo IAM application should be deployed on an HTTPS website.

:::

## Step 1: Creating a Hanzo IAM App and User Account for Authentication

In Hanzo IAM add an application (e.g. **Kubernetes**). Note **Name**, **Organization**, **Client ID**, and **Client secret**. Enable the grant types the cluster will use.

![Create an application in Hanzo IAM](/img/integration/go/kubernetes/Kubernetes_1.png)
...
![Add a user in Hanzo IAM](/img/integration/go/kubernetes/Kubernetes_2.png)
...
```shell
minikube start --extra-config=apiserver.oidc-issuer-url=https://demo.iam.com --extra-config=apiserver.oidc-client-id=294b09fbc17f95daf2fe
...
authentication can be performed by sending a POST request to the Hanzo IAM server.
Here is the code in Python which sends a POST request to the Hanzo IAM server
...
url = "https://demo.iam.com/api/login/oauth/access_token"
payload = json.dumps({
   "grant_type": "password",
  ...
  user:
    auth-provider:
      config:
        ...
        id-token: $(id_token)
        idp-issuer-url: https://demo.iam.com
