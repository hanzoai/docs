---
title: Grafana
description: Use Hanzo IAM as the OAuth provider for Grafana sign-in.
---

[Grafana](https://grafana.com/oss/grafana/) supports generic OAuth. Use Hanzo IAM as the IdP so users sign in to Grafana with their Hanzo IAM accounts. Ensure Grafana is installed and running.
...
![Create an application in Hanzo IAM](/img/integration/go/grafana/grafana_1.png)
...
auth_url = <Hanzo IAM endpoint>/login/oauth/authorize
token_url = <Hanzo IAM endpoint>/api/login/oauth/access_token
