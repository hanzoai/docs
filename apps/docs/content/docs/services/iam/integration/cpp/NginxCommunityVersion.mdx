...
description: Using Hanzo IAM with Nginx (Not Nginx-Plus) and Oauth2-Proxy
---
...
- Running Hanzo IAM service. If you haven't installed Hanzo IAM service yet, see [Server installation](/docs/services/iam/basic/server-installation) or [Try with Docker](/docs/services/iam/basic/try-with-docker).
...
1. Log in to your Hanzo IAM admin account.
...
6. Run oauth2-proxy with command-line parameters. Parameters marked with [required] must be configured according to your specific situation, while parameters marked with [optional] can optimize performance but can also be omitted. To run oauth2-proxy in the background, use a process manager such as `Screen`, `Supervisor`, or a terminal multiplexer.
    ...
    --oidc-issuer-url=https://auth.yourdomain.com \ #[required] Your Hanzo IAM URL (domain name or public IP)
    ...
    --scope=email+profile+groups+openid \ #[required] Obtained from Hanzo IAM: user email, user profile, groups, and login authentication
    ...
    --session-store-type=redis \ #[optional] Use KV cache. Omit if not using KV.
    --redis-connection-url=redis://127.0.0.1:6379/0 \ #[optional] Redis URL. Omit if not using Redis.
    --redis-password=123456 #[optional] KV password. Omit if not using KV or if KV has no password.
    ```
    ...
    **Note**: Adjust the config for your setup; Nginx version and environment may require changes.
    ...
    server {
        ...
        location ^~ / {
            ...
            # Note: This is not the Hanzo IAM deployment address or the Oauth2-Proxy running address, but the address where your backend service that needs login protection runs.
...
- Open the URL `project.yourdomain.com` in a browser -> Only see a page requiring login, including a button named "Sign in with OpenID Connect" -> Click the button and be redirected to your Hanzo IAM address, where they will be asked to log in -> Users enter their username and password, and Hanzo IAM verifies their credentials -> Automatically redirect back to your URL `project.yourdomain.com` -> Successfully access your service -> Users will be asked to log in again when the `--cookie-expire` time you set expires.
