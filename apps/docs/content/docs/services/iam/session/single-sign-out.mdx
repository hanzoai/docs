...
On SSO logout, Hanzo IAM sends a request to each notification provider configured for the application the user signed up with. The notifications include session IDs, access token hashes, and cryptographic signatures for secure, synchronized logout across all integrated systems.
...
To receive logout notifications, configure a notification provider (such as Custom HTTP, Telegram, or Slack) in your Hanzo IAM application's notification provider settings. For Custom HTTP providers:
...
This endpoint requires the user to be authenticated. Use any authentication method supported by Hanzo IAM:
...
For more details on authentication methods, see the [Hanzo IAM Public API](/docs/services/iam/basic/public-api#how-to-authenticate) documentation.
...
curl -X POST "https://iam.hanzo.ai/api/sso-logout?logoutAll=true" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
...
fetch('https://iam.hanzo.ai/api/sso-logout', {
  method: 'POST',
  headers: {
...
// Logout from current session only
fetch('https://iam.hanzo.ai/api/sso-logout?logoutAll=false', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${accessToken}`
  },
...
The API returns a standard Hanzo IAM response:
...
### Using Hanzo IAM SDKs

Most Hanzo IAM SDKs provide built-in support for SSO logout. Here are examples for different platforms:
...
Without a Hanzo IAM SDK, implement SSO logout by calling the logout endpoint:
...
async function logout() {
  ...
  try {
    // Use the full Hanzo IAM server URL in your application
    const response = await fetch('https://iam.hanzo.ai/api/sso-logout', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
...
```javascript
// ✅ Good: Uses HTTPS
const logoutUrl = 'https://iam.hanzo.ai/api/sso-logout';

// ❌ Bad: Uses HTTP (insecure)
const logoutUrl = 'http://iam.hanzo.ai/api/sso-logout';

### 4. Token Validation

Hanzo IAM validates the access token before processing the logout request. Ensure your token is valid and has not expired before making the logout call.
...
// Replace with your actual Hanzo IAM server URL
fetch('https://iam.hanzo.ai/api/sso-logout', {
  method: 'POST',
  headers: {
...
1. **Verify application integration**: Ensure all applications are properly integrated with Hanzo IAM and use the same organization
2. **Check token validation**: Make sure all applications validate tokens on each request
3. **Review session management**: Applications should not rely solely on local sessions; they must validate tokens with Hanzo IAM
...
- [Single Sign-On (SSO)](/docs/services/iam/session/single-sign-on): Learn how to enable SSO for your applications
- [Hanzo IAM Public API](/docs/services/iam/basic/public-api): Complete API reference including authentication methods
- [Tokens](/docs/services/iam/token/overview): Understand how Hanzo IAM manages access tokens and sessions
- [OAuth 2.0](/docs/services/iam/how-to-connect/oauth): Learn about OAuth 2.0 flows supported by Hanzo IAM
