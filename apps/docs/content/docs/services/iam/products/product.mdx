...
Attach one or more [payment providers](/docs/services/iam/provider/payment/overview) to the product. Hanzo IAM checks that the product’s **currency** matches each provider (e.g. USD product with PayPal); creation or update fails if there is a currency mismatch.

![product_provider.png](/img/products/product_provider.png)

### Success URL (Optional)

To redirect users to a custom URL after payment instead of the Hanzo IAM callback, set **Success URL**. When configured, Hanzo IAM will append the payment owner and transaction name as query parameters to your provided URL.
