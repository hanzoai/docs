...
Payments progress through several states during processing. When payment providers send notifications about completed transactions, Hanzo IAM updates both the payment and its related order atomically to keep them synchronized.
...
For transactions with external payment providers, Hanzo IAM creates transaction records only after receiving successful payment notifications. This approach guarantees that transactions reflect actual payments rather than just payment attempts. Balance-based payments work differently—they create transactions immediately since the balance check confirms payment success upfront.
