---
title: Webhooks
description: Send Hanzo IAM events to your application via HTTP webhooks for real-time integration.
---

Hanzo IAM can notify your application when events occur by sending HTTP `POST` requests with a JSON payload to a URL you configure. Use webhooks to react to sign-ups, logins, logouts, profile updates, and many other events.

## How webhooks work

When an event occurs in Hanzo IAM:

1. Hanzo IAM sends a `POST` request to the specified webhook URL.
...
1. In your Hanzo IAM instance, go to **Settings** → **Webhooks**.
2. Click **Add Webhook**.
...
6. Save. Hanzo IAM will send events to the URL from then on.

## Filtering webhook payloads

When working with webhooks, you might not always need the complete record data. Hanzo IAM allows you to filter the payload by configuring **ObjectFields** for each webhook. This feature is particularly valuable when you have privacy concerns, bandwidth constraints, or when your endpoint only processes specific fields.

The ObjectFields configuration accepts either "All" to receive the complete record, or a list of specific field names you want to include. When you specify field names, Hanzo IAM will send only those fields in the webhook payload, reducing payload size and exposing only the data your application needs.

If you configure multiple webhooks with different ObjectFields settings, each webhook operates independently. For example, one webhook might receive only user IDs and timestamps, while another receives full user profiles. Hanzo IAM ensures that the filtering applied to one webhook doesn't affect the data sent to others, even when they're triggered by the same event.
...
2. Copy the URL and set it as the webhook URL in Hanzo IAM.
3. Trigger an event (e.g. sign in to Hanzo IAM).
