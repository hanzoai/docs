---
title: Overview
description: User model, properties, roles, permissions, and bulk import in Hanzo IAM.
---

## User properties

Hanzo IAM manages user accounts. Each user has the following properties:
...
- `IsGlobalAdmin`: Indicates whether the user has permission to manage Hanzo IAM
...
- `Hanzo IAM`: User's unique identifier from another Hanzo IAM instance OAuth login
...
Organization admins have elevated permissions but are scoped to their organization only. Global admins (`built-in` organization users) have full access across all organizations in the Hanzo IAM instance.
...
- `guest-user`: Temporary users created through [guest authentication](/docs/services/iam/how-to-connect/guest-auth) without initial credentials
...
Custom tags can restrict application access. See [Application Tags](/docs/services/iam/application/tags).

## Identity verification

Hanzo IAM supports identity verification via ID Verification providers. Users can verify their identity by submitting their ID card information and real name, which gets validated through third-party services like Jumio.
...
This feature is useful for applications that require KYC (Know Your Customer) compliance or need to ensure user identity authenticity. See [ID Verification Providers](/docs/services/iam/provider/idv/overview) for more information on configuring identity verification.

## Email normalization

Hanzo IAM normalizes all email addresses to lowercase for uniqueness and to avoid duplicate accounts. This means that `user@example.com`, `User@Example.com`, and `USER@EXAMPLE.COM` are treated as the same email address, complying with RFC 5321 standards.
...
**Important:** You cannot change roles or permissions with `/v1/iam/update-user` (including via the `columns` parameter). Use the [Roles](/docs/services/iam/user/roles#managing-roles) and [Permissions](/docs/services/iam/user/permissions#managing-permissions) APIs and their management pages (e.g. **Roles** and **Permissions** in the sidebar) to assign and manage them.
...
After you choose an XLSX file, Hanzo IAM shows a preview of the data. Review it and confirm to start the import. A [sample file](https://github.com/iam/iam/blob/master/xlsx/user_test.xlsx) (`xlsx/user_test.xlsx`) with 5 test users is available for reference.
...
When migrating users from another system, you may need to preserve existing password hashes instead of re-hashing with the organization’s default password type. Use the `passwordType` field during user import to tell Hanzo IAM the format of the stored password.

:::note Example: bcrypt password
Example POST body for `/v1/iam/add-user`:
...
The password is already bcrypt-hashed, so `passwordType` is set to `"bcrypt"` so Hanzo IAM does not hash it again.
