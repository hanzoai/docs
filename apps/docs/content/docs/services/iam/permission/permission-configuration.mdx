...
This page describes the fields on the **Edit Permission** form, where you define permission policies for an [organization](/docs/services/iam/organization/overview).

## Opening the form

1. Sign in to Hanzo IAM.
...
**Organization** — The [organization](/docs/services/iam/organization/overview) that owns this policy. One org can have multiple permission policies. Choose from the dropdown.

**Name** — Unique identifier for the policy inside the organization. Must be unique; it is used when calling [Casbin APIs](/docs/services/iam/permission/exposed-casbin-apis).
...
:::tip
Design and test models in the [Casbin Online Editor](https://casbin.org/editor), then add them in Hanzo IAM.
:::
...
Hanzo IAM uses its own database to store permission policies:
...
Learn more about adapters in the [Adapter documentation](/docs/services/iam/permission/adapter).

### Adapter and Model relationship

In Hanzo IAM, adapters are **not configured per Permission**.
...
Which [users](/docs/services/iam/user/overview) the permission applies to; select them on the **Edit Permission** page.
...
For RBAC, select which [roles](/docs/services/iam/user/roles) apply to this permission on the **Edit Role** page.
...
Hanzo IAM does not use this field for external app authentication; you can ignore it or use it for your own categorization.
...
These resources are **not** the ones on the **Resources** page in the Hanzo IAM UI. Use any string here, e.g.:
...
Add multiple resources; Hanzo IAM creates permission rules for each resource–action combination.
...
Hanzo IAM **converts all these strings to lowercase** before storing them. Additionally, Hanzo IAM will **apply all actions to each resource**. You cannot specify that an action only takes effect on certain resources in this configuration page.
...
This option takes effect for Hanzo IAM itself to control application access.

:::info

If you want an external application to enforce permission controls using the interface Hanzo IAM exposes, this field won't do anything. You should describe the effect of pattern matching in the Model file using `allow` or `deny` rules.
...
- [Permission Overview](/docs/services/iam/permission/overview): Understand the basics of permissions in Hanzo IAM
- [Exposed Casbin APIs](/docs/services/iam/permission/exposed-casbin-apis): Use permissions in your external applications
- [Adapters](/docs/services/iam/permission/adapter): Configure policy storage adapters
- [Account Customization](/docs/services/iam/organization/accountCustomization): Configure View rule and Modify rule for user account fields
- [User Roles](/docs/services/iam/user/roles): Manage user roles in the **Edit Role** page
