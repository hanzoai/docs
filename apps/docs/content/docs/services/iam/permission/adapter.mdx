...
In Casbin, an **adapter** is the layer that loads and saves policy rules (e.g. to a database). Hanzo IAM lets you configure an adapter in the UI and run basic CRUD on policies.

## Adapter configuration

- **Type** — Adapter type (currently database only).
- **Host**, **Port**, **User**, **Password** — Database connection.
- **Database type** — MySQL, SQL, SQL Server, Oracle, or SQLite 3.
