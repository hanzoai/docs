...
description: Sync user data from an external database into Hanzo IAM.
---

The **database syncer** connects to an external database and syncs user data into Hanzo IAM. A demo users table can be imported from the [template XLSX](https://github.com/iam/iam/blob/master/xlsx/user_test.xlsx).
...
| **Database type** | MySQL, SQL, SQL Server, Oracle, or SQLite (Xorm-supported). |
...
| **Affiliation table** | Table used to sync user affiliation. Int codes can be mapped to names; see [getAffiliationMap()](https://github.com/iam/iam/blob/9f3ee275a8b747f914f0e74e897a79abeff96ccb/object/syncer_affiliation.go#L32) and [syncer_util](https://github.com/iam/iam/blob/9f3ee275a8b747f914f0e74e897a79abeff96ccb/object/syncer_util.go#L65) (e.g. `score` for int→string mapping). |
...
<video src="/video/syncer/update_iam.mp4" controls="controls" width="100%"></video>
...
<video src="/video/syncer/add_iam.mp4" controls="controls" width="100%"></video>
