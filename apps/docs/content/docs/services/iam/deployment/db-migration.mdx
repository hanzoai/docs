...
description: How Hanzo IAM handles database schema and data migrations.
---

Hanzo IAM uses [xorm](https://xorm.io/) for database access. Xorm handles many schema changes automatically, but **renaming columns** and some data migrations must be done manually via the [xorm migrate](https://pkg.go.dev/xorm.io/xorm/migrate) package.
