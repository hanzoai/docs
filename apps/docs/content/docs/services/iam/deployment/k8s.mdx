...
description: Deploy Hanzo IAM in a Kubernetes cluster using the example manifests.
---

## Deploy Hanzo IAM on Kubernetes

The Hanzo IAM repo includes an example manifest `k8s.yaml` in the project root, with a Deployment and a Service. For production or custom setups, consider using the [Helm chart](/docs/services/iam/basic/try-with-helm) instead.

Before deploying:

1. Update `conf/app.conf` so Hanzo IAM can connect to your database.
2. Ensure the database is running and the cluster can pull the Hanzo IAM image.
...
```yaml
# Example: deploying Hanzo IAM on Kubernetes
...
metadata:
  ...
  labels:
    app: iam
spec:
  # EDIT: use 1 replica if not using KV
  replicas: 1
  selector:
    matchLabels:
      app: iam
...
## Exposing Hanzo IAM with Ingress

Once Hanzo IAM is deployed, you typically want to expose it externally. There are several approaches to handle authentication for your applications:

### Direct Ingress Access

For simple setups where you want to expose Hanzo IAM itself, create an Ingress resource:
...
Your applications can then integrate with Hanzo IAM using the SDK or OAuth 2.0/OIDC protocols directly. This is the recommended approach for most use cases as it gives you full control over the authentication flow in your application code.

### Authentication with ingress-nginx auth annotations

While ingress-nginx provides external authentication capabilities through annotations, this approach has significant limitations when used with OAuth2/OIDC providers like Hanzo IAM. The `nginx.ingress.kubernetes.io/auth-url` annotation is designed for simple token validation endpoints, not for complete OAuth2 flows that require redirects and session management.
...
- Initiating the login redirect to Hanzo IAM
...
- **Session management** - For production deployments with multiple replicas, configure oauth2-proxy to use KV for session storage to ensure sessions work across pod restarts and multiple instances.
