...
description: How Hanzo IAM reports its version for binaries, Docker, and git builds.
---

Hanzo IAM exposes its version via the `/v1/iam/get-version-info` API so the running version can be checked in any environment.
...
```bash
curl http://localhost:8000/v1/iam/get-version-info
