...
description: Deploy Hanzo IAM frontend static assets to a CDN using a storage provider.
---
...
:::note
Build the frontend first. See [Server installation](/docs/services/iam/basic/server-installation#frontend) if needed.
:::

## Preparation

Create a [storage provider](/docs/services/iam/provider/storage/overview) in the Hanzo IAM UI (e.g. [Aliyun OSS](/docs/services/iam/provider/storage/aliyun-oss)).
...
The script is at [deployment/deploy_test.go](https://github.com/iam/iam/blob/7b0b426a76fd77b89817e0eafcccaed8d15b8cf4/deployment/deploy_test.go). Set the provider in `GetProvider()`; the ID format is `<owner>/<name>`.
