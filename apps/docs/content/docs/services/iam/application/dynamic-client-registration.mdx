...
**Dynamic Client Registration (DCR)** lets your software register an OAuth client with Hanzo IAM in one HTTP request instead of creating an application manually in the admin UI. That helps when you ship tools to end users: MCP clients, CLIs, or desktop apps can obtain client credentials at first run or install. Hanzo IAM implements [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591).
...
Use the `registration_endpoint` value (e.g. `/v1/iam/oauth/register`) for registration:

```json
{
  "issuer": "https://your-iam.com",
  "authorization_endpoint": "https://your-iam.com/oauth/authorize",
  "token_endpoint": "https://your-iam.com/oauth/token",
  "registration_endpoint": "https://your-iam.com/v1/iam/oauth/register",
  ...
}

## Registering a client

POST to `/v1/iam/oauth/register` with JSON metadata:

```bash
curl -X POST https://your-iam.com/v1/iam/oauth/register \
...
Your registration request needs at least one redirect URI. Everything else is optional, with Hanzo IAM applying sensible defaults:

- **redirect_uris** (required): Array of allowed callback URLs where Hanzo IAM redirects after authentication
...
const discovery = await fetch('https://your-iam.com/.well-known/openid-configuration')
  .then(r => r.json());
...
With these credentials, the client proceeds through the standard OAuth authorization code flow. The user authenticates in their browser, Hanzo IAM redirects back to your callback URL with an authorization code, and you exchange it for access tokens.

## Handling Registration Failures

When something goes wrong, Hanzo IAM returns RFC 7591 compliant errors with an `error` code and human-readable `error_description`. The most common issues: missing redirect URIs (`invalid_redirect_uri`), malformed parameters (`invalid_client_metadata`), or DCR being disabled for the organization (`access_denied`). Check the description field for specifics on what needs to be fixed.
