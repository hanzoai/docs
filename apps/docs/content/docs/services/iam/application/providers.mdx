...
| **Forget Password** | Send reset links or codes. See [Email provider](/docs/services/iam/provider/email/overview#password-reset-link) for reset links. |
| **Reset Password** | Use when the user resets their password. |
| **Set MFA** | Send codes during MFA setup. |
| **MFA Auth** | Use for MFA verification at login. See [MFA](/docs/services/iam/user/multi-factor-authentication). |
