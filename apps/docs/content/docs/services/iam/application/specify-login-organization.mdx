...
When a user’s session expires, Hanzo IAM remembers the last-used organization (stored in the browser when visiting e.g. `/login/my-org`). On the next sign-in, they are redirected to that organization’s login page instead of the generic `/login` page.
