...
For [invitations](/docs/services/iam/invitation/overview) with a shared application, Hanzo IAM generates organization-specific links with the `-org-{orgName}` suffix so users register in the correct organization.
