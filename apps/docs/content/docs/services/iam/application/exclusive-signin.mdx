...
For each sign-in with exclusive sign-in on: Hanzo IAM finds all sessions for that user and application, deletes them, creates a new session, and keeps only that session ID. This is done per application.
