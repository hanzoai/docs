...
description: Configure Hanzo IAM as the OAuth server for your MCP server.
---
...
- A running Hanzo IAM instance ([Server installation](/docs/services/iam/basic/server-installation) or [Hanzo IAM Cloud](https://iam.hanzo.ai))
...
When enabled, anyone can register new OAuth clients via `/v1/iam/oauth/register` without admin intervention. This is essential for MCP clients like Claude Desktop that auto-register on first use.
...
- **[OIDC Client →](../how-to-connect/oidc-client.md)**: Understand Hanzo IAM's OIDC capabilities
