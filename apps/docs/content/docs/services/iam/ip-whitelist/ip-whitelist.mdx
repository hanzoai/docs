...
Hanzo IAM can restrict entry pages (login, sign-up, forgot-password) by client IP. Access is allowed only if the client IP is in the configured allowlist. Allowlists can be set at **user**, **application**, and **organization** levels; all applicable levels are checked.

## Configuration

### User level

Hanzo IAM checks the user-level allowlist first.

1. Add the **IP allowlist** account item on the organization’s edit page (see [Account customization](/docs/services/iam/organization/accountCustomization)).
...
If the request passes the user-level check, Hanzo IAM then checks the application allowlist. Configure **IP allowlist** on the application edit page.
