...
- SP ACS URL and Entity ID: `http://localhost:8000/v1/iam/acs`.

:::note

The `/v1/iam/acs` endpoint only accepts POST requests. Ensure Keycloak is configured to use HTTP POST binding for SAML responses.
...
- **Client ID**: `http://localhost:8000/v1/iam/acs` - This will be the SP Entity ID used in the Hanzo IAM configuration later.
- **Client Protocol**: `saml`.
- **Client SAML Endpoint**: `http://localhost:8000/v1/iam/acs` - This URL is where you want the Keycloak server to send SAML requests and responses. Generally, applications have one URL for processing SAML requests. Multiple URLs can be set in the Settings tab of the client.
...
10. **Valid Redirect URIs** - Add `http://localhost:8000/v1/iam/acs`.
11. **Master SAML Processing URL** - `http://localhost:8000/v1/iam/acs`.
12. Fine Grain SAML Endpoint Configuration
    1. **Assertion Consumer Service POST Binding URL** - `http://localhost:8000/v1/iam/acs`.
    2. **Assertion Consumer Service Redirect Binding URL** - `http://localhost:8000/v1/iam/acs`.
