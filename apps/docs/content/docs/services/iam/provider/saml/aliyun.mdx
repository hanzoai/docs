...
description: Use Alibaba Cloud IDaaS (EIAM) as a SAML IdP for Hanzo IAM.
---
...
- IDP IdentityId: Keep the same as Issuer URL in Hanzo IAM.
- SP Entity ID & SP ACS URL (SSO Location): Use placeholders for now; after configuring Hanzo IAM, set both to `https://<your-iam-domain>/v1/iam/acs`.
- Assertion Attribute: Directly fill in as username.
- Account Association Mode: Account Association

:::note

The ACS URL (`/v1/iam/acs`) only accepts POST requests. Ensure your IdP is configured to use HTTP POST binding.
...
The primary account exists in IDaaS, and the sub account is the ID of the user in Hanzo IAM.
...
![Hanzo IAM provider](/img/providers/SAML/aliyun_iam.png)

Copy the **SP ACS URL** and the **SP Entity ID** and click the **Save** button.

Edit the application you want to configure in Hanzo IAM. Select the provider just added and click the button **Save**.

![Add provider for app](/img/providers/SAML/aliyun_iam_provider.png)
...
Fill in **SP Entity ID** and **SP ACS URL(SSO Location)** with the content copied in Hanzo IAM. Submit and enable application.

![Modify SAML app](/img/providers/SAML/aliyun_saml_modify_input.png)

## Test

Open the application’s login page; an IDaaS icon appears. Click it to sign in via Alibaba Cloud IDaaS; after success you are logged into Hanzo IAM.

![Hanzo IAM login](/img/providers/SAML/aliyun_iam_login.gif)
