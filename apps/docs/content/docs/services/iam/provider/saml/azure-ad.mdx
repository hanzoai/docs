...
- **Identifier (Entity ID)**: `https://<your-iam-domain>/v1/iam/acs`
  - Example: `https://door.example.com/v1/iam/acs`
- **Reply URL (Assertion Consumer Service URL)**: `https://<your-iam-domain>/v1/iam/acs`
  - Use the same URL as Entity ID

:::note

The `/v1/iam/acs` endpoint only accepts POST requests. Azure AD uses POST binding by default for SAML responses.
