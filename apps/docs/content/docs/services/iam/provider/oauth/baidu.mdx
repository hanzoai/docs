...
2. Set the redirect URL in the app settings. **Add your Hanzo IAM domain in the domain setting**, not in the callback URL field — Baidu’s callback URL validation often fails if you put the full URL there, and only one domain or URL is allowed (unlike the docs).
...
4. In Hanzo IAM add an **OAuth** provider, set **Type** to **Baidu**, and enter **Client ID** and **Client secret**.
