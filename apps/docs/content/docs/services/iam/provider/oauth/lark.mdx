...
On the [Lark Open Platform](https://open.feishu.cn/), create an application and enable it. In the app’s basic information, note **App ID** and **App Secret**. In security settings, add the redirect URL: `<your-iam-domain>/callback` (e.g. `https://door.example.com/callback`).
