...
3. In the app, enable **Sign In with LinkedIn** and add **Authorized redirect URLs**: use **Hanzo IAM’s callback URL**. In Hanzo IAM, set the application **Redirect URL** to your application’s callback URL. See [Application config](/docs/services/iam/application/config#how-the-flow-works).
...
5. In Hanzo IAM add an **OAuth** provider, set **Type** to **LinkedIn**, and enter **Client ID** and **Client secret**.
