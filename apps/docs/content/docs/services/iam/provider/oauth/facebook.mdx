...
Under **Facebook Login** → **Settings**, set **Valid OAuth Redirect URIs** to **Hanzo IAM’s callback URL**. In Hanzo IAM, the application **Redirect URL** is your application’s callback URL. See [Application config](/docs/application/config#how-the-flow-works).
