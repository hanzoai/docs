...
| **Path prefix** | Prefix for object keys (default `/`). With prefix `abcd/xxxx`, a file is stored at e.g. `https://cdn.example.com/abcd/xxxx/iam/avatar.png`. |
...
Supported: **Hanzo S3** (and any other S3-compatible endpoint, configured through the S3 provider type with its **Endpoint** set), **AWS S3**, **Azure Blob Storage**, **Alibaba Cloud OSS**, **Tencent Cloud COS**. More providers may be added over time.
