...
[MetaMask](https://metamask.io/) is a wallet and gateway to dApps. Hanzo IAM can use it as a Web3 identity provider for “Sign in with MetaMask”.

## 1. Create the provider in Hanzo IAM
...
:::tip
- Authorize only one Ethereum address per user; Hanzo IAM binds one address per account.
- To use a different address, disconnect the current one in Hanzo IAM first, then sign in again with the new address.
