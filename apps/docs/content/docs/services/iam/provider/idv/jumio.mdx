...
description: Use Jumio as an ID verification (KYC) provider in Hanzo IAM.
---

[Jumio](https://www.jumio.com/) verifies government-issued IDs and supports KYC flows. Configure it as an **ID Verification** provider in Hanzo IAM.
...
1. Hanzo IAM calls the Jumio API with the user's ID information
...
5. Upon successful verification, the user's account is marked as verified in Hanzo IAM
