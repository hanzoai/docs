...
description: Let users pay with their Hanzo IAM account balance (wallet).
---

The **Balance** payment provider lets users pay for products from their account balance instead of an external gateway. Hanzo IAM deducts the price from the user’s balance; no redirect to a third party.
