---
title: Overview
description: How Hanzo IAM uses OAuth tokens, JWT formats, and token lifecycle.
---
...
- **`Location`** (string): A general location string (e.g., `"New York"`). This field is used as the plain-string `address` value returned by the `/oauth/userinfo` endpoint when the `address` scope is requested.
