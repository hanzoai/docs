...
Create `conf/app.conf` (copy from [conf/app.conf](https://github.com/iam/iam/blob/master/conf/app.conf)); see [Config files](/docs/services/iam/basic/server-installation#config-files) for options. Then run:
...
Place `app.conf` in a `conf/` directory next to `docker-compose.yml` (copy from [app.conf](https://github.com/iam/iam/blob/master/conf/app.conf); see [Config files](/docs/services/iam/basic/server-installation#config-files)). Start Hanzo IAM and the database with:
