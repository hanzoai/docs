...
The backend reads a single config file: [**conf/app.conf**](https://github.com/iam/iam/blob/master/conf/app.conf). For a minimal setup, set `driverName` and `dataSourceName` for your database (see [Configure database](/docs/basic/server-installation#configure-database)). The table below lists every option.
