---
title: Hanzo Search
description: AI-powered search engine built in Rust with sub-50ms response times, hybrid semantic/full-text search, faceted filtering, geo search, chat completions, and multi-language support.
---

import { Cards, Card } from '@hanzo/docs-ui/components/card'

> **API reference** · [Hanzo Search API →](/docs/openapi/search) — every endpoint, generated from the OpenAPI spec.
...
**Default Port:** `7700`
**Source:** [github.com/hanzoai/search](https://github.com/hanzoai/search)
...
- **Chat Completions** -- a chat endpoint that searches your indexes and answers from what it finds
...
Hanzo Search includes a chat completions endpoint: the model searches your indexes and answers from what it finds, streaming the reply over SSE. It takes and returns the chat-completions JSON shape, so an existing HTTP client works against it:
...
| `SEARCH_ENV` | `development` | `development` or `production` |
| `SEARCH_DB_PATH` | `./data.ms` | Database storage directory |
| `SEARCH_HTTP_ADDR` | `localhost:7700` | Bind address and port |
| `SEARCH_HTTP_PAYLOAD_SIZE_LIMIT` | `100 MB` | Maximum request payload size |
| `SEARCH_LOG_LEVEL` | `INFO` | Log level: OFF, ERROR, WARN, INFO, DEBUG, TRACE |
| `SEARCH_NO_ANALYTICS` | `false` | Disable telemetry |
| `SEARCH_SCHEDULE_SNAPSHOT` | `false` | Enable periodic snapshots |
| `SEARCH_SNAPSHOT_DIR` | `snapshots/` | Snapshot storage directory |
| `SEARCH_DUMP_DIR` | `dumps/` | Dump storage directory |
...
|----------|-------------|
| `SEARCH_S3_BUCKET_URL` | S3 endpoint URL |
| `SEARCH_S3_BUCKET_REGION` | Bucket region |
| `SEARCH_S3_BUCKET_NAME` | Bucket name |
| `SEARCH_S3_ACCESS_KEY` | Access key |
| `SEARCH_S3_SECRET_KEY` | Secret key |
...
<Cards>
  ...
  <Card title="Hanzo Nexus" href="/docs/services/nexus">
