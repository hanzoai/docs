...
> **API reference** · [Sql →](/docs/openapi/sql) — every endpoint, generated from the OpenAPI spec.
...
**Gateway:** `api.hanzo.ai/v1/sql`
**In-cluster:** `postgres.hanzo.svc:5432`
**Version:** PostgreSQL 16 with pgvector 0.7+

## Features

- **Serverless SQL**: Auto-scaling compute that scales to zero when idle and spins up on demand
...
- **SQL and REST APIs**: Connect via standard SQL wire protocol or HTTP REST
...
All Hanzo SQL endpoints include a built-in PgBouncer connection pooler to efficiently multiplex application connections to SQL backends.
...
A database restores to any second within the backup retention window, and to an
on-demand snapshot taken before a migration. A restore lands as a NEW database —
verify its state before swapping it with production.

## Read Replicas

Scale read-heavy workloads by routing queries to a read replica:
...
Hanzo SQL exports SQL metrics to Hanzo Analytics and Prometheus:
...
<Cards>
  ...
  <Card title="Analytics" href="/docs/services/analytics">
    Unified analytics and query engine (Datastore)
  </Card>
</Cards>
