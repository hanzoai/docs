...
> **API reference** · [Hanzo Base — Collections →](/docs/openapi/base) — every endpoint, generated from the OpenAPI spec.
...
- **Realtime**: Subscribe to record changes over SSE or WebSocket
...
For production multi-instance deployments that need a shared writer (e.g. leader-elected cron), Base runs on **SQL** via `pgx`. SQLite remains the default everywhere else.

```
Local dev / single node   →  SQLite (encrypted, per-org)
Production multi-instance  →  SQL (pgx)
```
...
Subscribe to record changes over Server-Sent Events, or over a WebSocket transport.
...
Under the hood the client opens a stream, receives a `clientId`, and posts the subscriptions it wants (`posts`, `posts/RECORD_ID`). Subscriptions are gated by each collection's list/view rules.
