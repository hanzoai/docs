...
See [Node Pools](/docs/services/platform/clusters/node-pools) for instructions on adding capacity, configuring autoscaling, and managing node pool lifecycle after the cluster is created.
