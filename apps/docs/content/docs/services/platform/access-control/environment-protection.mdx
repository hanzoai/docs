...
**Protection rules and approvals have no REST endpoint.** Both are Studio actions — set the level under **Environments > Protection**, approve under **Deployments > Pending**, as described above.

What the API host does expose is the deploy itself, and its history:

| Method | Path | Description |
|--------|------|-------------|
| `POST` | `https://api.hanzo.ai/v1/platform/projects/{project}/apps/{app}/deploy` | Roll the app forward |
| `GET` | `https://api.hanzo.ai/v1/platform/projects/{project}/apps/{app}/deployments` | Deployment history |
| `GET` | `https://api.hanzo.ai/v1/platform/projects/{project}/apps/{app}/deployments/{id}` | One deployment, with its status |

A deploy into a **Restricted** environment still lands in `pending` — the call returns, the rollout waits for a reviewer. Poll the deployment to watch it clear.

<Callout type="warn">
There is no `environment.update` or `environment.approveDeployment` REST route, and no `/api` prefix anywhere in Hanzo. `app.platform.hanzo.ai` redirects to `platform.hanzo.ai`, the Studio's browser UI: its `/v1/*` answers `401 Unauthorized` to every path, including ones that do not exist, so nothing there is a programmatic surface. The API host is `api.hanzo.ai`.
</Callout>
