...
`platform.hanzo.ai` is the **Studio** — the browser UI, not an API host. Every programmatic call goes to `api.hanzo.ai/v1/platform`; paths the Studio does not route fall through to its HTML shell with a `404`. There is no `x-api-key` header, no tRPC procedure surface, and no `/api` prefix anywhere in Hanzo.
