...
> **API reference** · [Functions →](/docs/openapi/functions) — every endpoint, generated from the OpenAPI spec.
...
```bash
# Publish a function from source
hanzo functions create --name on-order \
  --runtime python3.12 \
  --handler handler.handle \
  --code ./handler.py \
  --env-names PAYMENT_API_KEY

# Publish from a container image
hanzo functions create --name processor \
...
# The output of a function's most recent run
hanzo functions logs on-order

# Invoke directly
hanzo functions invoke on-order --input '{"id":"order-123","total":59.99}'

# What triggers are wired
