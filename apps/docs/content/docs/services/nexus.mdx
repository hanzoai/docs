...
- **Models**: enso and Zen by default, with Ollama, HuggingFace and outside providers connectable
...
Nexus calls Hanzo's own models by default — `enso` and Zen on `api.hanzo.ai`,
with the key you already have. Connect an outside provider when a workflow
needs a specific model from one.

| Provider | Models |
|----------|--------|
| Hanzo | `enso`, `enso-ultra`, `zen5`, `zen5-coder`, `zen-embedding` |
...
```bash
curl -X POST https://nexus.hanzo.ai/v1/chat \
