...
**One API key. One binary. One surface at `api.hanzo.ai`.** Every service below is a native subsystem of the single Hanzo Cloud binary, served under `/v1` and described by the document the binary emits, [hanzoai/cloud](https://github.com/hanzoai/cloud)'s `openapi.yaml` — no separate gateway or ingress tier to wire up.
...
<Cards>
  <Card title="Chat" href="/docs/services/chat">AI chat on the enso and Zen models, with MCP tools.</Card>
  <Card title="Bot" href="/docs/services/bot">Agent gateway — channels, skills, and an HTTP API.</Card>
