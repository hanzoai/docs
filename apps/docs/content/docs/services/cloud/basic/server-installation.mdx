...
Since we must choose a database when opening SQL with xorm, you should prepare a database manually before running Hanzo Cloud.
...
For SQL, make sure `dataSourceName` has non-empty `dbName` and leave the standalone `dbName` field empty like the above example.
