...
|----------------------------------------------------------------------------|--------------------------------------------------------------|-----------------------------------------------------------------------------|
| [cloud-all-in-one](https://hub.docker.com/r/casbin/cloud-all-in-one) | Both Hanzo Cloud and a MySQL database are included in the image | This image already includes a toy database and is only for testing purposes |
| [cloud](https://hub.docker.com/r/casbin/cloud)                       | Only Hanzo Cloud is included in the image                       | This image can be connected to your own database and used in production     |

1. casbin/cloud-all-in-one: This image includes the cloud binary, a MySQL database, and all the necessary configurations. It is designed for new users who want to try Hanzo Cloud quickly. With this image, you can start Hanzo Cloud immediately with just one or two commands, without any complex configuration. However, please note that we **do not recommend** using this image in a production environment.
...
```shell
docker run -p 14000:14000 casbin/cloud-all-in-one
...
:::note
**Hanzo IAM**: By default Hanzo Cloud uses the hosted Hanzo IAM instance at `https://door.iam.com` for user authentication. If you need to manage users, applications, or customize the authentication flow, you must deploy your own Hanzo IAM instance and update Hanzo Cloud's `app.conf` (or the equivalent environment variables) to point to your Hanzo IAM server. You can look at **[Hanzo IAM configuration](/docs/basic/server-installation#configure-iam)** for more details.
...
```bash
docker run  -p 14000:14000 -v /folder/of/app.conf:/conf casbin/cloud:latest
