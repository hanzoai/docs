...
Hanzo Kafka is a stateless Kafka wire protocol gateway built on Hanzo PubSub. It accepts connections from any standard Kafka client and translates every operation into Hanzo Stream calls against PubSub. All storage, replication, and durability is delegated to Hanzo PubSub -- the broker itself is completely stateless and horizontally scalable.

**Endpoint:** `kafka.hanzo.ai:9092`
**Gateway:** `api.hanzo.ai/v1/kafka/*`
**Source:** [github.com/hanzoai/kafka](https://github.com/hanzoai/kafka)
...
echo "hello world" | kcat -b kafka.hanzo.ai:9092 -t my-topic -P

# Consume from the beginning
kcat -b kafka.hanzo.ai:9092 -t my-topic -C -o beginning
```
...
kafka-topics.sh --create --topic events \
  --partitions 3 \
  --bootstrap-server kafka.hanzo.ai:9092
...
producer = KafkaProducer(
    bootstrap_servers=["kafka.hanzo.ai:9092"],
...
func main() {
    ...
    producer, err := sarama.NewSyncProducer(
        []string{"kafka.hanzo.ai:9092"}, config,
    )
    if err != nil {
        log.Fatal(err)
    }
...
props.put("bootstrap.servers", "kafka.hanzo.ai:9092");
...
cprops.put("bootstrap.servers", "kafka.hanzo.ai:9092");
...
Hanzo Kafka translates Kafka concepts directly to Hanzo Stream primitives:
...
Run your own Hanzo Kafka instance with Docker:

```bash
docker run -d --name kafka \
  -p 9092:9092 \
  ghcr.io/hanzoai/kafka:latest \
...
```bash
git clone https://github.com/hanzoai/kafka.git
cd kafka
go build -o hanzo-kafka .
./hanzo-kafka --pubsub-url nats://localhost:4222
```

## Performance

Hanzo Kafka is optimized for high-throughput streaming workloads:
...
kafka-producer-perf-test.sh \
  ...
  --producer-props \
    ...
    batch.size=16384 \
    linger.ms=5 \
    bootstrap.servers=kafka.hanzo.ai:9092
```
...
<Cards>
  <Card title="PubSub" href="/docs/services/pubsub">
    Hanzo PubSub -- the persistence layer backing Hanzo Kafka
  </Card>
