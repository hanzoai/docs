---
title: Hanzo KV
description: KV-compatible in-memory key-value store with pub/sub, streams, Lua scripting, and cluster mode.
---

# Hanzo KV

> **API reference** · [Hanzo KV API →](/docs/openapi/kv) — every endpoint, generated from the OpenAPI spec.

Hanzo KV is a managed, KV-compatible in-memory key-value store providing sub-millisecond reads and writes, pub/sub messaging, stream processing, and rich data structures. It speaks the KV wire protocol (RESP3), so any KV or KV client works out of the box.

**Endpoint:** `kv.hanzo.ai`
**Gateway:** `api.hanzo.ai/v1/kv/*`
**Protocol:** KV wire protocol (RESP2/RESP3) on port `6379`
...
Connect directly using any KV-compatible CLI.
...
# Alternative KV-style URI (used by some services)
...
<Cards>
  ...
  <Card title="DB" href="/docs/services/sql">
    Serverless SQL for relational data and persistence.
  </Card>
