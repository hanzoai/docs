...
- **Local HTTP API**: Serves `/v1/*` on localhost -- chat completions, completions, and models
...
|       +-- serve.rs        # Local HTTP server on /v1/* (Axum)
...
# Start the local API server on /v1/*
...
| `serve` | Start the local HTTP server on `/v1/*` |
...
`serve` starts a local HTTP server holding the model in memory:
...
These endpoints take and return the same JSON shapes as the widely-implemented
chat-completions format, so an HTTP client written against that shape works
once its base URL points at the local server.
...
### Python (existing chat-completions clients)

Any Python client built on the chat-completions shape reaches the local server
by pointing its base URL at it:
...
<Cards>
  ...
  <Card title="Gateway" href="/docs/services/gateway">
    The Hanzo API gateway -- one host and one key in front of Engine and the rest of Hanzo Cloud
  </Card>
