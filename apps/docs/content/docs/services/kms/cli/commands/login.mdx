---
title: "kms login"
description: "Login into Hanzo KMS from the CLI"
---
...
<Accordion title="Native Kubernetes">
  ...
  <Accordion title="JWT Auth">
    ...
    <Steps>
      ...
      ```bash
        kms login --method=jwt-auth --jwt=<jwt-token> --machine-identity-id=<machine-identity-id>
      ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="--organization-slug">
    ```bash
    kms login --method=<auth-method> --organization-slug="team2" 
    ```
   #### Description
  ...
  <Accordion title="--email">
    ```bash
    kms login --email=<email> --password=<password> --organization-id=<organization-id>
    ```
  ...
  <Accordion title="--plain">
    ```bash
    kms login --email=<email> --password=<password> --organization-id=<organization-id> --plain
    ```
  ...
  <Accordion title="--jwt">
    ```bash
    kms login --jwt=<jwt-token> --machine-identity-id=<machine-identity-id>
    ```
  ...
  <Accordion title="--domain">
    ...
    kms login --domain="https://kms.hanzo.ai"

    # Example for localhost
    kms login --domain="http://localhost:8080"

    # Example for self-hosted
    kms login --domain="https://your-self-hosted-kms.hanzo.ai"
    ```
...
<AccordionGroup>
  <Accordion title="Browser Login (Default)">
    ...
    The browser will open to the Hanzo KMS login page, and upon successful authentication, the CLI will be automatically authenticated.
  ...
  <Accordion title="Direct Login (Non-Interactive)">
    ...
    kms login --domain https://kms.hanzo.ai --email user@example.com --password "your-password" --organization-id "your-organization-id"
...
In this example we'll be using the `universal-auth` method to login to obtain a Hanzo KMS access token, which we will then use to fetch secrets with.
