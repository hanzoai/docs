...
**Access scope**: the shared services org, `production` environment, all paths.
...
A machine identity is a client-credentials application in [Hanzo IAM](/docs/services/iam). Register it (which mints a `clientId`/`clientSecret`), then grant it read access to the appropriate KMS org. The credentials are what your workload exchanges for a token.
...
Use the returned `accessToken` as `Authorization: Bearer <accessToken>` on every secret read.
