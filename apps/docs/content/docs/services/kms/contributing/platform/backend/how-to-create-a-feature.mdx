...
Suppose you're interested in implementing a new feature in Hanzo KMS's backend, let's call it "feature-x." Here are the general steps you should follow.
...
1. If you haven't done it yet, create a new `.env.migration` file at the root of the Hanzo KMS directory then copy the contents of the file linked [here](https://github.com/hanzoai/kms/kms/blob/main/.env.migration.example)
2. Start the server.
...
Use the custom Hanzo KMS function `ormify` in `src/lib/knex` for simple database operations within the DAL.
