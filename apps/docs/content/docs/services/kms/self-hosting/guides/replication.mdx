...
title: "Replication"
description: "Learn how Hanzo KMS supports multi-region replication"

---

<Info>
    Hanzo KMS replication is a paid feature.

</Info>

Multi-region replication is available in Hanzo KMS Enterprise to support globally distributed deployments. Understanding the architecture, use cases, and operational considerations is essential before implementing this feature in production environments.

Hanzo KMS uses a primary/secondary (1:N) architecture with asynchronous PostgreSQL replication. This design prioritizes high availability and minimal read latency for applications deployed across multiple geographic regions.
...
- **Zero Client Changes**: Existing Hanzo KMS integrations, SDKs, and CLI tools work without modification. Regional instances route write operations to the primary while handling reads locally. Authentication tokens and API keys function identically across all instances.
...
Hanzo KMS distinguishes between _primary_ and _secondary_ instances. The primary holds write authority and is the sole instance permitted to modify the PostgreSQL database. Secondary instances handle read traffic locally and proxy write operations to the primary.

## Infrastructure components

Two data stores form Hanzo KMS's persistence layer:
...
The Hanzo KMS application servers are stateless and therefore hold no persistent data internally. This design simplifies regional deployment and horizontal scaling.
...
Understanding the implementation details can help evaluate whether Hanzo KMS's replication characteristics align with your requirements. 
...
Hanzo KMS relies on PostgreSQL's native replication, which provides:
...
- **Promotion capability**: Read replicas can be promoted to primary during disaster recovery. Promotion requires updating Hanzo KMS configuration to designate the promoted instance as primary and reconfiguring other secondaries.
...
All Hanzo KMS instances must run identical versions (mixing versions risks database schema mismatches or incompatible API behavior). Database migrations execute only on the primary and replicate to secondaries through standard PostgreSQL mechanisms.
...
Hanzo KMS uses versioned caching rather than active invalidation:
