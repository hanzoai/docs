...
title: "Upgrade Hanzo KMS Instance"
description: "How to upgrade Hanzo KMS self-hosted instance"

---

Keeping your Hanzo KMS instance up to date is key to making sure you receive the latest performance improvements, security patches, and feature updates. 
...
If a release requires specific attention, a note will be attached to the corresponding [release](https://github.com/hanzoai/kms/kms/releases) version.

During an upgrade, two key components are updated:

- **Hanzo KMS Application:** The core application code is updated.
...
2. **Plan Your Upgrade Path:**
   - Use our [Upgrade Path Tool](https://app.kms.hanzo.ai/upgrade-path) to analyze your upgrade path between your current version and target version.
   ...
   - Visit the [Hanzo KMS releases page](https://github.com/hanzoai/kms/kms/releases) for a complete list of available versions.
...
3. **Start the Upgrade Process:**  
   - Launch the new version of Hanzo KMS. During startup, the application will automatically compare the current database schema with the updated schema in the code.
   - If any differences are detected, Hanzo KMS will apply the necessary migrations automatically.
...
After upgrading your Hanzo KMS instance, you may encounter UI-related issues such as:
...
While we recommend staying up-to-date with the latest version, there may be scenarios where you need to downgrade your Hanzo KMS instance.
...
2. **Stop Hanzo KMS Services:**
   - Gracefully shut down all Hanzo KMS instances to prevent data corruption
...
4. **Deploy Previous Version:**
   - Deploy the previous Hanzo KMS version
