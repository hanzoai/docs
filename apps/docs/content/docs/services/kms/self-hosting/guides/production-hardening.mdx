...
description: "Security hardening recommendations for production Hanzo KMS deployments"
---

This document provides specific security hardening recommendations for production Hanzo KMS deployments. These recommendations follow Hanzo KMS's security model and focus on defense in depth.
...
These security configurations apply to **all** Hanzo KMS deployments regardless of how you deploy.
...
- **Required ports**: Hanzo KMS API (8080) and HTTPS (if applicable)
...
Set proper site URL for your Hanzo KMS instance:

```bash
# Required - Must be absolute URL with protocol
SITE_URL="https://app.kms.hanzo.ai"
```
...
**Keep frequent upgrade cadence**. Regularly update to the latest Hanzo KMS version for your deployment method.

## Deployment-Specific Hardening

### Docker Deployment

These recommendations are specific to Docker deployments of Hanzo KMS.
...
**Note**: Hanzo KMS requires temporary directory access for:
...
These recommendations are specific to Kubernetes deployments of Hanzo KMS.
...
These recommendations are specific to Linux binary deployments of Hanzo KMS.

#### System User Management

**Create dedicated user account**. Run Hanzo KMS under a dedicated service account:
...
[Unit]
Description=Hanzo KMS Secret Management
...
# Allow Hanzo KMS API access
sudo ufw allow 8080/tcp

# Allow HTTPS (if terminating TLS at Hanzo KMS)
...
- **Root Key Protection**: HSM encrypts Hanzo KMS's root encryption keys using hardware-protected keys
...
Hanzo KMS can integrate with external KMS providers to encrypt project secrets, providing enterprise-grade key management:
...
- **Envelope Encryption**: Hanzo KMS uses your cloud KMS to encrypt/decrypt project workspace keys, which in turn encrypt the actual secret data
- **Compliance**: Leverage your cloud provider's compliance certifications (FedRAMP, SOC2, ISO 27001)

#### Benefits for Production Deployments

- **Separation of Concerns**: Keys managed in your cloud infrastructure, separate from Hanzo KMS
...
**Implement log monitoring**. Set up centralized logging for security analysis and audit trails. Configure your SIEM or logging platform to monitor Hanzo KMS operations.

### Security Updates

**Regular security updates**. Monitor the [Hanzo KMS repository](https://github.com/hanzoai/kms/kms) for security updates and apply them promptly.
...
Hanzo KMS is compliant with FIPS 140-3, meeting U.S. and Canadian government cryptographic standards through validated cryptographic modules.
This certification is designed for organizations that require government-approved encryption implementations.
To deploy a FIPS-compliant instance, use the [kms/kms-fips](https://hub.docker.com/r/kms/kms-fips) Docker image, available to Enterprise customers.
Our FIPS 140-3 attestation letter is available in the [Hanzo KMS Trust Center](https://trust.kms.hanzo.ai/).

#### SOC 2 Compliance

Hanzo KMS is SOC 2 compliant, demonstrating adherence to rigorous security, availability, and confidentiality standards established by the American Institute of CPAs (AICPA).
This certification validates our security controls and operational practices for organizations requiring third-party audited security assurance. Our SOC 2 report is available in the [Hanzo KMS Trust Center](https://trust.kms.hanzo.ai/).

#### HIPAA Compliance

Hanzo KMS is HIPAA compliant, meeting the security and privacy requirements of the Health Insurance Portability and Accountability Act.
...
Our HIPAA certification is available in the [Hanzo KMS Trust Center](https://trust.kms.hanzo.ai/).