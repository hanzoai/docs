---
title: "Linux (HA)"
description: "Hanzo KMS High Availability Deployment architecture for Linux"
---
...
<Tabs>
  <Tab title="Debian/Ubuntu">
    ...
    Then install Hanzo KMS:
    ```bash
    sudo apt-get update && sudo apt-get install -y kms-core
    ```

    <Info>
      For production environments, we strongly recommend installing a specific version of the package to maintain consistency across reinstalls. [View available versions](https://github.com/hanzoai/kms/kms/releases).
    </Info>
  </Tab>
...
2. Monitor Hanzo KMS logs: `sudo kms-ctl tail`
