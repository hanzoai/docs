---
title: "Kubernetes (HA)"
description: "Reference architecture for self-hosting Hanzo KMS on Kubernetes (HA)"
---
Deploying Hanzo KMS on-premise with high availability requires expertise in networking, container orchestration, and database management. 
...
flowchart TB
    ...
    subgraph OS["Object Storage"]
        direction LR
        store["Hanzo S3 / any S3-compatible storage"]
...
## Core Components
### Kubernetes Cluster
Hanzo KMS is deployed on a Kubernetes cluster, which allows for container management, auto-scaling, and self-healing capabilities. 
...
This is the entry point where all other services will interact with Hanzo KMS.
...
- [Hanzo S3](/docs/services/s3), deployed alongside the cluster
- An existing enterprise object storage solution that speaks the S3 API
- An in-cluster Hanzo S3 deployment if neither option above is available
...
<Note>
Hanzo KMS does not support Redis cluster mode, and since this is an active-passive setup, we use a simple Redis deployment with backup/restore for failover.
</Note>

#### PostgreSQL Backup and Restore
PostgreSQL is the single source of truth for nearly all application data on Hanzo KMS.
...
Hanzo KMS can be deployed across multiple data centers in an active-passive configuration for disaster recovery. In this setup, one data center serves as the active site while others remain as passive standbys.
...
- The active Hanzo KMS deployment serving all user traffic
...
- A standby Hanzo KMS deployment (not receiving traffic)
...
If you operate the object store yourself, ensure:
- A high-availability deployment, not a single node
- Proper access controls and encryption for data at rest
- Regular monitoring of storage capacity and performance
- A backup of the object storage data itself