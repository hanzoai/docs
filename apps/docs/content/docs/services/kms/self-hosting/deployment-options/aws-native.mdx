...
description: "Deploy Hanzo KMS on AWS using ECS Fargate, RDS, ElastiCache, and ALB."
---
...
- An Hanzo KMS Docker image tag from [Docker Hub](https://hub.docker.com/r/kms/kms/tags)
...
<Step title="Securely store Hanzo KMS secrets and configuration">
...
<Step title="Set up IAM roles for ECS">
  ...
  **Task Execution Role** (for ECS agent operations):
  ...
  aws iam create-role \
    --role-name HanzoKMSECSExecutionRole \
  ...
  **Task Role** (for application access to AWS services):
  ...
  # Create the task role
  aws iam create-role \
    --role-name HanzoKMSTaskRole \
    --assume-role-policy-document file://ecs-task-execution-trust-policy.json

  # Create and attach the custom policy
  aws iam put-role-policy \
    --role-name HanzoKMSTaskRole \
    --policy-name HanzoKMSSecretsAccess \
  ...
  **Verify:** Confirm roles are created:
  ```bash
  aws iam get-role --role-name HanzoKMSECSExecutionRole
  aws iam get-role --role-name HanzoKMSTaskRole
  ```
</Step>
...
<Step title="Create the ECS cluster and task definition">
  ...
  **Create Task Definition:**
  ...
  ```json
  {
    ...
    "cpu": "1024",
    "memory": "2048",
    "executionRoleArn": "arn:aws:iam::<account-id>:role/HanzoKMSECSExecutionRole",
    "taskRoleArn": "arn:aws:iam::<account-id>:role/HanzoKMSTaskRole",
    ...
    Resources:
      ...
      TaskDefinition:
        ...
        Properties:
          ...
          ContainerDefinitions:
            - Name: kms
              Image: !Sub kms/kms:${HanzoKMSVersion}
              Essential: true
              PortMappings:
                - ContainerPort: 8080
                  Protocol: tcp
...
<AccordionGroup>
  ...
  <Accordion title="Cannot access Hanzo KMS in browser">
  ...
  <Accordion title="ECS Exec not working">
    **Verify prerequisites:**
    ...
    aws iam get-role-policy --role-name HanzoKMSTaskRole --policy-name HanzoKMSSecretsAccess
    ```
