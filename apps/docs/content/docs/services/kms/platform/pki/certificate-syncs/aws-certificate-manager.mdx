...
<Note>
  ...
  for Hanzo KMS to sync certificates to AWS Certificate Manager: `acm:ListCertificates`, `acm:DescribeCertificate`, `acm:ImportCertificate`, `acm:DeleteCertificate`, and `acm:ListTagsForCertificate`.
...
- **Tagging**: Automatically tag certificates with a HanzoKMSCertificate tag for easy identification and management.
...
To manually sync certificates, use the [Sync Certificates](/api-reference/endpoints/pki/syncs/aws-certificate-manager/sync-certificates) API endpoint or the manual sync option in the Hanzo KMS UI.

## FAQ

<Accordion title="Can I import certificates from AWS Certificate Manager back into Hanzo KMS?">
