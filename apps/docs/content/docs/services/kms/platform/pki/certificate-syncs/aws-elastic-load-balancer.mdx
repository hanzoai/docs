...
<Tabs>
    <Tab title="Hanzo KMS UI">
        ...
        - **Certificate Name Schema** (Optional): Customize how certificate tags are generated in ACM. Must include `{{certificateId}}` as a placeholder for the certificate ID to ensure proper certificate identification and management. If not specified, defaults to `Hanzo KMS-{{certificateId}}`.
...
- **Tagging**: Automatically tag certificates in ACM with a HanzoKMSCertificate tag for easy identification and management.
...
To manually sync certificates, use the [Sync Certificates](/api-reference/endpoints/pki/syncs/aws-elastic-load-balancer/sync-certificates) API endpoint or the manual sync option in the Hanzo KMS UI.

<Note>
  AWS Elastic Load Balancer does not support importing certificates back into
  Hanzo KMS due to security limitations where private keys cannot be extracted
  from AWS Certificate Manager. Only certificates managed by Hanzo KMS can be
