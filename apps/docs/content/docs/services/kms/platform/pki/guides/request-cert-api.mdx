...
The [API enrollment method](/documentation/platform/pki/enrollment-methods/api) lets you programmatically request TLS and other X.509 certificates from Hanzo KMS.

This is the most flexible way to request certificates from Hanzo KMS but requires you to implement certificate request and renewal logic on your own.
...
<Steps>
  ...
  <Step title="Request a certificate">
    ...
    <Note>
        ...
        with Hanzo KMS including [server-driven certificate renewal](/documentation/platform/pki/certificates/certificates#server-driven-certificate-renewal).
    </Note>
...
For example, you can install the KMS Agent on a VM and have it request and renew certificates for you or use an [ACME client](https://letsencrypt.org/docs/client-options/) paired with Hanzo KMS's [ACME enrollment method](/documentation/platform/pki/enrollment-methods/acme).
