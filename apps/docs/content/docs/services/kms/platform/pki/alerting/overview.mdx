...
description: "Learn how to set up alerting for expiring certificates with Hanzo KMS"
---

## Concept

In order to ensure that your certificates are always up-to-date and not expired, you can set up alerting in Hanzo KMS for expiring CA and leaf certificates based on customizable filters.
...
- **Daily Alerts**: When not enabled, a single notification is sent when the threshold is first reached. Enabling this option sends notifications every day until the certificate expires or is renewed.

<Warning>
  For certificates that cannot be renewed through Hanzo KMS, daily alerts will continue to be sent each day until the original expiration date.
</Warning>
