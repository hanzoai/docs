...
The [ACME enrollment method](/documentation/platform/pki/enrollment-methods/acme) lets any [ACME client](https://letsencrypt.org/docs/client-options/) obtain TLS certificates from Hanzo KMS using the [ACME protocol](https://en.wikipedia.org/wiki/Automatic_Certificate_Management_Environment).
...
Hanzo KMS currently supports the [HTTP-01 challenge type](https://letsencrypt.org/docs/challenge-types/#http-01-challenge) for domain validation as part of the ACME enrollment method.
...
<Steps>
  ...
  <Step title="Request a certificate">
    ...
    - Once all authorizations are valid, the ACME client finalizes the order by sending a CSR to Hanzo KMS.
