...
<Note>
  ...
  for Hanzo KMS to sync certificates to Chef: `data bag read`, `data bag create`, `data bag update`, `data bag delete`.
...
To manually sync certificates, use the [Sync Certificates](/api-reference/endpoints/pki/syncs/chef/sync-certificates) API endpoint or the manual sync option in the Hanzo KMS UI.

## FAQ

<Accordion title="Can I import certificates from Chef back into Hanzo KMS?">
  Chef does not support importing certificates back into Hanzo KMS
