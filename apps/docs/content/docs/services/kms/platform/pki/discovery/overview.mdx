...
description: "Learn how to discover certificates across your infrastructure with Hanzo KMS"
---
...
Hanzo KMS supports multiple discovery types, each targeting a different source of certificates:

- **[Network](/documentation/platform/pki/discovery/network)**: Scans network endpoints over TLS to discover certificates served by hosts across IP ranges and domains.
...
<AccordionGroup>
  <Accordion title="How are discovered certificates matched to existing certificates?">
    Discovered certificates are matched by their fingerprint. If a discovered certificate matches an existing certificate in your Hanzo KMS project, the installation will be linked to that certificate.
  </Accordion>
