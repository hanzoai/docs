...
Network discovery scans network endpoints over TLS to discover certificates served by hosts across IP ranges and domains. Optionally, you can use an [KMS Gateway](/documentation/platform/gateways/overview) to reach endpoints in private networks that are not accessible from the internet.
...
<Tabs>
  ...
  <Tab title="API">
    ...
    ```bash Request
    curl --request POST \
      --url https://kms.hanzo.ai/api/v1/pki/discovery \
    ...
    ```bash Request
    curl --request POST \
      --url https://kms.hanzo.ai/api/v1/pki/discovery/<discoveryId>/scan \
      --header 'Authorization: Bearer <access-token>'
    ```
  </Tab>
</Tabs>
