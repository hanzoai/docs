...
1. Configuring a Hanzo KMS root CA with details like name, validity period, and path length — This step is optional if you wish to use an external root CA with Hanzo KMS only serving the intermediate CAs.
...
<Note>
  If renewing an intermediate CA chained to a Hanzo KMS CA, then Hanzo KMS will
