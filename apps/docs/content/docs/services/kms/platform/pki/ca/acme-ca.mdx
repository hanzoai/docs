...
The ACME provider checks for the existence of this TXT record to verify domain control before issuing the certificate back to Hanzo KMS.
...
2. Creating an [App Connection](/integrations/app-connections/overview) in Hanzo KMS to store credentials for Hanzo KMS to connect to your DNS provider and create/remove DNS records as part of the DNS-01 challenge.
3. Registering an [External CA](/documentation/platform/pki/ca/external-ca) in Hanzo KMS with the ACME type and inputting required configuration including the **ACME Directory URL** of the upstream ACME-compatible CA and the **App Connection** for your DNS provider.
...
<Steps>
    <Step title="Create an App Connection to your DNS provider">
        Before registering an ACME-compatible CA with Hanzo KMS, you need to set up an [App Connection](/integrations/app-connections/overview) with the appropriate permissions for Hanzo KMS to perform the DNS-01 challenge with your DNS provider.
    ...
    <Step title="Register an ACME-compatible CA">
        <Tabs>
          <Tab title="Hanzo KMS UI">
              ...
              Great! You’ve successfully registered an external ACME-compatible CA with Hanzo KMS. Now check out the [Certificates](/documentation/platform/pki/certificates/overview) section to learn more about how to issue X.509 certificates using the ACME-compatible CA.
          </Tab>
          <Tab title="API">
            To register an ACME CA with Hanzo KMS using the API, make a request to the [Create External CA](https://kms.hanzo.ai/docs/api-reference/endpoints/certificate-authorities/acme/create) endpoint:
...
1. **Create a CNAME record** in your domain's DNS that points `_acme-challenge.{your-domain}` to a target in the zone configured with Hanzo KMS. For subdomains, create additional CNAMEs as needed (e.g., `_acme-challenge.app.example.com` CNAME to `_acme-challenge.ssl.example.com`).

2. **Register the ACME CA** in Hanzo KMS with the **hosted zone ID** of the target zone (e.g., `ssl.example.com` if your CNAME points to `_acme-challenge.ssl.example.com`).
...
1. **Create a dedicated Route53 hosted zone** for `acme.example.com`, Hanzo KMS needs write permissions here.
2. **Add a CNAME record in Cloudflare** pointing `_acme-challenge.example.com` to `_acme-challenge.acme.example.com`.
3. **Create an AWS App Connection** in Hanzo KMS with Route53 permissions for the `acme.example.com` zone.
4. **Register your ACME CA in Hanzo KMS** using the Route53 hosted zone ID for `acme.example.com`.
...
<AccordionGroup>
  ...
  <Accordion title="Does Hanzo KMS support DNS CNAME delegation for ACME challenges?">
    Yes. You can delegate `_acme-challenge` records to a different DNS zone by setting up a CNAME, then configuring Hanzo KMS to write TXT records in the target zone. See the [DNS CNAME Delegation](#dns-cname-delegation) section above for setup instructions.
  </Accordion>
</AccordionGroup>
