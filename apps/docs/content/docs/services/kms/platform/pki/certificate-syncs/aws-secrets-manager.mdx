...
<Note>
  ...
  for Hanzo KMS to sync certificates to AWS Secrets Manager: `secretsmanager:CreateSecret`, `secretsmanager:UpdateSecret`,
...
<Tabs>
    <Tab title="Hanzo KMS UI">
        ...
        ![Configure Options](/images/platform/pki/certificate-syncs/aws-secrets-manager/aws-secrets-manager-options.png)
            ...
            - **Preserve Secret on Renewal**: Only applies to certificate renewals. When a certificate is renewed in Hanzo KMS, this option controls how the renewed certificate is handled. If enabled, the renewed certificate will update the existing secret, preserving the same secret name. If disabled, the renewed certificate will be created as a new secret with a new name.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://app.kms.hanzo.ai/api/v1/cert-manager/syncs/aws-secrets-manager \
...
- **Automatic Deployment**: Deploy certificates in Hanzo KMS to AWS Secrets Manager as JSON secrets with customizable field names.
...
To manually sync certificates, use the [Sync Certificates](/api-reference/endpoints/pki/syncs/aws-secrets-manager/sync-certificates) API endpoint or the manual sync option in the Hanzo KMS UI.
...
<Accordion title="Can I import certificates from AWS Secrets Manager back into Hanzo KMS?">
  AWS Secrets Manager does not support importing certificates back into Hanzo KMS
