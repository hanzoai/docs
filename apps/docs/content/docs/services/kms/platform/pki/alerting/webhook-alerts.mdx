...
Hanzo KMS can send PKI certificate alert notifications to a webhook URL. This guide walks through creating a webhook alert, the payload format, and how to verify webhook signatures.
...
```json
{
  ...
  "data": {
    ...
    "metadata": {
      "totalCertificates": 1,
      "viewUrl": "https://app.kms.hanzo.ai/cert-manager/<project-id>/policies"
    }
  }
}
```
...
If you configure a signing secret for your webhook channel, Hanzo KMS will include an `x-kms-signature` header with each request. This allows you to verify that the webhook payload originated from Hanzo KMS.
