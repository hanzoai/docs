...
<Tabs>
    <Tab title="Hanzo KMS UI">
        ...
        - **Certificate Name Schema** (Optional): Customize how certificate names are generated. Must include `{{certificateId}}` as a placeholder. If not specified, defaults to `Hanzo KMS-{{certificateId}}`.
...
To manually sync certificates, use the [Sync Certificates](/api-reference/endpoints/pki/syncs/cloudflare-custom-certificate/sync-certificates) API endpoint or the manual sync option in the Hanzo KMS UI.

## FAQ

<Accordion title="Can I import certificates from Cloudflare back into Hanzo KMS?">
  Cloudflare does not support importing certificates back into Hanzo KMS
