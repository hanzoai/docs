...
The API enrollment method allows you to issue certificates against a specific [certificate profile](/documentation/platform/pki/certificates/profiles) over Web UI or by making an API request to Hanzo KMS.
...
<Steps>
  <Step title="Create a certificate profile in Hanzo KMS">
    ...
    ```bash Request
    curl --location --request POST 'https://app.kms.hanzo.ai/api/v1/cert-manager/certificate-profiles' \
