...
<Note>
  **Rotation Type: [Single-Phase](/documentation/platform/secret-rotation/overview#single-phase-rotation)**
  ...
  This is a limitation of the rotation mechanism and cannot be rectified by Hanzo KMS.
</Note>
...
- Ensure your network security policies allow SMB connections from Hanzo KMS to this rotation provider
...
## Create a Windows Local Account Rotation in Hanzo KMS
...
<Tabs>
    <Tab title="Hanzo KMS UI">
        ...
        - **Rotation Method** - The method to use when rotating the target user's password.
            - **Login as Target** - Hanzo KMS will use the target user's current credentials to authenticate and rotate its own password. See the [Login as Target Requirements](#login-as-target-requirements) section below for important configuration details.
            - **Login as Root** - Hanzo KMS will use the administrator credentials provided in the SMB Connection to rotate the target user's password. This is the recommended method for most use cases.
        ...
        If you suspect the credentials are out of sync (for example, after a manual password change on the server), you can regain access by using **Reconcile**. This will use the configured Windows App Connection's administrator account to reset the target user's password and sync it with Hanzo KMS.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://kms.hanzo.ai/api/v2/secret-rotations/windows-local-account \
