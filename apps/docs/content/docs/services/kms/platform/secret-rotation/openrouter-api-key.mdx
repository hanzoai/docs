...
## Create an OpenRouter API Key Rotation in Hanzo KMS
...
<Tabs>
    <Tab title="Hanzo KMS UI">
        ...
        - **API Key** – The name of the secret in Hanzo KMS where the rotated API key value will be stored.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
          --url https://kms.hanzo.ai/api/v2/secret-rotations/open-router-api-key \
