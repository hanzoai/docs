...
2. Create two designated database users for Hanzo KMS to rotate the credentials for. Be sure to grant each user login permissions for the desired database with the necessary privileges their use case will require.
...
3. Ensure your network security policies allow incoming requests from Hanzo KMS to this rotation provider, if network restrictions apply.

## Create a Microsoft SQL Server Credentials Rotation in Hanzo KMS
...
<Tabs>
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl --request POST \
        --url https://kms.hanzo.ai/api/v2/secret-rotations/mssql-credentials \
