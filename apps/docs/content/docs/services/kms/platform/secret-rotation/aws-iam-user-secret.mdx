...
Hanzo KMS's AWS IAM User secret rotation capability lets you update the **Access key** and **Secret access key** credentials of a target IAM user from within Hanzo KMS
...
2. Configuring the rotation strategy in Hanzo KMS with the credentials of the managing IAM user.
3. Pressing the **Rotate** button in the Hanzo KMS dashboard to trigger the rotation of the target IAM user's credentials. The strategy can also be configured to rotate the credentials automatically at a specified interval.

In the following steps, we explore the end-to-end workflow for setting up this strategy in Hanzo KMS.
...
<Steps>
	...
	<Step title="Configure the AWS IAM User secret rotation strategy in Hanzo KMS">
		<Tabs>
			...
			<Tab title="API">
				...
				```bash Request
				curl --request POST \
					--url https://kms.hanzo.ai/api/v2/secret-rotations/aws-iam-user-secret \
