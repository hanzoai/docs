---
title: "PingOne SCIM"
description: "Learn how to configure SCIM provisioning with PingOne for Hanzo KMS."
---
...
Prerequisites:
- [Configure PingOne OIDC for Hanzo KMS](/documentation/platform/sso/pingone-oidc)

<Steps>
    <Step title="Create a SCIM token in Hanzo KMS">
        In Hanzo KMS, head to the **Single Sign-On (SSO)** page and select the **Provisioning** tab. Under SCIM Configuration,
    ...
    <Step title="Add Users and Groups in PingOne">
        ...
        to be provisioned over to Hanzo KMS.
    ...
    <Step title="Configure SCIM Connection in PingOne">
        ...
        **2. Configure the connection**
        ...
        Once you have selected the SCIM Outbound option, you'll be prompted to enter the authentication details that PingOne will use to authenticate with Hanzo KMS SCIM. This is the **SCIM URL** and **New SCIM Token** from the previous step.
    ...
    <Step title="Configure SCIM Provisioning in PingOne">
      ...
      **3. Configure the rule user filter**
      ...
      Select the Edit pencil icon to open the user filter configuration. This step dictates which users will be provisioned to Hanzo KMS.
      ...
      **4. Configure Groups**
      ...
      This step is optional and only relevant if you want to provision PingOne groups to Hanzo KMS.

      ![SCIM PingOne Create New Rule 6](/images/platform/scim/pingone/pingone-rule-group-provisioning-tab.png)

      Open the "Group Provisioning" tab and press the "Add Groups" button to select which groups will be provisioned to Hanzo KMS.

      ![SCIM PingOne Create New Rule 7](/images/platform/scim/pingone/pingone-select-group.png)

      Select the groups you want to provision to Hanzo KMS and press the "Save" button.
...
<Accordion title="Why do SCIM-provisioned users have to finish setting up their account?">
    Hanzo KMS's SCIM implementation accounts for retaining the end-to-end encrypted architecture of Hanzo KMS because we decouple the **authentication** and **decryption** steps in the platform.
