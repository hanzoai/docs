...
Monitor and detect exposed secrets across your data sources, including code repositories, with Hanzo KMS Secret Scanning.
...
<Note>
  Secret Scanning is a paid feature. If you're using Hanzo KMS Cloud, then it is
  to use it.
</Note>
...
All scan activities can be monitored in real-time through the Hanzo KMS UI, which displays:
...
If you're intentionally committing a test secret that the secret scanner might flag, you can instruct Hanzo KMS to overlook that secret with the methods listed below.
...
You can then add the fingerprints of the findings you wish to exclude. The [Hanzo KMS scan](/cli/scanning-overview) report provides a unique Fingerprint for each secret found.
