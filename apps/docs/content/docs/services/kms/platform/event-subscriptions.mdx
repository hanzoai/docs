...
description: "Subscribe to events in Hanzo KMS for real-time updates"
---

<Note>
   Event Subscriptions is a paid feature that is available under the Enterprise license.
   Please contact [sales@hanzo.ai](mailto:sales@hanzo.ai).
</Note>

Event Subscriptions allow you to receive real-time notifications when specific actions occur within your Hanzo KMS projects. You can subscribe to changes such as secret modifications, with support for additional resource types coming soon.
...
1. Your application opens an SSE connection to the Hanzo KMS Events API.
2. When a subscribed event occurs (e.g., a secret is updated), Hanzo KMS pushes a notification through the connection.
...
The endpoint responds with `Content-Type: text/event-stream` to initiate an SSE connection. In the cURL example below, we use the `-N` flag to keep the connection open to receive incoming events from Hanzo KMS.

```bash
curl -X POST -N --location \
  'https://app.kms.hanzo.ai/api/v1/events/subscribe/project-events' \
