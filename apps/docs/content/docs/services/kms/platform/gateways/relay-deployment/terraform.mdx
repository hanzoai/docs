---
title: "Terraform"
description: "How to deploy Hanzo KMS Relay Servers using Terraform"
---

This guide walks you through deploying a Hanzo KMS Relay server using Terraform. Select a provider below for specific instructions.
...
- Credentials for the Hanzo KMS Relay to authenticate with your Hanzo KMS instance. This guide uses a Machine Identity token, but other methods are available. You can find a full list of authentication options [here](/cli/commands/relay#available-authentication-methods).
...
2.  **Customize values:** Update the placeholder values in `main.tf` to match your AWS environment and Hanzo KMS credentials. You'll need to replace:
