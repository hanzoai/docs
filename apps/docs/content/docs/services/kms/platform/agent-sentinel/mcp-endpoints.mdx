...
MCP Endpoints are the entry points that AI clients (like Claude, ChatGPT, or custom agents) use to access your configured MCP Servers. Instead of connecting AI clients directly to individual MCP servers, you connect them to a Hanzo KMS MCP Endpoint which acts as a secure gateway.
...
When you create an MCP endpoint, Hanzo KMS generates a unique URL that you can add to your AI client's MCP configuration. The AI client connects to this URL and can access all enabled tools from the connected MCP servers.
...
<Tabs>
  <Tab title="Claude">
    ...
    3. Add a new server with your Hanzo KMS endpoint URL
    4. Click **Connect**
  ...
  <Tab title="Other AI Clients">
    ...
    2. Add your Hanzo KMS endpoint URL as a new server
