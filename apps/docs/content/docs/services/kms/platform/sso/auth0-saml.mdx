---
title: "Auth0 SAML"
description: "Learn how to configure Auth0 SAML for Hanzo KMS SSO."
---

<Info>
  Auth0 SAML SSO feature is a paid feature. If you're using Hanzo KMS Cloud,
  to use it.
</Info>

<Steps>
   <Step title="Prepare the SAML SSO configuration in Hanzo KMS">
       In Hanzo KMS, head to the **Single Sign-On (SSO)** page and select the **General** tab. Click **Connect** for **SAML** under the Connect to an Identity Provider section. Select **Auth0**, then click **Connect** again.
   ...
   <Step title="Create a SAML application in Auth0">
        ...
        2.4. Back in Hanzo KMS, set **Issuer**, **Identity Provider Login URL**, and **Certificate** to the corresponding items from step 2.3.

        ![Auth0 SAML Hanzo KMS config](../../../images/sso/auth0-saml/kms-config.png)
    ...
    <Step title="Enable SAML SSO in Hanzo KMS">
        Enabling SAML SSO allows members in your organization to log into Hanzo KMS via Auth0.
    ...
    <Step title="Enforce SAML SSO in Hanzo KMS">
        ...
        To enforce SAML SSO, you're required to test out the SAML connection by successfully authenticating at least one Auth0 user with Hanzo KMS;
