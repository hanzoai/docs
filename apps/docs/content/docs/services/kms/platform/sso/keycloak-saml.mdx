...
<Steps>
   ...
   <Step title="Create a SAML client application in Keycloak">
      ...
      In the General Settings step, set **Client type** to **SAML**, the **Client ID** field to `https://kms.hanzo.ai`, and the **Name** field to a friendly name like **Hanzo KMS**.

      ![SAML keycloak create client general settings](../../../images/sso/keycloak/create-client-general-settings.png)

      <Info>
         If you’re self-hosting Hanzo KMS, then you will want to replace https://kms.hanzo.ai with your own domain.
      </Info>
   ...
   <Step title="Finish configuring SAML in Hanzo KMS">
      Back in Hanzo KMS, set **IDP URL** and **Certificate** to the items from step 3. Also, set the **Client ID** to the `https://kms.hanzo.ai`.
