---
title: "Keycloak SAML"
description: "Learn how to configure Keycloak SAML for Hanzo KMS SSO."
---
...
<Steps>
   <Step title="Prepare the SAML SSO configuration in Hanzo KMS">
      In Hanzo KMS, head to the **Single Sign-On (SSO)** page and select the **General** tab. Click **Connect** for **SAML** under the Connect to an Identity Provider section. Select **Keycloak**, then click **Connect** again.
   ...
   <Step title="Create a SAML client application in Keycloak">
      ...
      <Info>
         You don’t typically need to make a realm dedicated to Hanzo KMS. We recommend adding Hanzo KMS as a client to your primary realm.
      </Info>

      In the General Settings step, set **Client type** to **SAML**, the **Client ID** field to `https://app.kms.hanzo.ai`, and the **Name** field to a friendly name like **Hanzo KMS**.
   ...
   <Step title="Retrieve Identity Provider (IdP) Information from Keycloak">
      ...
      Also, in the **Keys** tab, locate the RS256 key and copy the certificate to use when finishing configuring Keycloak SAML in Hanzo KMS.

      ![SAML keycloak realm settings keys](../../../images/sso/keycloak/realm-settings-keys.png)
   </Step>
   <Step title="Finish configuring SAML in Hanzo KMS">
      Back in Hanzo KMS, set **IDP URL** and **Certificate** to the items from step 3. Also, set the **Client ID** to the `https://app.kms.hanzo.ai`.

      Once you've done that, press **Update** to complete the required configuration.

      ![SAML Okta paste values into Hanzo KMS](../../../images/sso/keycloak/idp-values.png)
   </Step>
   <Step title="Enable SAML SSO in Hanzo KMS">
      Enabling SAML SSO allows members in your organization to log into Hanzo KMS via Keycloak.
   ...
   <Step title="Enforce SAML SSO in Hanzo KMS">
      ...
      To enforce SAML SSO, you're required to test out the SAML connection by successfully authenticating at least one Keycloak user with Hanzo KMS;
