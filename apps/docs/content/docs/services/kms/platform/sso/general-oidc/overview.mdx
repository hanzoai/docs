...
description: "Learn how to configure OIDC for Hanzo KMS SSO with any OIDC-compliant identity provider"
---

<Info>
  OIDC SSO is a paid feature. If you're using Hanzo KMS Cloud, then it is
  it.
</Info>

You can configure your organization in Hanzo KMS to have members authenticate with the platform through identity providers via [OpenID Connect](https://openid.net/specs/openid-connect-core-1_0.html).
...
<Steps>
   <Step title="Setup Identity Provider">
      ...
      1.2. Configure **Redirect URL** to be `https://kms.hanzo.ai/api/v1/sso/oidc/callback`. If you're self-hosting Hanzo KMS, replace the domain with your own.
   ...
   <Step title="Finish configuring OIDC in Hanzo KMS">
      2.1. Back in Hanzo KMS, head to the **Single Sign-On (SSO)** page and select the **General** tab. Select **Connect** for **OIDC**.
    ...
    <Step title="Enforce OIDC SSO in Hanzo KMS">
      ...
      To enforce OIDC SSO, you're required to test out the OpenID connection by successfully authenticating at least one IdP user with Hanzo KMS.
