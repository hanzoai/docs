---
title: "Migrating from Vault to Hanzo KMS"
sidebarTitle: "Vault"
...
- Kubernetes auth method configurations (become [Machine Identities](/documentation/platform/identities/machine-identities) in Hanzo KMS)
- Kubernetes secrets engine roles (become [Dynamic Secrets](/documentation/platform/dynamic-secrets/kubernetes) in Hanzo KMS)
- Vault policies (translated to [Hanzo KMS project roles](/documentation/platform/access-controls/role-based-access-controls))
...
<Info>
**First time?** You'll need to [create a project](/documentation/platform/project) first. A project in Hanzo KMS typically represents a single application or microservice.
</Info>
...
The secrets will be imported into your current environment and folder path. If your Vault path has nested paths, you can create [folders](/documentation/platform/folder) in Hanzo KMS to maintain a similar structure.

### Migrating Kubernetes Auth Method

If you're using Vault's Kubernetes auth method to authenticate your pods, you can migrate this configuration to Hanzo KMS's [Machine Identities](/documentation/platform/identities/machine-identities). In Vault, you define Kubernetes roles that specify which service accounts can authenticate. In Hanzo KMS, this maps to a Machine Identity with Kubernetes authentication enabled.

**To import Kubernetes auth configuration:**

1. First, [create a Machine Identity](/documentation/platform/identities/machine-identities) in Hanzo KMS
...
**After importing:** You'll need to update your Kubernetes deployments to use the [Hanzo KMS Kubernetes Operator](/integrations/platforms/kubernetes/overview) or [Hanzo KMS SDK](/sdks/overview) instead of the Vault agent/sidecar.

### Migrating Kubernetes Secrets Engine Roles

If you're using Vault's Kubernetes secrets engine to generate short-lived Kubernetes credentials, you can migrate these roles to Hanzo KMS's [Dynamic Secrets](/documentation/platform/dynamic-secrets/kubernetes). Both platforms support generating temporary Kubernetes service account tokens or creating temporary service accounts, so the configuration is similar.
...
Learn more about how Dynamic Secrets work in Hanzo KMS in the [Dynamic Secrets documentation](/documentation/platform/dynamic-secrets/overview).

### Migrating Vault Policies

Hanzo KMS can translate your Vault HCL policies into [Hanzo KMS project roles](/documentation/platform/access-controls/role-based-access-controls). The table below shows how Vault capabilities map to Hanzo KMS permissions:
...
<Note>
  ...
  adjusted as needed since Vault and Hanzo KMS have different access control
  models.
</Note>
...
5. Review the automatically translated Hanzo KMS permissions
...
<Tip>
  **How policy translation works:**
  ...
  - Vault capabilities (`read`, `list`, `create`, etc.) are mapped to Hanzo KMS permissions
  ...
  **Example translation:**
  ...
  Becomes a Hanzo KMS role with **read** permission on secrets in the corresponding environment/folder path.
</Tip>
