...
Approvers are notified by email and/or Slack as soon as the request is initiated. In the Hanzo KMS Dashboard, they will be able to `approve` and `merge` (or `deny`) a request for a change in a particular environment. After that, depending on the workflows setup, the change will be automatically propagated to the right applications (e.g., using [Hanzo KMS Kubernetes Operator](https://kms.hanzo.ai/docs/integrations/platforms/kubernetes)).
