...
1. **Resource Role**: An IAM role in your AWS account that trusts Hanzo KMS. This is the "bridge" role that Hanzo KMS assumes first.
...
3. **Role Chaining**: Hanzo KMS uses AWS role chaining - it first assumes the Resource Role, then uses those credentials to assume the Target Role. This provides an additional layer of security and audit capability.

4. **External ID**: A unique identifier (your Hanzo KMS Project ID) used in the trust policy to prevent [confused deputy attacks](https://docs.aws.amazon.com/IAM/latest/UserGuide/confused-deputy.html).
...
1. **Resource Role** - Trusted by Hanzo KMS, can assume target roles
...
<Steps>
  ...
  <Step title="Create the Resource Role with Trust Policy">
    Create an IAM role (e.g., `HanzoKMSResourceRole`) with:
...
<Steps>
  <Step title="Create the Target Role Trust Policy">
    ...
    ```json
    {
      ...
      "Statement": [{
        "Effect": "Allow",
        "Principal": {
          "AWS": "arn:aws:iam::<YOUR_ACCOUNT_ID>:role/HanzoKMSResourceRole"
        },
        "Action": "sts:AssumeRole",
        "Condition": {
          "StringEquals": {
  ...
  <Step title="Create the Account in Hanzo KMS">
