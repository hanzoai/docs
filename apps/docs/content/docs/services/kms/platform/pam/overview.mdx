...
Hanzo KMS Privileged Access Management (PAM) provides a centralized way to manage and secure access to your critical infrastructure. It allows you to enforce fine-grained, policy-based controls over resources like databases, servers, and more, ensuring that only authorized users can access sensitive systems, and only when they need to.

## The PAM Workflow

At its core, KMS PAM is designed to decouple **user identity** from **infrastructure credentials**. Instead of sharing static passwords or SSH keys, users authenticate with their SSO identity, and Hanzo KMS handles the rest.

Here is how a typical access lifecycle looks:

1.  **Discovery**: A user logs into Hanzo KMS and sees a catalog of resources (databases, servers) and the accounts under each resource they are allowed to access. Access is **resource-centric**: users navigate to a resource first, then select an account within that resource.
...
3.  **Credential Injection**: Hanzo KMS validates the request. If allowed, it establishes a secure tunnel and automatically injects the credentials for the target account. **The user never sees the underlying password or key.**
...
2.  **Resource**: Configured within Hanzo KMS. It points to a specific IP/Host accessible by the Gateway.
...
Ready to set up KMS PAM? Follow our step-by-step guide to deploy a gateway, define resources, and grant your team secure access to critical infrastructure.
...
KMS PAM supports a variety of resource types:
