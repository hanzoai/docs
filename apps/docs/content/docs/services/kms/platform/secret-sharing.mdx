...
Hanzo KMS offers a secure solution for sharing secrets over the internet in a time and view-count bound manner. It is possible to share secrets without signing up via [kms.hanzo.ai](https://kms.hanzo.ai) or via Hanzo KMS Dashboard (which has more advanced functionality).
