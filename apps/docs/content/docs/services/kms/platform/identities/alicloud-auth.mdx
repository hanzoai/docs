...
description: "Learn how to authenticate with Hanzo KMS using Alibaba Cloud user accounts."
---
...
At a high level, Hanzo KMS authenticates an Alibaba Cloud user by verifying its identity and checking that it meets specific requirements (e.g., its ARN is whitelisted) at the `/api/v1/auth/alicloud-auth/login` endpoint. If successful,
