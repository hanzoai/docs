...
description: "Learn how to authenticate to Hanzo KMS from any platform or environment."
---
...
The following sequence diagram illustrates the Universal Auth workflow for authenticating clients with Hanzo KMS.
...
sequenceDiagram
  ...
  Note over Client,Infis: Step 3: Access Hanzo KMS API with Token
  Client->>Infis: Make authenticated requests using the short-lived access token
```

## Concept

In this method, Hanzo KMS authenticates a client by verifying the credentials issued for it at the `/api/v1/auth/universal-auth/login` endpoint. If successful,
then Hanzo KMS returns a short-lived access token that can be used to make authenticated requests to the Hanzo KMS API.

To be more specific:

1. The client submits a **Client ID** and **Client Secret** to Hanzo KMS at the `/api/v1/auth/universal-auth/login` endpoint.
2. Hanzo KMS verifies the credential pair.
3. If all is well, Hanzo KMS returns a short-lived access token that the client can use to make authenticated requests to the Hanzo KMS API.

## Guide

In the following steps, we explore how to create and use identities for your workloads and applications to access the Hanzo KMS API
...
<Steps>
  ...
  <Step title="Creating a Client Secret">
    ...
    and password used to authenticate with the Hanzo KMS API.
  ...
  <Step title="Accessing the Hanzo KMS API with the identity">
    To access the Hanzo KMS API as the identity, you should first perform a login operation
    ...
    <Tip>
      ...
      - For Hanzo KMS Cloud US users: `https://app.kms.hanzo.ai`
      - For Hanzo KMS Cloud EU users: `https://eu.kms.hanzo.ai`
    </Tip>

    #### Sample request

    ```bash Request
    curl --location --request POST 'https://app.kms.hanzo.ai/api/v1/auth/universal-auth/login' \
    ...
    Next, you can use the access token to authenticate with the [Hanzo KMS API](/api-reference/overview/introduction)
...
4. The workload authenticates with Hanzo KMS using the Client Secret and Client ID to obtain the initial access token (JWT).
5. The workload uses the access token to authenticate and continuously renews it before expiration:

```bash
curl --location --request POST 'https://app.kms.hanzo.ai/api/v1/auth/universal-auth/renew' \
  --header 'Authorization: Bearer <accessToken>'
```

This approach allows your workload to securely bootstrap and maintain access to Hanzo KMS without hard-coded secrets, solving the secret zero problem.

**FAQ**

<AccordionGroup>
  <Accordion title="Why is the Hanzo KMS API rejecting my identity credentials?">
