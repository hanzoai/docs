---
title: Azure
description: "Learn how to authenticate Azure pipelines with Hanzo KMS using OpenID Connect (OIDC)."
---
...
The following sequence diagram illustrates the OIDC Auth workflow for authenticating Azure pipelines with Hanzo KMS.
...
3. Hanzo KMS fetches the public key that was used to sign the identity token from Azure's identity provider using OIDC Discovery.
...
5. If all is well, Hanzo KMS returns a short-lived access token that the Azure pipeline can use to make authenticated requests to the Hanzo KMS API.

<Note>
  Hanzo KMS needs network-level access to Azure's identity provider endpoints.
</Note>
...
<Steps>
    ...
    In Azure DevOps, to authenticate with Hanzo KMS using OIDC, you must configure a service connection that enables workload identity federation.
    ...
    steps:
    - task: AzureCLI@2
      displayName: 'Retrieve secrets from Hanzo KMS using OIDC'
      inputs:
