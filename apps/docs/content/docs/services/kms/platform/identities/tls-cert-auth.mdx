...
description: "Learn how to authenticate with Hanzo KMS using TLS Certificate."
---

**TLS Certificate Auth** is an authentication method that verifies a user's TLS Client certificate using the provided CA Certificate, allowing secure access to Hanzo KMS resources.

## Diagram

The following sequence diagram illustrates the TLS Certificate Auth workflow for authenticating users with Hanzo KMS.
...
sequenceDiagram
  ...
  Client->>Hanzo KMS: Send request to /api/v1/auth/tls-cert-auth/login

  Note over Hanzo KMS: Step 3: Request verification using CA Certificate
...
At a high level, Hanzo KMS authenticates the client's TLS Certificate by verifying its identity and checking that it meets specific requirements (e.g., it is bound to the allowed common names) at the `/api/v1/auth/tls-cert-auth/login` endpoint. If successful, Hanzo KMS returns a short-lived access token that can be used to make authenticated requests to the Hanzo KMS API.

To be more specific:

1. The client sends a TLS request with the client certificate to Hanzo KMS at the `/api/v1/auth/tls-cert-auth/login` endpoint.
2. Hanzo KMS verifies the incoming request using the provided CA certificate.
3. Hanzo KMS checks the user's properties against set criteria such as Allowed Common Names.
4. If all checks pass, Hanzo KMS returns a short-lived access token that the client can use to make authenticated requests to the Hanzo KMS API.

<Accordion title="TLS with Load Balancer/Proxy">
  Most of the time, the Hanzo KMS server will be behind a load balancer or
...
<Note>
  Hanzo KMS US/EU and dedicated instances are deployed with AWS ALB. TLS Certificate Auth must flow through our ALB mTLS pass-through in order to authenticate.
  When you are authenticating with TLS Certificate Auth, you must use the port `8443` instead of the default `443`. Example: `https://app.kms.hanzo.ai:8443/api/v1/auth/tls-cert-auth/login`
</Note>
...
access the Hanzo KMS API using request signing.
...
To create an identity, head to your Organization Settings > Access Control > [Identities](https://app.kms.hanzo.ai/organization/access-management?selectedTab=identities) and press **Create identity**.

![identities organization](/images/platform/identities/identities-org.png)

When creating an identity, you specify an organization-level [role](/documentation/platform/access-controls/role-based-access-controls) for it to assume; you can configure roles in Organization Settings > Access Control > [Organization Roles](https://app.kms.hanzo.ai/organization/access-management?selectedTab=roles).
...
- **Role (required):** A role from the [**Organization Roles**](https://app.kms.hanzo.ai/organization/access-management?selectedTab=roles) tab for the identity to assume. The organization role assigned will determine what organization-level resources this identity can have access to.
...
To access the Hanzo KMS API as the identity, you need to send a TLS request to `/api/v1/auth/tls-cert-auth/login` endpoint.
...
try {
  ...
  const kmsUrl = "https://app.kms.hanzo.ai:8443"; // or your self-hosted Hanzo KMS URL
...
- **NEVER** allow direct connections to Hanzo KMS for TLS cert auth - this enables header injection attacks
...
<Note>
  Hanzo KMS will handle the actual certificate validation against the configured
