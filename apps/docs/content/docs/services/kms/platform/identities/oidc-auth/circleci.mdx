---
title: CircleCI
description: "Learn how to authenticate CircleCI jobs with Hanzo KMS using OpenID Connect (OIDC)."
---
...
The following sequence diagram illustrates the OIDC Auth workflow for authenticating CircleCI jobs with Hanzo KMS.
...
2. The CircleCI OIDC token is sent to Hanzo KMS at the `/api/v1/auth/oidc-auth/login` endpoint.
3. Hanzo KMS fetches the public key that was used to sign the identity token provided by CircleCI.
...
5. If all is well, Hanzo KMS returns a short-lived access token that CircleCI jobs can use to make authenticated requests to the Hanzo KMS API.

<Note>Hanzo KMS needs network-level access to the CircleCI servers.</Note>
...
<Steps>
    ...
    <Step title="Using CircleCI OIDC token to authenticate with Hanzo KMS">
        The following is an example of how to use the `$CIRCLE_OIDC_TOKEN` with the Hanzo KMS [terraform provider](https://registry.terraform.io/providers/Hanzo KMS/kms/latest/docs) to manage resources in a CircleCI pipeline.
