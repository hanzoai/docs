---
title: "Heroku Sync"
description: "Learn how to configure a Heroku Sync for Hanzo KMS."
---
...
<Tabs>
    <Tab title="Hanzo KMS UI">
        ...
        ![Configure Options](/images/secret-syncs/heroku/heroku-options.png)
            ...
            - **Initial Sync Behavior**: Determines how Hanzo KMS should resolve the initial sync.
                - **Import - Prefer values from Hanzo KMS**: Import secrets from Heroku to Hanzo KMS; if a secret with the same name already exists in Hanzo KMS, do nothing. Afterwards, sync secrets to Heroku.
                - **Import - Prefer values from Heroku**: Import secrets from Heroku to Hanzo KMS; if a secret with the same name already exists in Hanzo KMS, replace its value with the one from Heroku. Afterwards, sync secrets to Heroku.
    ...
    <Tab title="API">
        ...
        ```bash Request
        curl    --request POST \
        --url https://app.kms.hanzo.ai/api/v1/secret-syncs/heroku \
