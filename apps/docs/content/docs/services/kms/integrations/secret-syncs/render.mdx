---
title: "Render Sync"
description: "Learn how to configure a Render Sync for Hanzo KMS."
---
...
<Tabs>
  <Tab title="Hanzo KMS UI">
    ...
    ![Configure Options](/images/secret-syncs/render/render-sync-options.png)
      ...
      - **Initial Sync Behavior**: Determines how Hanzo KMS should resolve the initial sync.
        ...
        - **Import Secrets (Prioritize Hanzo KMS)**: Imports secrets from the Render service before syncing, prioritizing values from Hanzo KMS over Render when keys conflict.
        - **Import Secrets (Prioritize Render)**: Imports secrets from the Render service before syncing, prioritizing values from Render over Hanzo KMS when keys conflict.
  ...
  <Tab title="API">
    ...
    ```bash Request
    curl --request POST \
      --url https://app.kms.hanzo.ai/api/v1/secret-syncs/render \
