...
description: "Learn how to sync secrets to third-party services with Hanzo KMS."
---

Secret Syncs enable you to sync secrets from Hanzo KMS to third-party services using [App Connections](/integrations/app-connections/overview).
...
<Note>
    Hanzo KMS is continuously expanding it's Secret Sync third-party service support. If the service you need isn't available,
    you can contact us at team@kms.hanzo.ai to make a request.
</Note>

## Key Schemas

Key Schemas transform your secret keys by applying a prefix, suffix, or format pattern during sync to external destinations. This makes it clear which secrets are managed by Hanzo KMS and prevents accidental changes to unrelated secrets.

Any destination secrets which do not match the schema will not get deleted or updated by Hanzo KMS.
...
<Note>
    When importing secrets from the destination into Hanzo KMS, the schema is stripped from imported secret keys.
</Note>
