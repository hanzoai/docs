---
title: "Gradle"
description: "How to use Hanzo KMS to inject environment variables with Gradle"
---
...
This documentation provides an overview of how to use Hanzo KMS with [Gradle](https://gradle.org/).
...
With this command, Hanzo KMS will automatically inject the environment variables associated with the current Hanzo KMS project into the Gradle process. 
...
## Advantages of Using Hanzo KMS with Gradle
...
2. **Reproducibility**: Ensure consistent builds by leveraging the environment variables from the related Hanzo KMS project.
3. **Security**: Protect sensitive information by using Hanzo KMS's secrets management without exposing them in scripts or logs.
