---
title: "Terraform"
description: "Learn how to fetch secrets from Hanzo KMS with Terraform using both traditional data sources and ephemeral resources"
url: "https://registry.terraform.io/providers/Hanzo KMS/kms/latest/docs"
---
{/* 
This guide demonstrates how to use Hanzo KMS to manage secrets in your Terraform infrastructure code, supporting both traditional data sources and ephemeral resources for enhanced security. It uses:

- Hanzo KMS (you can use [Hanzo KMS Cloud](https://kms.hanzo.ai) or a [self-hosted instance of Hanzo KMS](https://kms.hanzo.ai/docs/self-hosting/overview)) to store your secrets
- The [Terraform Provider](https://registry.terraform.io/providers/Hanzo KMS/kms/latest/docs) to fetch secrets for your infrastructure
...
First, specify the Hanzo KMS provider in your Terraform configuration:
...
Hanzo KMS provides two methods to fetch and use secrets in your Terraform configurations:
...
resource "aws_db_instance" "example" {
  ...
  # Securely inject credentials from Hanzo KMS
  username = jsondecode(ephemeral.kms_secret.db_creds.value)["username"]
  password = jsondecode(ephemeral.kms_secret.db_creds.value)["password"]
}
```
...
To eliminate the need for static credentials, you can authenticate your workflow using [OpenID Connect (OIDC)](https://kms.hanzo.ai/docs/documentation/platform/identities/oidc-auth/general) through providers like the [KMS Secrets GitHub Action](https://github.com/hanzoai/kms/secrets-action).
Once authenticated, you can securely access secrets through the Hanzo KMS provider:
...
2. **Organize Secrets**: Structure your secrets in Hanzo KMS using folders to maintain clean separation:
   ```hcl
...
- [Terraform Provider Registry](https://registry.terraform.io/providers/Hanzo KMS/kms/latest/docs)
- [GitOps Best Practices](https://kms.hanzo.ai/blog/gitops-best-practices) */}
