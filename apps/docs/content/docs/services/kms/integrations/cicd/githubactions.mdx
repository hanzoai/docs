...
The [KMS Secrets Action](https://github.com/hanzoai/kms/secrets-action) enables GitHub Actions workflows to fetch secrets from Hanzo KMS at runtime using [OpenID Connect (OIDC)](/documentation/platform/identities/oidc-auth/github) authentication with [machine identities](/documentation/platform/identities/machine-identities).
...
<Note>
  In GitHub Actions, the [KMS Secrets Action](https://github.com/hanzoai/kms/secrets-action) handles steps 1-2 and 5, automatically fetching secrets after authentication and injecting them as environment variables into your workflow.
</Note>
...
3. Configure your GitHub Actions workflow to use the [KMS Secrets Action](https://github.com/hanzoai/kms/secrets-action) with OIDC authentication.
...
2. **Workflow Presents Its Identity**  
   The [KMS Secrets Action](https://github.com/hanzoai/kms/secrets-action) sends this token to Hanzo KMS as proof of the workflow’s identity.
...
- An [Hanzo KMS project](/documentation/platform/project) ([Hanzo KMS Cloud](https://kms.hanzo.ai) or [self-hosted instance](/self-hosting/overview))
...
**KMS Secrets Action**: The [KMS Secrets Action](https://github.com/hanzoai/kms/secrets-action) step handles:
...
- `identity-id` - The machine identity ID from Hanzo KMS (safe to commit)
- `project-slug` - Your Hanzo KMS project slug (found in project settings)
...
5. **Project Slug**: Verify the `project-slug` matches your Hanzo KMS project slug exactly. You can find this in your project settings.
6. **Environment Slug**: Confirm the `env-slug` matches the exact environment name in Hanzo KMS (e.g., `dev`, `staging`, `prod`). Environment slugs are case-sensitive and must match exactly.
...
- [KMS Secrets Action](https://github.com/hanzoai/kms/secrets-action) - Official GitHub Action repository
