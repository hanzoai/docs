---
sidebarTitle: "Hanzo KMSDynamicSecret CRD"
title: "Using the Hanzo KMSDynamicSecret CRD"
description: "Learn how to generate dynamic secret leases in Hanzo KMS and sync them to your Kubernetes cluster."
---

## Overview

The **Hanzo KMSDynamicSecret** CRD allows you to easily create and manage dynamic secret leases in Hanzo KMS and automatically sync them to your Kubernetes cluster as native **Kubernetes Secret** resources.
This means any Pod, Deployment, or other Kubernetes resource can make use of dynamic secrets from Hanzo KMS just like any other K8s secret.

This CRD offers the following features:

- **Generate a dynamic secret lease** in Hanzo KMS and track its lifecycle.
- **Write** the dynamic secret from Hanzo KMS to your cluster as native Kubernetes secret.
...
- A [machine identity](/documentation/platform/identities/machine-identities) ready for use in Hanzo KMS that has permissions to create dynamic secret leases in the project.
- You have already configured a dynamic secret in Hanzo KMS.
- The operator is installed on to your Kubernetes cluster.

## Configure Dynamic Secret CRD

The example below shows a sample **Hanzo KMSDynamicSecret** CRD that creates a dynamic secret lease in Hanzo KMS, and syncs the lease to your Kubernetes cluster.

```yaml dynamic-secret-crd.yaml
apiVersion: secrets.kms.hanzo.ai/v1alpha1
kind: Hanzo KMSDynamicSecret
metadata:
  name: kmsdynamicsecret
spec:
  hostAPI: https://app.kms.hanzo.ai/api # Optional, defaults to https://app.kms.hanzo.ai/api
  ...
  # If set to "Revoke", leases will be revoked when the Hanzo KMSDynamicSecret CRD is deleted.
  ...
  managedSecretReference:
    ...
    secretNamespace: default # Must be the same namespace as the Hanzo KMSDynamicSecret CRD.
...
Apply the Hanzo KMSDynamicSecret CRD to your cluster.

```bash
kubectl apply -f dynamic-secret-crd.yaml
```

After applying the Hanzo KMSDynamicSecret CRD, you should notice that the dynamic secret lease has been created in Hanzo KMS and synced to your Kubernetes cluster. You can verify that the lease has been created by doing:
...
### Hanzo KMSDynamicSecret CRD properties
...
The lease revocation policy defines what the operator should do with the leases created by the operator, when the Hanzo KMSDynamicSecret CRD is deleted.

Valid values are `None` and `Revoke`.

Behavior of each policy:

- `None`: The operator will not override existing secrets in Hanzo KMS. If a secret with the same key already exists, the operator will skip pushing that secret, and the secret will not be managed by the operator.
- `Revoke`: The operator will revoke the leases created by the operator when the Hanzo KMSDynamicSecret CRD is deleted.
...
<Accordion title="dynamicSecret.projectId">
  The project ID of where the dynamic secret is stored in Hanzo KMS.
...
<Accordion title="dynamicSecret.projectSlug">
  The project slug of where the dynamic secret is stored in Hanzo KMS.
...
<Accordion title="dynamicSecret.environmentSlug">
  The environment slug of where the dynamic secret is stored in Hanzo KMS.
</Accordion>

{" "}

<Accordion title="dynamicSecret.secretsPath">
  The path of where the dynamic secret is stored in Hanzo KMS. The root path is
  `/`.
</Accordion>
...
The `authentication` field dictates which authentication method to use when pushing secrets to Hanzo KMS.
The available authentication methods are `universalAuth`, `kubernetesAuth`, `awsIamAuth`, `azureAuth`, `gcpIdTokenAuth`, and `gcpIamAuth`.
  ...
  <Accordion title="universalAuth">
    The universal authentication method is one of the easiest ways to get started with Hanzo KMS. Universal Auth works anywhere and is not tied to any specific cloud provider.
  ...
  <Accordion title="kubernetesAuth">
    ...
    Valid fields:
    - `identityId`: The identity ID of the machine identity you created.
    - `serviceAccountRef`: The name and namespace of the service account that will be used to authenticate with Hanzo KMS.
  ...
  <Accordion title="awsIamAuth">
    The AWS IAM machine identity authentication method is used to authenticate with Hanzo KMS.
  ...
  <Accordion title="azureAuth">
    The AWS IAM machine identity authentication method is used to authenticate with Hanzo KMS. Azure Auth can only be used from within an Azure environment.
...
<Accordion title="tls">
  ...
  <Accordion title="caRef">
    ...
    Valid fields:
    - `secretName`: The name of the Kubernetes secret containing the CA certificate to use for connecting to the Hanzo KMS instance with SSL/TLS.
    - `secretNamespace`: The namespace of the Kubernetes secret containing the CA certificate to use for connecting to the Hanzo KMS instance with SSL/TLS.
    - `key`: The name of the key in the Kubernetes secret which contains the value of the CA certificate to use for connecting to the Hanzo KMS instance with SSL/TLS.
...
### Applying the Hanzo KMSDynamicSecret CRD to your cluster

Once you have configured the `Hanzo KMSDynamicSecret` CRD with the required fields, you can apply it to your cluster. After applying, you should notice that a lease has been created in Hanzo KMS and synced to your Kubernetes cluster.
...
Deployments referring to Kubernetes secrets containing Hanzo KMS dynamic secrets don't automatically reload when the dynamic secret lease expires. This means your deployment may use expired dynamic secrets unless manually redeployed.
To address this, we've added functionality to automatically redeploy your deployment when the associated Kubernetes secret containing your Hanzo KMS dynamic secret updates.
  ...
  template:
    ...
    spec:
      ...
      - name: nginx
        ...
        envFrom:
        - secretRef:
            name: managed-secret # The name of your managed secret, the same that you're using in your Hanzo KMSDynamicSecret CRD (spec.managedSecretReference.secretName)
        ports:
        - containerPort: 80
```
</Accordion>
