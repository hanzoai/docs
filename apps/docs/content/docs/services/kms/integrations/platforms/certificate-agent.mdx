...
- Leveraging workload identity: The agent can authenticate with Hanzo KMS as a [machine identity](/documentation/platform/identities/machine-identities) using an infrastructure-native authentication method such as [AWS Auth](/docs/services/kms/platform/identities/aws-auth), [Azure Auth](/docs/services/kms/platform/identities/azure-auth), [GCP Auth](/docs/services/kms/platform/identities/gcp-auth), [Kubernetes Auth](/docs/services/kms/platform/identities/kubernetes-auth), etc.
...
1. Create a [certificate profile](/documentation/platform/pki/certificates/profiles) in Hanzo KMS with the [API enrollment method](/documentation/platform/pki/enrollment-methods/api) configured on it.
...
This section describes how to use the KMS Agent to request certificates from Hanzo KMS. It covers how the agent authenticates with Hanzo KMS,
and how to configure it to start requesting certificates from Hanzo KMS.

### Authentication

The KMS Agent can authenticate with Hanzo KMS as a [machine identity](/documentation/platform/identities/machine-identities) using one of its supported authentication methods.

Upon successful authentication, the agent receives a short-lived access token that it uses to make subsequent authenticated requests to obtain and renew certificates from Hanzo KMS;
...
kms:
  address: "https://kms.hanzo.ai" # The URL of the Hanzo KMS instance (e.g. https://kms.hanzo.ai, https://kms.hanzo.ai, https://your-self-hosted-instance.com)
  retry-strategy:
    max-retries: 3
    max-delay: "5s"
    base-delay: "200ms"
...
- Authenticate with Hanzo KMS using the [Universal Auth](/integrations/platforms/certificate-agent#universal-auth) authentication method.
...
Since there are several ways you might want to use the KMS Agent to request certificates from Hanzo KMS,
...
The code snippet below shows a configuration file that instructs the agent to request a certificate from Hanzo KMS
