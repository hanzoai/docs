---
title: "Hanzo KMS Proxy"
description: "Learn how to use the Hanzo KMS Proxy to proxy requests to Hanzo KMS with built-in caching for high availability."
---

## Overview

Hanzo KMS Proxy is a client-side daemon that acts as an API proxy for Hanzo KMS, providing caching capabilities for high availability. Applications connect to the proxy instead of directly to Hanzo KMS, and the proxy handles request forwarding, response caching, and automatic cache refresh.

**Key features:**

- **API Proxy**: Forwards all requests to your Hanzo KMS instance transparently
- **Caching**: Caches secret responses in-memory with automatic refresh
- **High Availability**: Continues serving cached secrets even when Hanzo KMS is unreachable (optimistic eviction strategy)
...
<Note>
The Hanzo KMS Proxy is ideal for scenarios where you need to reduce load on your Hanzo KMS instance, improve response times, or ensure secret availability during network disruptions.
</Note>
...
flowchart LR
    A[Application] -->|Request| B[Hanzo KMS Proxy]
    B -->|Cache Hit| A
    B -->|Cache Miss| C[Hanzo KMS]
    C -->|Response| B
    B -->|Cache & Return| A
...
The Hanzo KMS Proxy sits between your applications and your Hanzo KMS instance, acting as a transparent intermediary. Applications make requests to the proxy using the same API endpoints they would use with Hanzo KMS directly, simply replace your Hanzo KMS host URL with the proxy's address. The proxy handles authentication headers transparently, forwarding them to Hanzo KMS as needed.

When a request arrives, the proxy first checks its in-memory cache for a matching response. Cache lookups are based on the request method, path, query parameters, and authentication token, ensuring that different users and query variations are cached separately. If a matching entry exists, the proxy returns the cached response immediately without contacting Hanzo KMS, significantly reducing latency and load on your Hanzo KMS instance.

For requests that aren't in the cache, the proxy forwards them to Hanzo KMS, waits for the response, and then stores the response in its cache before returning it to the application. This means the first request for any unique combination of parameters will have normal latency, but subsequent identical requests will be served from cache.

To keep cached data fresh, the proxy runs two background processes. The first periodically validates that cached access tokens are still valid by making test requests to Hanzo KMS. If a token has been revoked, all cache entries associated with that token are immediately evicted. The second process refreshes cached secrets on a configurable interval, re-fetching data from Hanzo KMS and updating the cache with the latest values. Both processes use an optimistic strategy: if Hanzo KMS is unreachable, the proxy continues serving cached data rather than evicting entries, ensuring your applications remain operational during outages.

## Install the Hanzo KMS Proxy

The Hanzo KMS Proxy is available through the KMS CLI. Please refer to the [CLI installation documentation](/cli/overview) for more information on how to install the KMS CLI.
...
Then configure your application to use `http://localhost:8081` as the Hanzo KMS host instead of `https://app.kms.hanzo.ai`.
...
<ParamField query="--domain" type="string" required>
  Domain of your Hanzo KMS instance (e.g., https://app.kms.hanzo.ai)
</ParamField>
...
<ParamField query="--tls-enabled" type="boolean" default={true}>
  ...
  Note that all communication between the proxy and Hanzo KMS is always encrypted by default. If you are self-hosting your Hanzo KMS instance, ensure that your Hanzo KMS instance is configured to use TLS.
</ParamField>
...
<ParamField query="--eviction-strategy" type="string" default="optimistic">
  ...
  - `optimistic` - Only evict cached secrets when the proxy is able to reach Hanzo KMS and verify that the underlying cached secret is stale.
</ParamField>
...
<ParamField query="--static-secrets-refresh-interval" type="duration" default="1h">
  How often to refresh cached secrets from Hanzo KMS. Defaults to `1h`. Supports duration formats like `60s|5m|1h|1d|1w|1y`.
</ParamField>
...
These endpoints represent read operations where caching provides the most benefit: reducing latency for frequently accessed secrets and decreasing load on your Hanzo KMS instance.
...
These requests are forwarded directly to Hanzo KMS and their responses are returned to the application without being stored. This design ensures that write operations always reach Hanzo KMS immediately and that the cache only contains secret data that benefits from being cached.
...
For each unique token in the cache, the proxy makes a test request to Hanzo KMS using one of that token's cached requests.
If Hanzo KMS returns `401 Unauthorized` or `403 Forbidden`, all cache entries associated with that token are immediately evicted.

**When the proxy is configured with the optimistic eviction strategy**:
If Hanzo KMS is unreachable during token validation (network errors or `5xx` responses), the proxy keeps cached entries intact rather than evicting them. This ensures that your applications remain operational even if Hanzo KMS is temporarily unavailable.
...
During each refresh cycle, the proxy identifies all cache entries that were last updated longer ago than the refresh interval, then re-fetches each one from Hanzo KMS.
...
**When the proxy is configured with the `optimistic` eviction strategy**:
If Hanzo KMS is unreachable during the refresh cycle (network errors or `5xx` responses), the proxy keeps cached entries intact rather than evicting them. This ensures that your applications remain operational even if Hanzo KMS is temporarily unavailable.
...
<Tip>
  For immediate cache invalidation, perform secret mutations through the proxy rather than directly to Hanzo KMS. For optimal performance it's recommended to use the proxy for all API requests.
</Tip>

### Eviction Strategy
The eviction strategy is used to determine when to evict cached entries from the cache. Currently the Hanzo KMS Proxy only supports the `optimistic` eviction strategy.

#### Optimistic Eviction
* Keeps cached data when Hanzo KMS is unreachable (network errors, or `5xx` responses)
* Ensures high availability when your Hanzo KMS instance is unreachable
...
<Note>
  The `optimistic` eviction strategy prioritizes availability. Applications will continue receiving cached secrets even if Hanzo KMS is down, at the cost of potentially serving slightly stale data.
</Note>
...
Configure your Hanzo KMS SDK to point to the proxy instead of Hanzo KMS directly:
...
const client = new HanzoKMSSDK({
  siteUrl: "http://localhost:8081", // Proxy address instead of Hanzo KMS
...
```python
from kms_sdk import HanzoKMSSDKClient
client = HanzoKMSSDKClient(host="http://localhost:8081") # Proxy address instead of Hanzo KMS
```

### Direct API Calls

```bash
# Instead of calling Hanzo KMS directly:
# curl https://app.kms.hanzo.ai/api/v3/secrets/raw?...
...
<AccordionGroup>
  <Accordion title="Stale data after secret updates">
    If you're updating secrets directly in Hanzo KMS (not through the proxy), the proxy will not automatically invalidate the cache. You will need to manually purge the cache by restarting the proxy, or by waiting for the next `static-secrets-refresh-interval` cycle.

    <Note>
      The `static-secrets-refresh-interval` is the interval at which the proxy will refresh cached secrets from Hanzo KMS.
    </Note>
