...
description: "How to use the Hanzo KMS Kubernetes Agent Injector to inject secrets directly into Kubernetes pods."
---

## Overview

The Hanzo KMS Kubernetes Agent Injector allows you to inject secrets directly into your Kubernetes pods. The Injector will create a [KMS Agent](/integrations/platforms/kms-agent) container within your pod that syncs secrets from Hanzo KMS into a shared volume mount within your pod.


The KMS Agent Injector will patch and modify your pod's deployment to contain an [KMS Agent](/integrations/platforms/kms-agent) container which renders your Hanzo KMS secrets into a shared volume mount within your pod.
...
flowchart LR
    subgraph Secrets Management
        SS(Hanzo KMS) --> INJ(Hanzo KMS Injector)
    end
...
We're looking to add support for other Windows versions in the future. If you're using a different Windows version, please let us know by opening [an issue](https://github.com/hanzoai/kms/kms-agent-injector/issues/new), and we'll look into adding support for your desired version as soon as possible.
...
<AccordionGroup>
  <Accordion title="Run Agent Injector on Windows Server 2019">
    ...
    ```yaml values.yaml
    image:
      repository: kms/kms-agent-injector
...
<AccordionGroup>
  ...
  <Accordion title="org.kms.hanzo.ai/agent-image">
    Specify a custom agent image to use for the agent sidecar / init container(s). Example: `kms/cli:0.43.32`.
...
<Accordion title="kms.address">
  The address of your Hanzo KMS instance. This field is optional and will default to `https://app.kms.hanzo.ai` if not provided.
</Accordion>
...
<Accordion title="kms.auth.type">
  The authentication type to use to connect to Hanzo KMS. Currently only the `kubernetes` authentication type is supported.
  ...
  Please note that the pod's default service account will be used to authenticate with Hanzo KMS.
</Accordion>
...
In this example we'll create a basic nginx deployment and print a Hanzo KMS secret called `API_KEY` to the container logs.

### Create secrets in Hanzo KMS
First you'll need to create the secret in Hanzo KMS.
...
The agent injector will authenticate with Hanzo KMS using a [Kubernetes Auth](/documentation/platform/identities/kubernetes-auth) machine identity. Please follow the [instructions](/documentation/platform/identities/kubernetes-auth) to create a machine identity configured for Kubernetes Auth.
The agent injector will use the service account token of the pod to authenticate with Hanzo KMS.
...
Replace the `<your-project-id>`, `<your-environment-slug>`, with your project ID and the environment slug of where you created your secrets in Hanzo KMS. Replace `<your-kms-machine-identity-id>` with the ID of your machine identity configured for Kubernetes Auth.
```yaml config-map.yaml
apiVersion: v1
kind: ConfigMap
metadata:
...
```
It may take a few minutes for the pod to be ready and for the Hanzo KMS secrets to be injected. You can check the status of the pod by running:
...
API_KEY=sk_api_... # The secret you created in Hanzo KMS
```
...
Defaulted container "simple-app-demo" out of: simple-app-demo, kms-agent-init (init)
API_KEY is set to: sk_api_... # The secret you created in Hanzo KMS
```
...
<Accordion title="The pod is stuck in `Init` state">
  ...
  ```bash
  Starting kms agent...
  11:10AM INF starting Hanzo KMS agent...
  11:10AM INF Hanzo KMS instance address set to https://daniel1.tunn.dev
