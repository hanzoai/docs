---
sidebarTitle: "Hanzo KMSSecret CRD"
title: "Using the Hanzo KMSSecret CRD"
description: "Learn how to use the Hanzo KMSSecret CRD to fetch secrets from Hanzo KMS and store them as native Kubernetes secret resource"
---
...
<Accordion title="authentication.universalAuth">
  ...
  <Steps>
    ...
    <Step title="Add reference for the Kubernetes secret containing the identity credentials">
      Once the secret is created, add the `secretName` and `secretNamespace` of the secret that was just created under `authentication.universalAuth.credentialsRef` field in the Hanzo KMSSecret resource.
    </Step>
  ...
  <Tabs>
    <Tab title="Short-lived service account tokens (Recommended)">
      ...
      <Steps>
        ...
        <Step title="Add your identity ID & service account to your Hanzo KMSSecret resource">
          Once you have created your machine identity and added it to your project(s), you will need to add the identity ID to your Hanzo KMSSecret resource.
          In the `authentication.kubernetesAuth.identityId` field, add the identity ID of the machine identity you created.
          See the example below for more details.
        </Step>
        <Step title="Add your Kubernetes service account token to the Hanzo KMSSecret resource">
...
<Accordion title="authentication.awsIamAuth">
  ...
  <Steps>
    ...
    <Step title="Add your identity ID to your Hanzo KMSSecret resource">
...
<Accordion title="authentication.gcpIamAuth">
  ...
  <Steps>
    ...
    <Step title="Add your identity ID and service account token path to your Hanzo KMSSecret resource">
...
<Accordion title="authentication.ldapAuth">
  ...
  <Steps>
    ...
    <Step title="Add reference for the Kubernetes secret containing the identity credentials">
      Once the secret is created, add the `secretName` and `secretNamespace` of the secret that was just created under `authentication.ldapAuth.credentialsRef` field in the Hanzo KMSSecret resource.
    </Step>
...
You can generate a [service token](/documentation/platform/token) for an Hanzo KMS project by heading over to the Hanzo KMS dashboard then to Project Settings.
...
Once you have configured the Hanzo KMSSecret CRD with the required fields, you can apply it to your cluster.
...
<Accordion title="Custom Metadata (With template.metadata)">
  ...
  - Hanzo KMSSecret labels and annotations are NOT propagated to the managed secret
