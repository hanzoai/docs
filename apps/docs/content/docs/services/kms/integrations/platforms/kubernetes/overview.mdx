...
description: "How to use Hanzo KMS to inject, push, and manage secrets within Kubernetes clusters"
---

The Hanzo KMS Operator is a collection of Kubernetes controllers that streamline how secrets are managed between Hanzo KMS and your Kubernetes cluster. 
...
When these CRDs are configured, the Hanzo KMS Operator will continuously monitors for changes and performs necessary updates to keep your Kubernetes secrets up to date. 
...
<Tabs>
  ...
  <Tab title="Supported Kubernetes distributions">
    ...
    It may work in other Kubernetes distributions, but those are not officially supported. Please report any issues [here](https://github.com/hanzoai/kms/kms/issues).
  </Tab>
</Tabs>
...
1. [Hanzo KMSSecret](/integrations/platforms/kubernetes/kms-secret-crd): Sync secrets from Hanzo KMS to a Kubernetes secret.
2. [Hanzo KMSPushSecret](/integrations/platforms/kubernetes/kms-push-secret-crd): Push secrets from a Kubernetes secret to Hanzo KMS.
3. [Hanzo KMSDynamicSecret](/integrations/platforms/kubernetes/kms-dynamic-secret-crd): Sync dynamic secrets and create leases automatically in Kubernetes.
...
To connect to Hanzo KMS instances behind a private/self-signed certificate, you can configure the TLS settings in the CRD
...
To configure global settings that will apply to all CRD instances (`Hanzo KMSSecret`, `Hanzo KMSPushSecret`, and `Hanzo KMSDynamicSecret`), you can define these configurations in a Kubernetes ConfigMap.
...
| -------- | --------------------------------------------------------------------------------- | ----------------------------- |
| hostAPI  | If `hostAPI` in a CRD instance is left empty, this value will be used | https://app.kms.hanzo.ai/api |
...
The Hanzo KMSSecret resource uses the `status.conditions` field to report its current state and any errors encountered.
