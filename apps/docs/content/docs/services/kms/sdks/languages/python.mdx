---
title: "Hanzo KMS Python SDK"
sidebarTitle: "Python"
icon: "/images/sdks/languages/python.svg"
---

If you're working with Python, the official Hanzo KMS Python SDK package is the easiest way to fetch and work with secrets for your application. 
...
1. **Removal of `rest`**: The SDK no longer exposes the entire Hanzo KMS API. This was nessecary as we have moved away from using an OpenAPI generator approach. We aim to add support for more API resources in the near future. If you have any specific requests, please [open an issue](https://github.com/hanzoai/kms/python-sdk-official/issues).

2. **New response types**: The 1.0.3 release uses return types that differ from the older versions. The new return types such as `BaseSecret`, are all exported from the Hanzo KMS SDK.
...
You can refer to our [legacy documentation](https://github.com/hanzoai/kms/python-sdk-official/tree/9b0403938ee5ae599d42c5f1fdf9158671a15606?tab=readme-ov-file#kms-python-sdk) if need be.
...
client = HanzoKMSSDKClient(host="https://app.kms.hanzo.ai")
...
# Use the SDK to interact with Hanzo KMS
secrets = client.secrets.list_secrets(project_id="<project-id>", environment_slug="dev", secret_path="/")
```

## HanzoKMSSDKClient Parameters

The `HanzoKMSSDKClient` takes the following parameters, which are used as a global configuration for the lifetime of the SDK instance.

- **host** (`str`, _Optional_): The host URL for your Hanzo KMS instance. Defaults to `https://app.kms.hanzo.ai`.
...
```python
client = HanzoKMSSDKClient(
  host="https://app.kms.hanzo.ai", # Defaults to https://app.kms.hanzo.ai
...
4. `kms`: Perform cryptographic operations with Hanzo KMS.
...
**Parameters:**
- `identity_id` (str): The ID of the OIDC identity configuration in Hanzo KMS.
...
**Parameters:**
- `identity_id` (str): The ID of the LDAP identity configuration in Hanzo KMS.
...
- `path` (str, optional): The path to the dynamic secret. Defaults to "/".
- `is_forced` (bool, optional): A boolean flag to delete the dynamic secret from Hanzo KMS without trying to remove it from the external provider. Defaults to `False`.
...
- `path` (str, optional): The path to the dynamic secret. Defaults to "/".
- `is_forced` (bool, optional): A boolean flag to revoke the lease from Hanzo KMS without trying to remove it from the external provider. Defaults to `False`.
