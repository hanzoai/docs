...
```rust
use kms::{AuthMethod, Client, HanzoKMSError, encode_base64, decode_base64};
use kms::secrets::GetSecretRequest;

async fn fetch_secret() -> Result<(), HanzoKMSError> {
...
- `decode_base64(data: &str) -> Result<String, HanzoKMSError>`: Decodes a base64 string
