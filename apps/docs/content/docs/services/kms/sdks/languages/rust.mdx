...
```rust
let mut client = Client::builder()
    .base_url("https://kms.hanzo.ai") // Optional: set custom Hanzo KMS instance URL
    .build()
    .await?;
```

**Parameters**
- `.base_url(url)`: Optional method to set the Hanzo KMS instance URL. Defaults to `https://kms.hanzo.ai` for Hanzo KMS Cloud. Use `https://kms.hanzo.ai` for EU and `http://localhost:8080` for local development.
