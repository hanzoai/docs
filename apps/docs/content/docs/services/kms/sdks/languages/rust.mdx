---
title: "Hanzo KMS Rust SDK"
sidebarTitle: "Rust"
icon: "/images/sdks/languages/rust.svg"
---

If you're working with Rust, the official Hanzo KMS Rust SDK package is the easiest way to fetch and work with secrets for your application.
...
```rust
use kms::{AuthMethod, Client, Hanzo KMSError, encode_base64, decode_base64};
use kms::secrets::GetSecretRequest;

async fn fetch_secret() -> Result<(), Hanzo KMSError> {
    // 1. Build the client. You can chain methods to configure it.
    let mut client = Client::builder()
        .base_url("https://app.kms.hanzo.ai") // Optional: defaults to https://app.kms.hanzo.ai
        .build()
        .await?;
...
```rust
let mut client = Client::builder()
    .base_url("https://app.kms.hanzo.ai") // Optional: set custom Hanzo KMS instance URL
    .build()
    .await?;
```

**Parameters**
- `.base_url(url)`: Optional method to set the Hanzo KMS instance URL. Defaults to `https://app.kms.hanzo.ai` for Hanzo KMS Cloud. Use `https://eu.kms.hanzo.ai` for EU and `http://localhost:8080` for local development.
...
- `decode_base64(data: &str) -> Result<String, Hanzo KMSError>`: Decodes a base64 string
...
1. **Client ID and Secret**: Create a Universal Auth machine identity in your Hanzo KMS project settings
2. **Project ID**: Found in your project settings or URL when viewing a project in the Hanzo KMS dashboard
...
**Note**: Integration tests require a running Hanzo KMS instance and valid authentication credentials.