...
Below you'll find an example that uses the Hanzo KMS SDK to fetch a secret with the key `API_KEY` using [Machine Identity Universal Auth](https://kms.hanzo.ai/docs/documentation/platform/identities/universal-auth)

More examples can be found in the [/examples](https://github.com/hanzoai/kms/kms-cpp-sdk/tree/main/examples) folder.

```cpp
#include <iostream>
#include <libkms/HanzoKMSClient.h>

int main() {

  try {
    Hanzo KMS::HanzoKMSClient client(
        Hanzo KMS::ConfigBuilder()
            .withHostUrl("https://kms.hanzo.ai") // Optionally change this to your custom Hanzo KMS instance URL.
            .withAuthentication(
  ...
  } catch (const Hanzo KMS::HanzoKMSError &e) {
...
### HanzoKMSClient Class
`HanzoKMSClient(Config &config)`
...
Hanzo KMS::HanzoKMSClient client(config);
```

- `withHostUrl(string)` _(optional)_: Specify a custom Hanzo KMS host URL, pointing to your Hanzo KMS instance. Defaults to `https://kms.hanzo.ai`
- `withAuthentication(Hanzo KMS::Authentication)`: Configure the authentication that will be used by the SDK. See [Authentication Class](#authentication-class) for more details.
