...
description: "Learn how to manage secrets in Microsoft Power Apps with Hanzo KMS."
---
...
Importantly, it's crucial to avoid hardcoding these secrets, as they would then be accessible to anyone with collaboration rights to the code. This underscores the importance of using a secret management solution like Hanzo KMS.

In this article, we'll demonstrate how to retrieve app secrets from Hanzo KMS for use in a Power Apps application. We'll create a simple application with a dedicated data connector to illustrate the ease of integrating Hanzo KMS with Power Apps. This tutorial assumes some prior programming experience in C#.
...
<Steps>
    ...
    <Step title="Set Up Hanzo KMS in an Azure Function">

        You can add Hanzo KMS in an Azure Function quite easily using the [Hanzo KMS SDK for .NET](https://kms.hanzo.ai/docs/sdks/languages/csharp) (or other languages). This enables the function to communicate with Hanzo KMS to obtain secrets, among other things.
        ...
        var settings = new ClientSettings
        {
            ...
            // SiteUrl = "http://localhost:8080", <-- This line can be omitted if you're using Hanzo KMS Cloud.
        };
        var kms = new HanzoKMSClient(settings);
        ...
        Knowing the URL of Hanzo KMS as well as the Client Id and Client Secret, we can now access the desired values.
        ...
        ```
        This creates a request to the resource protected by an API key that is retrieved from Hanzo KMS.
