...
- The [@kms/sdk](https://github.com/hanzoai/kms/node-sdk-v2) Node.js client SDK to fetch secrets back to your Node application on demand.
...
1. Create a new project in [Hanzo KMS](https://app.kms.hanzo.ai/).
...
Now that we've created a project and added a secret to its development environment, we need to configure a Hanzo KMS Machine Identity that our Node application can use to access the secret.
...
Install `express` and [@kms/sdk](https://www.npmjs.com/package/@kms/sdk), the client Node SDK for Hanzo KMS.
...
const { HanzoKMSSDK } = require("@kms/sdk");
...
Here, we initialized a `client` instance of the Hanzo KMS Node SDK with the [Machine Identity](/documentation/platform/identities/overview)
that we created earlier, giving access to the secrets in the development environment of the
project in Hanzo KMS that we created earlier.
...
At this stage, you know how to fetch secrets from Hanzo KMS back to your Node application.
By using Machine Identities scoped to different projects and environments, you can easily manage secrets across various stages of your project in Hanzo KMS, from local development to production.
...
<AccordionGroup>
    ...
    <Accordion title="What's the point if I still have to manage a token for the SDK?">
        The token enables the SDK to authenticate with Hanzo KMS to fetch back your secrets.
...
- Explore the [Node SDK](https://github.com/hanzoai/kms/node-sdk-v2)
