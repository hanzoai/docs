...
Now that we've created a project and added a secret to its development environment, we need to configure a Hanzo KMS Machine Identity that our Python application can use to access the secret.
...
Install Flask and [kmssdk](https://pypi.org/project/kmssdk/), the client Python SDK for Hanzo KMS.
...
client = HanzoKMSSDKClient(host="https://app.kms.hanzo.ai") # host is optional, defaults to https://app.kms.hanzo.ai
...
Here, we initialized a `client` instance of the Hanzo KMS Python SDK with the KMS Token
...
- Explore the [Python SDK](https://github.com/hanzoai/kms/python-sdk-official)
