...
description: "Learn how to structure Hanzo KMS for centralized platform administration or team self-service autonomy"
---

Organizations adopt different approaches to secrets management governance based on their security requirements, compliance obligations, and team structures. Hanzo KMS supports a spectrum of governance models—from fully centralized platform administration to team-driven self-service.

This guide covers how to configure Hanzo KMS for different governance approaches and what tradeoffs to consider.
...
Hanzo KMS supports multiple authentication methods that can be configured based on your requirements:
...
[Machine identities](/documentation/platform/identities/machine-identities) authenticate applications, services, and automated systems with Hanzo KMS. Your governance model shapes how these identities are managed:
...
Hanzo KMS supports multiple machine identity authentication methods:
...
Hanzo KMS's [role-based access control](/documentation/platform/access-controls/role-based-access-controls) operates at two levels: organization and project. How you configure these controls determines who can do what across your secrets infrastructure.
...
<Note>
Projects in Hanzo KMS are isolated from one another. Secret referencing and imports work within a single project—you cannot reference secrets across different projects.
</Note>

## Integrations and Secret Delivery

Hanzo KMS offers multiple methods for delivering secrets to applications and infrastructure.

### Secret Syncs

[Secret Syncs](/integrations/secret-syncs/overview) push secrets to third-party platforms like AWS Secrets Manager, Azure Key Vault, GCP Secret Manager, and others. Syncs keep external secret stores updated when values change in Hanzo KMS.
...
- [Hanzo KMS Kubernetes Operator](/integrations/platforms/kubernetes/kms-operator) — Syncs secrets to Kubernetes Secrets resources
...
| **Secret sync patterns** | Standardized CRD configurations | Teams define their own Hanzo KMSSecret resources |
...
[Gateways](/documentation/platform/gateways/overview) enable Hanzo KMS to securely access private resources—such as databases in isolated VPCs—without exposing them to the public internet. Gateways are lightweight components deployed within your private network that establish secure, outbound-only connections to Hanzo KMS.
...
Gateways operate at two levels within Hanzo KMS:
...
Visibility into secrets access and changes is critical for security and compliance. Hanzo KMS provides audit capabilities at both organization and project levels.
...
Beyond access control, Hanzo KMS offers additional security settings.
...
- Training for teams on Hanzo KMS features and security best practices
- Runbooks for common operations (secret rotation, access requests, incident response)

## Summary

Here's a quick reference for how key Hanzo KMS features map to each governance model:
