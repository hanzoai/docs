...
The `zt` CLI ships with Hanzo ZT itself — it is not part of the `hanzo` CLI.
Build it from [hanzoai/zt](https://github.com/hanzoai/zt); the local development
tutorial in that repo has the steps.

### 2. Login to the Controller

```bash
zt edge login https://zt-api.hanzo.ai:443 \
  --username admin \
...
zt edge create identity my-service \
  --role-attributes "services,backend"

# Enroll the identity (generates certificate)
zt edge create enrollment ott my-service -o my-service.jwt
zt edge enroll --jwt my-service.jwt --out my-service.json
```
...
zt edge create config my-api-intercept intercept.v1 \
  '{"protocols":["tcp"],"addresses":["my-api.hanzo.internal"],"portRanges":[{"low":443,"high":443}]}'

zt edge create config my-api-host host.v1 \
