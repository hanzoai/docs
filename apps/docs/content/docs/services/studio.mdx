...
curl -X PUT https://studio.hanzo.ai/compute/config \
...
```bash
curl -X POST https://studio.hanzo.ai/prompt \
  -H "Authorization: Bearer $HANZO_API_KEY" \
...
```bash
curl https://studio.hanzo.ai/queue \
  -H "Authorization: Bearer $HANZO_API_KEY"
```
...
```bash
curl https://studio.hanzo.ai/models \
  -H "Authorization: Bearer $HANZO_API_KEY"
```
