...
> **API reference** · [Hanzo Engine API →](/docs/openapi/engine) — every endpoint, generated from the OpenAPI spec.

Hanzo Engine is a high-performance, multimodal inference engine written in Rust. It serves LLMs, vision models, speech models, image generation models, and embedding models over HTTP on `/v1/*`, with optional MCP (Model Context Protocol) server support.
...
- **HTTP API**: Serves `/v1/chat/completions`, `/v1/completions`, `/v1/embeddings`, `/v1/images/generations`, and `/v1/models`
...
Hanzo Engine serves a REST API on `/v1/*`. The endpoints take and return the
same JSON shapes as the widely-implemented chat-completions format, so an HTTP
client written against that shape works once its base URL points at the Engine.
