...
description: SQLite, ZAP binary protocol (SQL, DocumentDB, KV, Datastore), and custom driver support.
---
...
| SQL | `orm.OpenZap` + `ZapSQL` | ZAP binary | Production SQL |
| MongoDB/FerretDB | `orm.OpenZap` + `ZapDocumentDB` | ZAP binary | Document storage |
| KV | `orm.OpenZap` + `ZapKV` | ZAP binary | KV storage |
| Datastore | `orm.OpenZap` + `ZapDatastore` | ZAP binary | Analytics/columnar |
...
**Document backends** (DocumentDB via ZapDocumentDB):
- `Get` → `find` with `{_id, kind}` filter
- `Put` → `update` with `$set` (upsert) or `insert`
- `Query` → `find` with DocumentDB query operators
- `Delete` → `update` with `{deleted: true}`

**KV backends** (KV via ZapKV):
- `Get` → `GET orm:kind:id`
- `Put` → `SET orm:kind:id`
...
| **Backends** | SQLite only | SQL, DocumentDB, KV, Datastore |
