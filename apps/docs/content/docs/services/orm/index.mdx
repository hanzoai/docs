...
// Or ZAP binary protocol (SQL, DocumentDB, KV, Datastore)
...
| **KV Cache** | Read-through caching with Memory LRU, KV, or no-op backends |
| **SQLite Driver** | WAL mode, JSON storage, `json_extract` queries, `sqlite-vec` ready |
| **ZAP Driver** | Binary protocol to SQL, DocumentDB, KV, Datastore via zap-sidecar |
...
<Cards>
  <Card title="DB" href="/docs/services/sql">
    Serverless SQL for relational data and embeddings
  </Card>
  <Card title="KV" href="/docs/services/kv">
    KV for caching, sessions, and pub/sub
  </Card>
