---
title: Caching
description: Read-through KV cache with memory LRU and KV backends.
---
...
| **KV** | `orm.NewKVCache(...)` | Multi-process, distributed apps |
...
|-----------|--------------|-------------------|
| Get by ID | ~1-5ms (SQLite) | < 0.1ms (memory) / < 1ms (KV) |
| Query (10 results) | ~5-20ms | < 0.5ms (memory) / < 2ms (KV) |
