---
title: Hanzo MQ
description: High-performance message queue and job processing built on Hanzo KV (KV Streams) with reliable delivery, priority queues, and dead letter support.
---

# Hanzo MQ

> **API reference** · [Hanzo MQ API →](/docs/openapi/mq) — every endpoint, generated from the OpenAPI spec.

Hanzo MQ (`@hanzo/mq`) is a distributed message queue and job processing system built on top of [Hanzo KV](/docs/services/kv) (KV-compatible). It provides reliable, at-least-once delivery with priority queues, delayed jobs, dead letter queues, rate limiting, and parent-child job dependencies -- all backed by atomic Lua scripts running on KV Streams.

**Package:** `@hanzo/mq` (v5.70.0)
**Protocol:** KV Streams-based
**Backend:** Hanzo KV / KV
...
| Feature | Description |
|---------|-------------|
| **KV Streams Backend** | All operations use atomic Lua scripts on KV Streams for consistency |
...
| **Global Events** | Real-time event streaming for job lifecycle via KV Streams |
...
Multiple workers consuming the same queue name form an implicit consumer group. Hanzo MQ uses atomic KV operations to ensure each job is delivered to exactly one worker at a time.
...
|--------|------|---------|-------------|
| `connection` | `ConnectionOptions` | `localhost:6379` | Hanzo KV / KV connection |
...
|---------|-----------|-------|
| **KV** 6.2+ | `INFO server` response contains `redis_version` | Full feature support |
| **KV** 7.2+ | `INFO server` response contains `valkey_version` | Recommended for Hanzo KV deployments |
...
<Cards>
  <Card title="KV" href="/docs/services/kv">
    The KV-compatible key-value store that powers MQ's backend storage
  </Card>
