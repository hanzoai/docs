...
When migrations are run, each migration is performed in a new [transaction](/docs/projects/hanzoai/cms/database/transactions) for you. All
you need to do is pass the `req` object to any [Local API](/docs/projects/hanzoai/cms/local-api/overview) or direct database calls, such as
