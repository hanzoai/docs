...
To convert richtext to JSX, import the `RichText` component from `@hanzo/cms-richtext-lexical/react` and pass the richtext content to it:
...
If you want to share the same node rendering logic between the admin panel and your frontend, consider using [Views](/docs/projects/hanzoai/cms/rich-text/views) instead. Views let you define a single node map that works in both contexts.
