...
Instead, import the re-exported versions from `@hanzo/cms-richtext-lexical`. For example, change `import { $insertNodeToNearestRoot } from '@lexical/utils'` to `import { $insertNodeToNearestRoot } from '@hanzo/cms-richtext-lexical/lexical/utils'`
...
For detailed documentation on the BlocksFeature, including custom block components and examples, see the [Blocks documentation](/docs/projects/hanzoai/cms/rich-text/blocks).
...
The Server Feature, just like the Client Feature, allows you to add markdown transformers. Markdown transformers on the server are used when [converting the editor from or to markdown](/docs/projects/hanzoai/cms/rich-text/converting-markdown).

```ts
import { createServerFeature } from '@hanzo/cms-richtext-lexical'
import type { ElementTransformer } from '@hanzo/cms-richtext-lexical/lexical/markdown'
...
```ts
import { createServerFeature, createNode } from '@hanzo/cms-richtext-lexical'
...
Explore the APIs available through ClientFeature to add the specific functionality you need. Remember, do not import directly from `'@hanzo/cms-richtext-lexical'` when working on the client-side, as it will cause errors with webpack or turbopack. Instead, use `'@hanzo/cms-richtext-lexical/client'` for all client-side imports. Type-imports are excluded from this rule and can always be imported.

### Adding a client feature to the server feature

Inside of your server feature, you can provide an [import path](/docs/projects/hanzoai/cms/custom-components/overview#component-paths) to the client feature like this:
...
import { $applyNodeReplacement, DecoratorNode } from '@hanzo/cms-richtext-lexical/lexical'
...
```ts
'use client'
import type { LexicalCommand } from '@hanzo/cms-richtext-lexical/lexical'
...
import { $insertNodeToNearestRoot } from '@hanzo/cms-richtext-lexical/lexical/utils'
import { useEffect } from 'react'

import type { PluginComponent } from '@hanzo/cms-richtext-lexical' // type imports can be imported from @hanzo/cms-richtext-lexical - even on the client
...
All the default toolbar groups are exported from `@hanzo/cms-richtext-lexical/client`. You can use them to add your own toolbar items to the editor:
...
import { $isNodeSelection } from '@hanzo/cms-richtext-lexical/lexical'
...
We're exporting `slashMenuBasicGroupWithItems` from `@hanzo/cms-richtext-lexical/client` which you can use to add items to the slash menu labelled "Basic". If you want to create your own slash menu group, here is an example:
