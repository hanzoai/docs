...
editor, based on Slate, has been deprecated and will be removed in 4.0. You can read [its documentation](/docs/projects/hanzoai/cms/rich-text/slate),
or the [migration guide](/docs/projects/hanzoai/cms/rich-text/migration) to migrate from Slate to Lexical (recommended).

</Banner>

The editor is the most important property of the [rich text field](/docs/projects/hanzoai/cms/fields/rich-text).
...
```bash
pnpm install @hanzo/cms-richtext-lexical
```
...
Every single piece of saved data is 100% fully typed within lexical. It provides a type for every single node, which can be imported from `@hanzo/cms-richtext-lexical` - each type is prefixed with `Serialized`, e.g., `SerializedUploadNode`.
...
Make sure to only use types exported from `@hanzo/cms-richtext-lexical`, not from the lexical core packages. We only have control over the types we export and can make sure they're correct, even though the lexical core may export types with identical names.
...
Learn more about [Views](/docs/projects/hanzoai/cms/rich-text/views).
...
```ts
import { hasText } from '@hanzo/cms-richtext-lexical/shared'
