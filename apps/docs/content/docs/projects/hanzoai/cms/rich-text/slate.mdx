...
The [default Payload editor](/docs/projects/hanzoai/cms/rich-text/overview) is currently based on Lexical. This documentation is about our old Slate-based editor which has been deprecated and will be removed in 4.0. We recommend [migrating to Lexical](/docs/projects/hanzoai/cms/rich-text/migration) instead.
...
```
npm install --save @hanzo/cms-richtext-slate
```
...
This allows [fields](/docs/projects/hanzoai/cms/fields/overview) to be saved as extra fields on a link inside the Rich Text Editor. When this is present, the fields will render inside a modal that can be opened by clicking the "edit" button on the link element.
...
This allows [fields](/docs/projects/hanzoai/cms/fields/overview) to be saved as meta data on an upload field inside the Rich Text Editor. When this is present, the fields will render inside a modal that can be opened by clicking the "edit" button on the upload element.
...
Similar to the `relationship` element, the `upload` element is a user-friendly way to reference [Upload-enabled collections](/docs/projects/hanzoai/cms/upload/overview) with a UI specifically designed for media / image-based uploads.
