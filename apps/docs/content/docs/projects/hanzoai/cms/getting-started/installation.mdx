...
- Next.js (one of the following version ranges):
  ...
  - `15.4.11` - `15.4.x`
  - `16.2.6`+
- Any [compatible database](/docs/projects/hanzoai/cms/database/overview) (MongoDB, Postgres or SQLite)
...
- `sharp` - Image resizing, cropping, and focal point support (only needed if you use [Upload](/docs/projects/hanzoai/cms/upload/overview) collections with image manipulation)
- `graphql` - Only needed if you want to use the [GraphQL API](/docs/projects/hanzoai/cms/graphql/overview)
...
Next, install a [Database Adapter](/docs/projects/hanzoai/cms/database/overview). Payload requires a Database Adapter to establish a database connection. Payload works with all types of databases, but the most common are MongoDB and Postgres.
...
<Banner type="success">
  **Note:** New [Database Adapters](/docs/projects/hanzoai/cms/database/overview) are becoming
...
Although this is just the bare minimum config, there are _many_ more options that you can control here. To reference the full config and all of its options, [click here](/docs/projects/hanzoai/cms/configuration/overview).
