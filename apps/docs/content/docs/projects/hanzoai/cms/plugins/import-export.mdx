...
![https://www.npmjs.com/package/@hanzo/cms-plugin-import-export](https://img.shields.io/npm/v/@hanzo/cms-plugin-import-export)
...
```bash
pnpm add @hanzo/cms-plugin-import-export
```
...
By default, the plugin uses Payload's [Jobs Queue](/docs/projects/hanzoai/cms/jobs-queue/overview) for import and export operations. Queued jobs require a runner to be configured, otherwise imports and exports will stay in "pending" status and never complete.
...
```ts
import type { FieldBeforeExportHook } from '@hanzo/cms-plugin-import-export'
...
```ts
import type { FieldBeforeImportHook } from '@hanzo/cms-plugin-import-export'
