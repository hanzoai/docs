...
If your front-end imports from `@hanzo/cms-ui`, ensure that you do not bundle the entire package as this can significantly increase your bundle size.

To do this, import using the full path to the specific component you need:

```ts
import { Button } from '@hanzo/cms-ui/elements/Button'
```

Custom components within the Admin Panel, however, do not have this same restriction and can import directly from `@hanzo/cms-ui`:
...
<Banner type="warning">
  **Note:** This is enabled by default in `@hanzo/create-cms-app` since v3.28.0. If
