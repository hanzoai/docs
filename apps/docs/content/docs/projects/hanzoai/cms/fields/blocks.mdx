...
| **`blocks`** \*        | Array of [block configs](/docs/projects/hanzoai/cms/fields/blocks#config-options) to be made available to this field.                                                                                                                                                                                     |
| **`validate`**         | Provide a custom validation function that will be executed on both the Admin Panel and the backend. [More details](/docs/projects/hanzoai/cms/fields/overview#validation).                                                                                                                                |
...
| **`defaultValue`**     | Provide an array of block data to be used for this field's default value. [More details](/docs/projects/hanzoai/cms/fields/overview#default-values).                                                                                                                                                      |
| **`localized`**        | Enable localization for this field. Requires [localization to be enabled](/docs/projects/hanzoai/cms/configuration/localization) in the Base config. If enabled, a separate, localized set of all data within this field will be kept, so there is no need to specify each nested field as `localized`.   |
| **`unique`**           | Enforce that each entry in the Collection has a unique value for this field. This creates a database-level unique index on the field's path. [More details](/docs/projects/hanzoai/cms/database/indexes#unique-fields).                                                                                   |
...
<Banner type="warning">
  ...
  [`validate`](/docs/projects/hanzoai/cms/fields/overview#validation) function on this Blocks field.
...
If you're using this block within the [Lexical editor](/docs/projects/hanzoai/cms/rich-text/overview), you can also customize how the block is rendered in the Lexical editor itself by specifying custom components.
...
| **`dbName`**               | Custom table name for this block type when using SQL Database Adapter ([Postgres](/docs/projects/hanzoai/cms/database/postgres)). Auto-generated from slug if not defined.                               |
| **`custom`**               | Extension point for adding custom data (e.g. for plugins)                                                                                                                           |
