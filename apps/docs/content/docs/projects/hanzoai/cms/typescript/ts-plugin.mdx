...
Payload ships an optional TypeScript Language Service Plugin (`@hanzo/cms-typescript-plugin`) that enhances your IDE experience when working with [Custom Components](../custom-components/overview). It understands Payload's `PayloadComponent` import path conventions and provides:
...
```bash
pnpm add -D @hanzo/cms-typescript-plugin
```
...
{
  "compilerOptions": {
    "plugins": [{ "name": "next" }, { "name": "@hanzo/cms-typescript-plugin" }]
  }
}
```
...
| Package import          | `'@hanzo/cms-ui/rsc#MyComponent'` | Resolved via node_modules      |
...
```json
{
  "compilerOptions": {
    "plugins": [
      {
        "name": "@hanzo/cms-typescript-plugin",
        "baseDir": "./src"
      }
    ]
  }
