...
This defines a reusable task with a unique `slug`, an `inputSchema` that validates and types the input data, and a `handler` function containing the work to be performed. The `retries` option ensures the task will automatically retry up to 3 times if it fails. Learn more about [Tasks](/docs/projects/hanzoai/cms/jobs-queue/tasks).
...
The [`autoRun`](/docs/projects/hanzoai/cms/jobs-queue/queues#cron-jobs) configuration automatically processes queued jobs on a schedule using cron syntax. In this example, Payload checks for pending jobs every 5 minutes and executes them. Alternatively, you can [manually trigger job processing](/docs/projects/hanzoai/cms/jobs-queue/queues#local-api) with `payload.jobs.run()` or use the [API endpoint](/docs/projects/hanzoai/cms/jobs-queue/queues#endpoint) for serverless platforms.
...
<Banner type="warning">
  **For Serverless Platforms:** If deploying to Vercel or similar platforms,
  ...
  approach](/docs/projects/hanzoai/cms/jobs-queue/queues#vercel-cron-example) instead.
</Banner>
...
See [Job Schedules](/docs/projects/hanzoai/cms/jobs-queue/schedules) for more details on scheduling options and advanced patterns.
