...
Similarly to the Update function, returns a boolean or a [query constraint](/docs/projects/hanzoai/cms/queries/overview) to limit which documents can be deleted by which users.
...
Determines which users can [unlock](/docs/projects/hanzoai/cms/authentication/operations#unlock) other users who may be blocked from authenticating successfully due to [failing too many login attempts](/docs/projects/hanzoai/cms/authentication/overview#config-options).
