# Configuring OpenTelemetry Demo App with O11y

[The OpenTelemetry Astronomy Shop](https://github.com/open-telemetry/opentelemetry-demo) is an e-commerce web application, with **15 core microservices** in a **distributed system** which communicate over gRPC. Designed as a **polyglot** environment, it leverages a diverse set of programming languages, including Go, Python, .NET, Java, and others, showcasing cross-language instrumentation with OpenTelemetry. The intention is to get a quickstart application to send data and experience O11y firsthand.

This guide provides a step-by-step walkthrough for setting up the **OpenTelemetry Demo App** with **O11y** as backend for observability. It outlines steps to export telemetry data to **O11y self-hosted with Docker**, **O11y self-hosted with Kubernetes** and **O11y cloud**. 
<br/>

__Table of Contents__
- [Send data to O11y Self-hosted with Docker](#send-data-to-o11y-self-hosted-with-docker)
  ...
  - [Monitor with O11y (Docker)](#monitor-with-o11y-docker)
- [Send data to O11y Self-hosted with Kubernetes](#send-data-to-o11y-self-hosted-with-kubernetes)
  ...
  - [Monitor with O11y (Kubernetes)](#monitor-with-o11y-kubernetes)
- [What's next](#whats-next)


# Send data to O11y Self-hosted with Docker

In this guide you will install the OTel demo application using Docker and send telemetry data to O11y hosted with Docker, referred as O11y [Docker] from now.
...
To add O11y [Docker] as the backend, open the file `src/otel-collector/otelcol-config-extras.yml` and add the following,
```yaml
exporters:
  otlp:
...
The O11y OTel collector [O11y's otel-collector service] listens at 4317 port on localhost. When the OTel demo app is running within a Docker container and needs to transmit telemetry data to O11y, it cannot directly reference 'localhost' as this would refer to the container's own internal network. Instead, Docker provides a special DNS name, `host.docker.internal`, which resolves to the host machine's IP address from within containers. By configuring the OpenTelemetry Demo application to send data to `host.docker.internal:4317`, we establish a network path that allows the containerized application to transmit telemetry data across the container boundary to the O11y OTel collector running on the host machine's port 4317.
...
>
<br>
<u>To send data to O11y Cloud</u>
...
exporters:
  otlp:
    endpoint: "https://ingest.{your-region}.o11y.cloud:443"
    tls:
      insecure: false
    headers:
...
Both O11y and OTel demo app [frontend-proxy service, to be accurate] share common port allocation at 8080. To prevent port allocation conflicts, modify the OTel demo application config to use port 8081 as the `ENVOY_PORT` value as shown below, and run docker compose command.

Also, both O11y and OTel Demo App have the same `PROMETHEUS_PORT` configured, by default both of them try to start at `9090`, which may cause either of them to fail depending upon which one acquires it first. To prevent this, we need to mofify the value of `PROMETHEUS_PORT` too.
...
Navigate to `http://localhost:8081/` where you can access OTel demo app UI. Generate some traffic to send to O11y [Docker].

## Monitor with O11y [Docker]
...
This verifies that your OTel demo app is successfully sending telemetry data to O11y [Docker] as expected. 


# Send data to O11y Self-hosted with Kubernetes

In this guide you will install the OTel demo application using Helm and send telemetry data to O11y hosted with Kubernetes, referred as O11y [Kubernetes] from now.
...
You’ll need to **install the Helm repository** to start sending data to O11y cloud.

```sh
helm repo add open-telemetry https://open-telemetry.github.io/opentelemetry-helm-charts
```
The OpenTelemetry Collector’s configuration is exposed in the Helm chart. All additions made will be merged into the default configuration. We use this capability to add O11y as an exporter, and make pipelines as desired.
...
```
Replace namespace with your appropriate namespace. This file will replace the chart’s existing settings with our new ones, ensuring telemetry data is sent to O11y [Kubernetes].
...
If you wish to send data to cloud instance of O11y, we have to create a `values.yaml` which will override the existing configurations that comes with the Helm chart.
...
```
Navigate to `http://localhost:8081/` where you can access OTel demo app UI. Generate some traffic to send to O11y [Kubernetes].



## Monitor with O11y [Kubernetes]
...
This verifies that your OTel demo app is successfully sending telemetry data to O11y [Kubernetes] as expected. 
