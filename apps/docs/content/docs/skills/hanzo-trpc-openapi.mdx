---
title: "Hanzo tRPC OpenAPI"
description: "`@hanzo/trpc-openapi` generates OpenAPI 3.x documents from tRPC v11 routers and serves those procedures as REST through adapters for Express, Fastify, Next.js, Nuxt, Koa and the Fetch API. One router definition, two surfaces, no drift."
---

## Overview

`@hanzo/trpc-openapi` is a TypeScript library that generates **OpenAPI 3.x documents** from tRPC v11 routers and provides HTTP adapters to serve tRPC procedures as REST endpoints. Published on npm as `@hanzo/trpc-openapi`. One router definition produces both the tRPC client types and the REST surface, so the OpenAPI document can never drift from the code that serves it.
...
- Optional input/output parameters are emitted as required in the generated
  document, so consumers get a tight contract rather than an all-optional blob
- `.meta({ openapi: ... })` is optional on every procedure — without it the
  method comes from the procedure type and the path from the procedure path
- `override` flag: use only the provided openapi object, merge no defaults
- `additional` flag: merge the provided openapi on top of the defaults
...
- `hanzo/hanzo-ai-api.md` - the Hanzo AI API
