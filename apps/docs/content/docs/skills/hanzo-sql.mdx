---
title: "Hanzo SQL"
description: "Hanzo SQL is a PostgreSQL fork with pre-built extensions for AI workloads: pgvector (vector similarity), pg_cron (scheduled jobs), pg_documentdb (MongoDB wire protocol), and PostGIS (geospatial). S..."
---
...
2. **Docker** for container deployment (or full SQL build toolchain)
...
Init script (`docker-entrypoint-initdb.d/01-extensions.sql`) enables them on startup. DocumentDB extensions are wrapped in exception handlers so SQL starts even if they fail to load.
...
sql/
  ...
  conf/
    postgresql.conf      # Hanzo-tuned SQL config
  ...
  src/                   # SQL source code
  contrib/               # SQL contrib modules
  doc/                   # SQL documentation
...
- `hanzo/hanzo-kv.md` - KV-compatible cache
- `hanzo/hanzo-orm.md` - Go ORM with SQL backend
- `hanzo/hanzo-datastore.md` - Datastore (OLAP)
