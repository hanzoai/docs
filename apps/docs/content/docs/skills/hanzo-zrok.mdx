---
title: "Hanzo Zrok"
description: "Hanzo Zrok is a zero-trust sharing platform on the Hanzo ZT fabric — share web services, files and network resources through firewalls and NAT with no network configuration changes. Single binary, public and private modes, self-hostable."
---

## Overview

Hanzo Zrok is a **zero-trust sharing platform** built on the Hanzo ZT fabric. It lets users securely share web services, files, and network resources through firewalls and NAT without any network configuration changes. Single binary. Public and private sharing modes. Self-hostable.
...
Zrok is an application layer over the ZT zero-trust fabric, and links these
libraries to speak it:
...
`hanzoai/zrok`. License: Apache 2.0.

**Note**: the Go module path is still `github.com/openziti/zrok/v2`. Import that path, not `hanzoai/zrok` — the rename has not reached go.mod, and the docs would be lying if they said otherwise.
...
- **Hanzo Zrok** (`hanzoai/zrok`) = ZT-based sharing platform for network services
