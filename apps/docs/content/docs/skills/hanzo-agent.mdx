---
title: "Hanzo Agent SDK - Multi-Agent Orchestration Framework"
description: "Hanzo Agent SDK is a Python multi-agent framework for agents that plan, use tools, hand off to specialists, and keep memory. Guardrails, tracing and streaming are built in; MCP and ZAP tools work out of the box."
---

## Overview

Hanzo Agent SDK is a **Python multi-agent framework** for building agents that plan, use tools, hand off work to specialists, and keep memory across turns. Guardrails, tracing and streaming are part of the core rather than add-ons, which is what makes it usable for production agentic workflows rather than demos.

**NOTE**: the distribution is `hanzo-agent` but the import path is `agents` — `from agents import Agent, Runner`.

### Why Hanzo Agent SDK?

- **Multi-agent**: Orchestrate teams of specialized agents
- **OpenAI compatible**: works against any OpenAI-compatible endpoint, including `api.hanzo.ai/v1`
...
`hanzoai/agent`. Distribution `hanzo-agent` v0.0.4; import path `agents`.

**NOTE**: do not confuse this with `hanzoai`, the Hanzo Python SDK published from `hanzoai/python-sdk` — they are different packages for different jobs.
...
- Running agent workloads self-hosted, against your own models
