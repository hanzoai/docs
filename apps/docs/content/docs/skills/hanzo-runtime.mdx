---
title: "Hanzo Runtime - Secure AI Code Execution Sandbox"
description: "Hanzo Runtime is a secure isolated sandbox for executing AI-generated code — sub-90ms sandbox creation, OCI/Docker compatible, with programmatic File, Git, LSP and Execute APIs. SDKs published as `hanzo-runtime` (PyPI) and `@hanzo/runtime` (npm)."
---
...
`hanzoai/runtime`. License: AGPL-3.0.
...
4. **SQL** for persistent state
5. **KV** for caching and messaging
...
| TypeORM migration fail | Missing SQL | Start SQL, set DATABASE_URL |
