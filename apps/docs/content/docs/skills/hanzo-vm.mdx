---
title: "Hanzo VM"
description: "Hanzo VM is an open-source cloud operating system and virtual machine management platform — one Go binary with a bundled React frontend, managing VMs across AWS, Azure, GCP, Alibaba, Tencent and DigitalOcean plus libvirt and Proxmox. Apache-2.0."
---
...
- **Database**: SQL (default), MySQL, or any XORM-supported DB
...
1. **SQL** (or MySQL) database accessible
...
| `redisEndpoint` | - | KV for sessions (optional, falls back to file) |
...
vm/
  ...
  docker-compose.yml       # VM + SQL stack
...
Sessions, RBAC roles, and the OIDC login flow all resolve through that one
dependency — Hanzo VM has no user table of its own.
...
| DB connection error | Wrong dataSourceName | Check SQL host/port/user in `conf/app.conf` |
...
| Session expired | KV not configured | Set `redisEndpoint` in app.conf or accept file-based sessions |
...
- `hanzo/hanzo-sql.md` - SQL database
