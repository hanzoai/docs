...
- 4 backend proxies, each in `internal/`:
  ...
  - `datastore/` -- Datastore proxy (clickhouse-go v2, native TCP)
...
- Adding MCP tool support to PostgreSQL, Redis, Datastore, or MongoDB
...
hanzoai/zap/
  ...
  internal/
    ...
    datastore/
      proxy.go              # Datastore proxy (clickhouse-go v2)
    documentdb/
...
- `github.com/ClickHouse/clickhouse-go/v2` v2.43.0 -- Datastore native driver
