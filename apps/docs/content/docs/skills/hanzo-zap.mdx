---
title: "Hanzo ZAP"
description: "Hanzo ZAP (`hanzoai/zap`) is a Go sidecar that bridges the ZAP protocol to backend infrastructure services. It runs as a sidecar container alongside databases and caches, translating ZAP protocol c..."
---

## Overview

Hanzo ZAP (`hanzoai/zap`) is a **Go sidecar that bridges the ZAP protocol to backend infrastructure services**. It runs alongside databases and caches, translating ZAP protocol calls from the Hanzo Gateway into native protocol calls against the co-located backend. Supports 4 modes: SQL (SQL via pgx), KV (KV), Datastore (native TCP), and DocumentDB (DocumentDB wire protocol). ZAP's schema maps 1:1 with MCP (Model Context Protocol), so any service implementing the ZAP interface gets MCP tools and resources for free.

### What it actually is

- Single Go binary (`bin/zap`) with mode flag selecting the backend
- 4 backend proxies, each in `internal/`:
  - `sql/` -- SQL proxy (pgx v5 driver)
  - `kv/` -- Valkey/Redis proxy (hanzoai/kv-go v9)
  - `datastore/` -- Datastore proxy (native TCP)
...
- Adding MCP tool support to SQL, KV, Datastore, or DocumentDB
...
hanzoai/zap/
  ...
  internal/
    ...
    sql/
      proxy.go              # SQL proxy (pgx v5)
    kv/
      proxy.go              # Valkey/Redis proxy (kv-go v9)
    datastore/
      proxy.go              # Datastore proxy (native TCP)
    documentdb/
...
- `github.com/jackc/pgx/v5` v5.7.2 -- SQL driver
- `github.com/hanzoai/kv-go/v9` v9.17.2-hanzo.1 -- Valkey/Redis client
- `github.com/hanzo-ds/go` -- Datastore native driver
...
| `sql_health` | Check SQL connection health |
...
| `kv_cmd` | Execute arbitrary KV command |
...
| Tool | Description |
|------|------------|
| `datastore_query` | Execute Datastore SQL query, return JSON rows |
...
- `hanzo/hanzo-sql.md` -- SQL (hanzoai/sql)
- `hanzo/hanzo-kv.md` -- KV (hanzoai/kv)
- `hanzo/hanzo-datastore.md` -- Datastore analytics
