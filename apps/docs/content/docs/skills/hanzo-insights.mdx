---
title: "Hanzo Insights - Product Analytics Platform"
description: "Hanzo Insights is a full product analytics platform — product analytics, feature flags, session recording, A/B testing, heatmaps, LLM analytics, error tracking, surveys and its own query language (InsightsQL), on Datastore and SQL."
---

## Overview

Hanzo Insights is a **full product analytics platform** with product analytics, feature flags, session recording, A/B testing, heatmaps, LLM analytics, error tracking, surveys, web analytics, and a custom query language (InsightsQL). Polyglot monorepo: Django/Python backend, React/TypeScript frontend, Rust high-performance services, Go livestream server. Datastore for event storage, SQL for metadata, Kafka for event streaming, KV for caching. Self-hostable with Docker Compose or K8s.
...
- **Rust services**: axum, rdkafka, sqlx, datastore (workspace with 30+ crates)
- **Livestream**: Go service for real-time event streaming
- **Query engine**: InsightsQL (ANTLR4 grammar -> Python3 + C++ targets)
- **Databases**: Datastore (events), SQL (metadata), Kafka (streaming), KV (cache)
...
- Data warehouse querying (Snowflake, BigQuery, SQL, S3)
- Self-hosted analytics with full data ownership

## Hard requirements

1. **Datastore** for event storage (columnar analytics)
2. **SQL** for metadata and Django ORM
3. **Kafka** (Redpanda) for event streaming pipeline
4. **KV** for caching and task queues
5. **Object storage** (Hanzo S3, or any S3-compatible endpoint) for session recordings and batch exports
...
| API | `https://insights.hanzo.ai/v1/` |
...
insights/
  insights/           # Django app (Python backend)
    ...
    datastore/        # Datastore query builders
    session_recordings/
    heatmaps/
    batch_exports/
  ...
  funnel-udf/         # Rust Datastore UDF for funnels
  ...
  docker/             # Docker configs (Datastore, Caddy, Temporal, etc.)
...
- `objectstorage`: Hanzo S3 (`ghcr.io/hanzoai/storage`)
...
Custom query language with ANTLR4 grammar. Two parser targets: Python3 (backend queries) and C++ (Datastore UDFs). Supports:
