...
# Calls made through this client are traced automatically
...
```bash
# Create trace
curl -X POST https://console.hanzo.ai/v1/o11y/traces \
...
# Get trace
curl https://console.hanzo.ai/v1/o11y/traces/{trace_id} \
  -u "${LANGFUSE_PUBLIC_KEY}:${LANGFUSE_SECRET_KEY}"
```
...
const consoleTools: Tool[] = [
  {
    ...
    async execute({ trace_id }) {
      const res = await fetch(
        `https://console.hanzo.ai/v1/o11y/traces/${trace_id}`,
  ...
  },
  {
    ...
    async execute({ name, label }) {
      const res = await fetch(
        `https://console.hanzo.ai/v1/prompts/${name}?label=${label}`,
        { headers: { 'Authorization': `Bearer ${process.env.HANZO_API_KEY}` } }
      )
      return await res.json()
    }
  },
  {
    ...
    async execute({ period }) {
      const res = await fetch(
        `https://api.hanzo.ai/v1/billing/usage?period=${period}`,
        { headers: { 'Authorization': `Bearer ${process.env.HANZO_API_KEY}` } }
      )
      return await res.json()
    }
