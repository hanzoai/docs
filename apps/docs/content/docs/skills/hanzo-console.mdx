...
- **SDK compatible**: works with the OpenTelemetry and Langfuse clients
...
| SDK host | `https://console.hanzo.ai` |
| Auth (Hanzo) | `Authorization: Bearer ${HANZO_API_KEY}` |
| Auth (SDK key pair) | Basic auth with public/secret key pair |
...
| 401 | Unauthorized | Check API keys or SDK key pair |
