---
title: "Hanzo PubSub - Event Streaming and Message Queue"
description: "Hanzo PubSub is the messaging backbone of Hanzo infrastructure — subject-based pub/sub, durable JetStream streams with replay, exactly-once delivery, and a bundled Kafka-compatible sidecar. Written in Go, one binary."
---

## Overview

Hanzo PubSub is a **high-performance messaging system** for pub/sub, persistent streams, and exactly-once delivery. It provides the messaging backbone for Hanzo infrastructure: subject-based routing for fire-and-forget events, durable JetStream streams when you need replay, and a bundled Kafka-compatible sidecar so existing Kafka producers can publish without being rewritten. Written in Go, one binary.
...
`hanzoai/pubsub`. Go module `github.com/hanzoai/pubsub`.
...
- `hanzo/hanzo-s3.md` - S3-compatible object storage (uses PubSub for event notifications)
- `hanzo/hanzo-platform.md` - PaaS deployment
- `hanzo/hanzo-universe.md` - Production K8s infrastructure
- `hanzo/hanzo-database.md` - SQL database
