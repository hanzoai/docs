---
title: "Hanzo Vector - High-Performance Vector Search Engine"
description: "Hanzo Vector is the vector search engine behind /v1/vector — Rust, dense and sparse similarity search with payload filtering, quantization, distributed clustering and GPU acceleration, over a Qdrant-compatible REST and gRPC API."
---

## Overview

Hanzo Vector is the **vector search engine** behind `/v1/vector` — written in Rust, serving dense and sparse similarity search with payload filtering, quantization, distributed clustering, and GPU acceleration.

It speaks the **Qdrant wire protocol** over both REST and gRPC. That is a deliberate interface choice, not an accident of history: every Qdrant client SDK (Python, JS, Rust, Go) and every tool that talks to Qdrant works against Hanzo Vector unchanged, and the binary keeps the `qdrant` name so that compatibility holds all the way down to the container command line.

### Why Hanzo Vector?

- **Qdrant-compatible API**: every Qdrant client SDK (Python, JS, Rust, Go) works unchanged
...
- **gRPC server**: tonic, serving the Qdrant protobuf service definitions
...
Repo: `hanzoai/vector`, branch `master` (not `main`).
...
- `hanzo/hanzo-s3.md` - S3-compatible object storage
- `hanzo/hanzo-ai-api.md` - the Hanzo AI API (generates embeddings for vector storage)
