---
title: "Hanzo Gateway - API Gateway for Hanzo and Lux Networks"
description: "Hanzo Gateway is a high-performance API gateway routing every Hanzo and Lux API endpoint across production clusters. It is the unified API entry point for all Hanzo (`api.hanzo.ai`) and Lux (`api.lux.network`) traffic, with per-endpoint rate limiting, JWT auth, billing enforcement, circuit breakers, and ZAP binary transport — all driven by declarative JSON."
---

## Overview

Hanzo Gateway is a **high-performance API gateway** that routes every Hanzo and Lux API endpoint across production clusters. It is the unified API entry point for all Hanzo (`api.hanzo.ai`) and Lux (`api.lux.network`) traffic, handling per-endpoint rate limiting, JWT authentication, billing enforcement, CORS, circuit breakers, host-based reverse proxying, and ZAP binary transport -- all driven by declarative JSON configuration. One binary, one config file per cluster, no imperative routing code.
...
- **Publishable-key security**: Per-IP rate limiting and origin validation for the browser key (`pk-*`)
...
- **Language**: Go 1.26
- **HTTP layer**: Gin, over an internalized routing/proxy engine (`internal/`)
...
The routing and proxy engine is vendored into `internal/` rather than pulled as
a dependency, so the whole request path is one auditable tree. On top of it the
repo owns the parts that make the gateway Hanzo's: IAM auth middleware, widget
key security, host-based proxy routing, ZAP binary transport, and billing
enforcement. There is no phone-home telemetry.
...
| Repo | `github.com/hanzoai/gateway/v2` |
...
1. **Auth middleware** (`auth_middleware.go`): Validates JWT via JWKS, checks billing via Commerce API (fail-open), injects `X-IAM-Org-Id`, `X-IAM-User-Id`, `X-IAM-User-Email` headers. An opaque API key — `sk-*` or `pk-*`, the two spellings the door recognizes — is resolved against IAM rather than parsed as a JWT.

2. **Publishable-key security** (`widget_security.go`): Per-IP rate limiting (10 req/min default) and origin validation for `pk-*` keys. A publishable key ships in a browser bundle by design, so the origin is the only thing left to check.

3. **Host proxy** (`hostProxyMiddleware` in `legacy_transports.go`, reading the table `routes.go` compiles): Routes 40+ Hanzo domains to their backend K8s services via `httputil.ReverseProxy`. Supports WebSocket upgrades natively. Path-based routing for multi-backend hosts (e.g., `platform.hanzo.ai` routes `/v1/` to the platform API, `/` to paas-ui).
...
Client --> api.hanzo.ai/v1/chat/completions
  ...
  |
  Widget security (skip for non-pk- tokens)
  |
  Host proxy (api.hanzo.ai matched, /v1/* passed through as-is)
  |
  http://cloud.hanzo.svc.cluster.local:8000/v1/chat/completions
  |
  Cloud resolves model → provider (Fireworks, Together, etc.)
  |
  Streaming response back through gateway (no buffering)
```

For `api.hanzo.ai`, AI endpoints (`/v1/chat`, `/v1/completions`, `/v1/models`, etc.) are routed directly to cloud via reverse proxy **with no path rewrite** — cloud speaks `/v1/*` natively, so the path that arrives is the path that is served (`legacy_transports.go`). The target defaults to `http://cloud.hanzo.svc.cluster.local:8000` and is overridable with `GATEWAY_CLOUD_API_URL` (`routes.go`). Non-AI paths (`/billing/*`, `/auth/*`, `/analytics/*`, etc.) fall through to the declarative endpoint handlers defined in `gateway.json`.

### Host-Based Routing

The gateway routes 40+ exact hostnames to backend services, declared in `routes.yaml`
(loaded from KMS via `GATEWAY_ROUTES_KMS_PATH`, or from a local file via
`GATEWAY_ROUTES_FILE`) and compiled into one `httputil.ReverseProxy` per prefix by
`routes.go`. Examples:
...
| `platform.hanzo.ai` | Multi-path: `/v1/` (plus `/sync/`, `/socket.io/`) to platform:4000, `/` to paas-ui:80 |
| `kms.hanzo.ai` | `kms.hanzo.svc:80` |
| `cloud.hanzo.ai` | Multi-path: `/v1/` to cloud:8000, `/` to the console SPA on console:4000 |
...
Order matters — first prefix match wins, so a `/v1/` entry is declared before `/`.

Host redirects (301): `app.hanzo.ai` to `https://hanzo.app`. The deployed table is the
one held in KMS; `routes.yaml` in the repo is the fallback and the schema, so read the
live config before relying on a specific row.

Subdomain patterns: Hosts containing `lux-test` or `lux-dev` route to testnet/devnet luxd nodes.

### Configuration Structure

All routing is defined in JSON config files (`configs/hanzo/gateway.json`, `configs/lux/gateway.json`). Each uses the gateway's v3 config schema:
...
AUTH_PUBLIC_PATHS=/healthz,/__stats,/.well-known/,/favicon
...
configs/
  hanzo/
    gateway.json        # Hanzo API Gateway config
    ingress.json        # Hanzo Ingress sidecar config
  lux/
    gateway.json        # Lux API Gateway config
k8s/
...
executor.go             # Request executor (wires all components)
...
| 403 Forbidden (browser) | Publishable key (`pk-*`) used from an unlisted origin | Add the origin to `AllowedOrigins` in `widget_security.go`; server-side, use a secret key instead |
| API key rejected | Key spelled a way the door does not recognize | Two spellings exist: `sk-` and `pk-`. Anything else is refused, never defaulted. |
...
| Config validation fails | JSON syntax error or schema violation | Run `make validate`; check against the v3 config schema |
...
- `hanzo/hanzo-ai-api.md` - the Hanzo AI API that cloud-api routes through
