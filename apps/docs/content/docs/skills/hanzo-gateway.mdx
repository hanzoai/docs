...
Client --> api.hanzo.ai/v1/chat/completions
  ...
  |
  http://cloud-api.hanzo.svc.cluster.local:8000/v1/chat/completions
  |
...
For `api.hanzo.ai`, AI endpoints (`/v1/chat`, `/v1/completions`, `/v1/models`, etc.) are routed directly to cloud-api via reverse proxy with `/v1/` rewritten to `/api/`. Non-AI paths (`/billing/*`, `/auth/*`, `/analytics/*`, etc.) fall through to the declarative endpoint handlers defined in `gateway.json`.
