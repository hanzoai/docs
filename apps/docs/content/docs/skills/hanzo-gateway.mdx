...
description: "Hanzo Gateway is a high-performance API gateway that routes 147+ endpoints across production clusters. It serves as the unified API entry point for all Hanzo (`api.hanzo.ai`) ..."
---

## Overview

Hanzo Gateway is a **high-performance API gateway** that routes 147+ endpoints across production clusters. It serves as the unified API entry point for all Hanzo (`api.hanzo.ai`) and Lux (`api.lux.network`) traffic with per-endpoint rate limiting, JWT authentication, billing enforcement, CORS, circuit breakers, host-based reverse proxying, and ZAP binary transport -- all driven by declarative JSON configuration.
...
For `api.hanzo.ai`, AI endpoints (`/v1/chat`, `/v1/completions`, `/v1/models`, etc.) are routed directly to cloud-api via reverse proxy with `/v1/` rewritten to `/api/`. Non-AI paths (`/billing/*`, `/auth/*`, `/analytics/*`, etc.) fall through to gateway endpoint handlers defined in `gateway.json`.
...
executor.go             # gateway executor (wires all components)
