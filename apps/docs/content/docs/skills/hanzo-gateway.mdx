...
1. **Auth middleware** (`auth_middleware.go`): Validates JWT via JWKS, checks billing via Commerce API (fail-open), injects `X-IAM-Org-Id`, `X-IAM-User-Id`, `X-IAM-User-Email` headers. API keys (`sk-*`, `fw_*`, `hz_*`, `pk-*`) pass through without JWT validation.
...
| 403 Forbidden (widget) | Widget key (`hz_*`) used from unauthorized origin | Add origin to `AllowedOrigins` in `widget_security.go`; or use `sk-*` API key instead |
| API key rejected | Key prefix not recognized | Recognized prefixes: `sk-`, `fw_`, `hz_`, `pk-` |
