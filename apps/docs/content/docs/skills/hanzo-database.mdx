---
title: "Hanzo Database - SQL, KV & Vector Storage"
description: "Hanzo Database covers the PostgreSQL (with pgvector), Redis/Valkey, MongoDB, and Hanzo S3 configurations used across the Hanzo ecosystem — all running in-cluster on K8s, no managed database services."
---

## Overview

Hanzo Database covers the PostgreSQL (with pgvector), Redis, MongoDB, and Hanzo S3 configurations used across the Hanzo ecosystem. **All databases run in-cluster on K8s** — no managed database services (DO Managed DB decommissioned Feb 2026).

### Why In-Cluster?

- **Cost**: 5-10x cheaper than managed DBs at scale
- **Latency**: Same-cluster networking, no external hops
- **Control**: Full SQL config, extensions, versions
- **Consistency**: Same setup across all environments

## When to use

- Configuring SQL for Hanzo services
- Setting up pgvector for AI embeddings
- KV caching, sessions, and job queues
- DocumentDB for document storage (Chat)
...
| KV | 6379 | `redis:7-alpine` | Cache, sessions, BullMQ queues |
| DocumentDB | 27017 | `mongo:7` | Document storage (Chat) |
| Hanzo S3 | 9000 | `ghcr.io/hanzoai/s3` | S3-compatible object storage |
...
|---------|----------|------|
| `postgres.hanzo.svc` | — | Shared SQL instance |
