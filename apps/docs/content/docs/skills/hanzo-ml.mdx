---
title: "Hanzo ML"
description: "Hanzo ML is a minimalist Rust ML framework for edge AI, quantization and multimodal inference — the tensor computation layer of the Hanzo stack, with CUDA, Metal and ROCm acceleration and no Python runtime required."
---

## Overview

Hanzo ML is a **minimalist Rust ML framework** optimized for edge AI, quantization, and multimodal inference. It is the tensor computation layer of the Hanzo stack, with GPU acceleration via CUDA, Metal (Apple Silicon) and ROCm, plus CPU paths through MKL and Accelerate. No Python runtime is required at any point — a model loads and runs from a single Rust binary.

### What's in the workspace

- **Core**: `hanzo-ml` (tensors), `hanzo-nn` (layers), `hanzo-transformers` (90+ model implementations), `hanzo-datasets`
- **Kernels**: `hanzo-flash-attn` and `hanzo-flash-attn-v3` (CUDA), `hanzo-metal-kernels`, `hanzo-kernels`, `hanzo-rocm-kernels`
- **Interop**: `hanzo-ml-pyo3` for Python, `hanzo-onnx` for ONNX evaluation, `hanzo-ml-wasm-examples` for browser inference
- **Training**: `hanzo-train` and `hanzo-training` alongside inference, in the same tensor types
...
| Version | 0.11.18 (workspace), 0.11.94 (`hanzo-ml`) |
...
├── hanzo-train/              # Training loop + optimizers
