---
title: "Hanzo vLLM"
description: "Hanzo vLLM is a Rust LLM inference server with an OpenAI-compatible API, PagedAttention, continuous batching, and multi-GPU/multi-node support. Binary `candle-vllm`, version 0.2.1, MIT licensed."
---

## Overview

Hanzo vLLM is a **Rust LLM inference server** with an OpenAI-compatible API, PagedAttention, continuous batching, and multi-GPU/multi-node support. No Python in the serving path — point any OpenAI SDK at it and it answers. Binary name: `candle-vllm`. Version 0.2.1.
...
`hanzoai/vllm`. MIT License (Copyright 2023 Eric Buehler). Default branch: `master`.
...
- **AI API routing**: Use `hanzo/hanzo-ai-api.md`
...
## Relationship to Hanzo ML

The two are separate layers of the same stack:

- **hanzoai/ml** -- the ML framework: tensors, neural network layers, GPU kernels
- **hanzoai/vllm** -- the inference server: OpenAI-compatible API, continuous
  batching, scheduling, PagedAttention

## Related Skills

- `hanzo/hanzo-ml.md` - Rust ML framework (underlying tensor engine)
- `hanzo/hanzo-cloud.md` - Cloud LLM inference API (uses this as a backend)
- `hanzo/hanzo-ai-api.md` - the Hanzo AI API (routes to this among others)
