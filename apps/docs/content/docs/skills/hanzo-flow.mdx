---
title: "Hanzo Flow"
description: "Hanzo Flow is a visual workflow builder for AI agents and pipelines — drag-and-drop LLM chains and multi-agent orchestrations, each deployable as a REST API or MCP server with no extra code."
---

## Overview

Hanzo Flow is a **visual workflow builder** for AI-powered agents and pipelines, maintained at `github.com/hanzoai/flow`. It gives you a drag-and-drop canvas for designing LLM chains, multi-agent orchestrations, and data processing pipelines — and every workflow you build can be served as a REST API or an MCP server with zero additional code. The graph you drew is the deployment artifact.

The project is a **Python-first** monorepo with a React/TypeScript frontend. It ships four packages: `hanzo-flow` (the application, entry point `flow`), `hanzo-flow-base` (the backend framework layer), `hanzo-flow-sdk` (the client SDK), and `lfx` (a lightweight CLI executor for running flows headlessly).
...
| Package | `hanzo-flow` on PyPI (CLI: `flow`) |
| CLI Executor | `lfx` on PyPI |
| SDK | `hanzo-flow-sdk` on PyPI |
| Version | 1.8.2 (`hanzo-flow`), 0.9.6 (`hanzo-flow-base`), 0.3.1 (`lfx`), 0.1.6 (`hanzo-flow-sdk`) |
| Default Port | 7860 (dev), 3006 (Docker), 8000 (`lfx serve`) |
...
```
flow/
├── pyproject.toml              # Root workspace: hanzo-flow v1.8.2 (CLI: flow)
...
│   ├── backend/
│   │   ├── base/
│   │   │   ├── pyproject.toml  # hanzo-flow-base v0.9.6
│   │   │   └── flow/           # Core framework (API, services, graph engine)
│   │   │   │   ├── api/        # REST API (v1, v2)
│   │   │   │   ├── base/       # Base classes
...
│   ├── sdk/                    # hanzo-flow-sdk v0.1.6 (client SDK)
│   └── frontend/               # React/TypeScript UI
│       ├── package.json        # flow v1.9.6 (private)
...
- **Vector Stores**: Chroma, Pinecone, Qdrant, Weaviate, Milvus, pgvector, Elasticsearch, MongoDB Atlas, Astra DB, Hanzo Datastore, Couchbase, Cassandra
...
### Key Dependencies (hanzo-flow-base)
...
- `hanzo/hanzo-studio.md` -- Visual node-based AI engine (image/video generation)
