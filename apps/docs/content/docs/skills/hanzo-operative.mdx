---
title: "Hanzo Operative - Computer Use Agent"
description: "Hanzo Operative (`hanzo-operative`) is an autonomous computer use agent that drives a desktop through screenshots, mouse, keyboard, and bash. It calls the Anthropic Claude API with computer-use tool definitions and ships as a Streamlit UI plus three container images."
---

## Overview

Hanzo Operative (`hanzo-operative`) is an **autonomous computer use agent** that enables AI to control desktop environments via screenshots, mouse, keyboard, and bash commands. It calls the Anthropic Claude API with computer-use tool definitions to analyze screenshots and decide the next action, then executes it against a real X display. The entry point is a Streamlit web UI; the runtime ships as three container images (app, desktop, Xvfb) so the agent gets a disposable desktop of its own.
