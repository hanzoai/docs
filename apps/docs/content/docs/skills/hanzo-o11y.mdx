...
description: "Hanzo O11y is a full-stack observability platform for logs, metrics and traces — OpenTelemetry in, Hanzo Datastore underneath, one query surface instead of three products. Go backend, React frontend. Live at o11y.hanzo.ai."
---

## Overview

Hanzo O11y is a **full-stack observability platform** for logs, metrics, and traces. Go 1.25 backend (Gin/gorilla-mux) with a React 18 + Vite frontend. Uses **Hanzo Datastore** (Hanzo's Apache-2.0 Datastore fork) as the telemetry store and ingests over **OpenTelemetry**, so any OTLP exporter points at it without a vendor agent. Logs, metrics and traces land in one store and one query surface rather than three products. Live at `o11y.hanzo.ai`.
...
- **Metadata Store**: SQLite (community) or SQL (enterprise)
...
`hanzoai/o11y`. Go module `github.com/hanzoai/o11y`.
...
4. SQLite (or SQL in enterprise) stores metadata: users, orgs, dashboards, alerts, saved views
...
pkg/
  ...
  sqlstore/        # SQLite/SQL metadata store
...
|----------|-----------|-----------|
| traces | OTLP | datastoretraces, metadataexporter, o11ydatastoremeter |
| metrics | OTLP | o11ydatastoremetrics, metadataexporter, o11ydatastoremeter |
| metrics/prometheus | prometheus scraper | o11ydatastoremetrics, metadataexporter, o11ydatastoremeter |
| logs | OTLP | datastorelogsexporter, metadataexporter, o11ydatastoremeter |
...
| Frontend 502 | Backend not ready | Check `localhost:8080/v1/o11y/health` |
