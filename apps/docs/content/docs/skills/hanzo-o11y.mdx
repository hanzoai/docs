...
Hanzo O11y is a **full-stack observability platform** for logs, metrics, and traces. Go 1.25 backend (Gin/gorilla-mux) with a React 18 + Vite frontend. Uses **Hanzo Datastore** as the telemetry store and **OpenTelemetry** for ingestion. Live at `o11y.hanzo.ai`.
