...
Hanzo O11y is a **full-stack observability platform** for logs, metrics, and traces. Go 1.25 backend (Gin/gorilla-mux) with a React 18 + Vite frontend. Uses **Hanzo Datastore** (Hanzo's Apache-2.0 ClickHouse fork) as the telemetry store and ingests over **OpenTelemetry**, so any OTLP exporter points at it without a vendor agent. Logs, metrics and traces land in one store and one query surface rather than three products. Live at `o11y.hanzo.ai`.
