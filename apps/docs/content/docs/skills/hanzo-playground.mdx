...
description: "Control Plane for AI Bots"
---
...
- **Dual storage**: SQLite/BoltDB for local dev, SQL for production
...
- **Database**: SQLite/BoltDB (local), SQL (cloud), Goose migrations
...
control-plane/
  ...
  internal/
    ...
    storage/                   # Data layer (SQLite/BoltDB, SQL)
...
# Uses SQLite + BoltDB (no SQL needed)
```
...
|-----|--------|-------------|
| `/v1/cloud/nodes/provision` | POST | Provision new agent |
| `/v1/cloud/nodes/:node_id` | DELETE | Deprovision agent |
| `/v1/cloud/nodes` | GET | List cloud agents |
| `/v1/cloud/nodes/:node_id/logs` | GET | Get agent logs |
| `/v1/cloud/teams/provision` | POST | Batch provision |
...
| `postgresql` | SQL | Production, cloud |
| `cloud` | SQL | Distributed deployments |
...
|-------|-------|----------|
| Control plane won't start | Bad DATABASE_URL | Check SQL connection string |
| Migrations fail | SQL not running | Start SQL, verify connection |
