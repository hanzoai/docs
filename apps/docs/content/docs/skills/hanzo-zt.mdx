---
title: "Hanzo ZT"
description: "Hanzo ZT is the zero-trust network overlay fabric behind Hanzo's secure networking — a programmable mesh with identity-based access control, end-to-end encryption, dark services and smart routing. Controller, router and tunnel all build from this repo."
---

## Overview

Hanzo ZT is the **zero-trust network overlay fabric** that powers Hanzo's secure networking infrastructure. It provides a programmable, scalable mesh network with identity-based access control, end-to-end encryption, dark services, and smart routing. All Hanzo ZT executables — controller, router, tunnel — are built from this single repo, under the `hanzozt` module namespace.
...
`hanzoai/zt`. Go module `github.com/hanzozt/zt/v2`; every core library is under the `hanzozt/*` namespace. Related ecosystem repos referenced in README: `hanzoai/edge`, `hanzoai/fabric`, `hanzoai/foundation`, plus SDKs (`zt-sdk-c`, `sdk-golang`, `zt-sdk-jvm`, `zt-sdk-swift`, `zt-sdk-nodejs`, `zt-sdk-csharp`).
