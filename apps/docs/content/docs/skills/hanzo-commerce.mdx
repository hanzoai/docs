---
title: "Hanzo Commerce - Multi-Tenant E-Commerce and Billing Platform"
description: "Hanzo Commerce is the multi-tenant commerce platform behind billing, payments, subscriptions, storefronts and API usage metering for the whole Hanzo ecosystem — one Go binary, 100+ models, 4 database backends, 6 payment providers and a complete OpenAPI 3.0 surface."
---
...
| Object Storage | Hanzo S3 via `hanzoai/storage-go` |
...
Extensible lifecycle hooks (Base-compatible pattern):
...
commerce/
  ...
  infra/               Infrastructure Clients
    ...
    storage.go         S3 storage client (hanzoai/storage-go)
...
|----------|--------|---------|
| `/v1/store` | GET/POST | Storefront listing/create |
| `/v1/product` | CRUD | Products |
| `/v1/variant` | CRUD | Product variants |
| `/v1/collection` | CRUD | Product collections |
| `/v1/cart` | CRUD | Shopping carts |
| `/v1/order` | CRUD | Orders |
| `/v1/checkout` | POST | Payment processing |
| `/v1/checkout/sessions` | POST | Stripe Checkout sessions |
| `/v1/subscription` | CRUD | Subscriptions |
| `/v1/coupon` | CRUD | Coupon codes |
| `/v1/promotion` | CRUD | Promotions |
| `/v1/inventory` | CRUD | Inventory tracking |
| `/v1/fulfillment` | CRUD | Order fulfillment |
| `/v1/review` | CRUD | Product reviews |
...
| `/v1/billing/deposit` | POST | Create deposit transaction |
| `/v1/billing/credit` | POST | Grant starter credit ($5, 30-day expiry) |
| `/v1/billing/zap` | POST | Clear balance |

### Analytics

Commerce does not serve an analytics surface of its own — it is a *client* of
the event spine. Every wire (single event, batch, identify, custom commerce
event) goes to the one ingest door, dispatched by the shape of the body:

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/v1/event` | POST | The one ingest door — a bare event, a bare array, or a `{batch:[…]}` / `{events:[…]}` envelope. Batch is a body, not a path. |
| `/v1/event.js` | GET | The tag that feeds the door, served from the same origin |
| `/v1/analytics/overview` | GET | Read lens: totals for a range |
| `/v1/analytics/timeseries` | GET | Read lens: bucketed series |
| `/v1/analytics/top` | GET | Read lens: top-N breakdown |
...
| `/v1/account` | GET/POST | User account |
| `/v1/user` | CRUD | User management (admin) |
| `/v1/organization` | CRUD | Organization management |
| `/v1/namespace` | CRUD | Namespace management (admin-only, authed) |
| `/v1/accesstoken` | CRUD | API token management |
...
`POST /v1/checkout/sessions` -- public endpoint (no token auth).
...
Org resolved from `X-IAM-Org-Id` header or request body. Per-request Stripe client (multi-tenant safe).
