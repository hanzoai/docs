...
- **4 database backends**: SQLite (embedded, dev), PostgreSQL (production), MongoDB/FerretDB, Datastore (analytics)
...
- **Event-driven**: NATS/JetStream pub/sub + Datastore analytics pipeline
...
| Databases | SQLite + sqlite-vec, PostgreSQL + pgvector, MongoDB, Datastore |
...
| Analytics | Datastore (hanzoai/datastore-go) |
...
- Per-org SQL: tenant_id column isolation
...
commerce/
  ...
  db/                  Database Backends
    ...
    mongo.go           DocumentDB (alternative)
    datastore.go       Datastore via hanzoai/datastore-go (analytics)
