---
title: "Hanzo Dev - AI-Powered Development Platform"
description: "Hanzo Dev is an AI-powered development platform — a Rust-native CLI (`dev`, `dev-tui`, `dev-exec`) with 40+ crates, a Node.js wrapper, an app server, MCP client and server, and a cloud task system. Built with Bazel and Cargo."
---
...
The primary implementation is the Rust workspace in `hanzo-dev/` (40+ crates). The `codex-cli/` and `codex-rs/` directories are the superseded TypeScript and early-Rust generations, kept building but no longer where work happens.
...
hanzoai/dev/
  ...
  codex-cli/            # Legacy TS CLI (superseded by hanzo-dev/)
