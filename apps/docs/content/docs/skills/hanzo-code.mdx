---
title: "Hanzo Code"
description: "Hanzo Code is an open-source AI-native code editor with LLM chat, autocomplete, inline diffs and agent tool use built into the editor core rather than bolted on as an extension. Ships as an Electron desktop app for macOS, Windows and Linux."
---

## Overview

Hanzo Code is an **open-source AI-native code editor**. LLM chat, autocomplete, inline diffs and agent tool use live in the editor core rather than in an extension, which is what lets the model see and edit across the whole workspace instead of one buffer at a time. The AI code lives in `src/vs/workbench/contrib/code/browser/`. Full extension, theme and keybinding compatibility with the VS Code ecosystem. Ships as a standalone Electron desktop app for macOS, Windows, and Linux.
...
`github.com/hanzoai/code`. Package `code-dev` v1.94.0.

**Note for contributors**: internal code paths and `product.json`'s `applicationName` still read `void`. Those are load-bearing identifiers — the data folder and bundle id derive from them — so do not rename them casually.
...
2. **npm** (not pnpm -- the workspace layout is npm's)
...
- `hanzo/hanzo-ai-api.md` - the Hanzo AI API (api.hanzo.ai)
