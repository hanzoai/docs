---
title: "Hanzo Editor"
description: "Hanzo Editor is an alias for Hanzo Code (`hanzoai/code`) — the AI-native code editor with LLM chat, autocomplete, inline diffs and agent tool use built into the editor core."
---

## Overview

Hanzo Editor (`hanzoai/code`) is an **open-source AI-native code editor** with LLM chat, autocomplete, inline diffs, and agent tool use built directly into the editor core. Ships as a standalone Electron desktop app.
...
- Package `code-dev` v1.94.0, shipped as "Code"
...
### Identifiers you should not rename

- `product.json`: `applicationName: "void"`, `dataFolderName: ".code-editor"`, `darwinBundleIdentifier: "com.hanzoai.code"`. The first two determine the user's config directory — changing them orphans every existing install.
- Internal contributor docs still use the `void` name (`VOID_CODEBASE_GUIDE.md`, `.voidrules`, `void_icons/`).
