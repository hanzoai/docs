---
title: "Hanzo Suite"
description: "Hanzo Business Suite is the open-source ERP/CRM behind team.hanzo.ai — CRM, e-commerce, warehouse, projects, accounting, POS, HR, marketing and manufacturing across 584 addons, on Python and SQL, integrated with Hanzo AI infrastructure."
---

## Overview

Hanzo Business Suite is a **full-featured open-source ERP/CRM platform** integrated with Hanzo AI infrastructure. It covers CRM, e-commerce, warehouse management, project management, billing and accounting, POS, HR, marketing and manufacturing across 584 bundled addons — the whole back office in one application rather than six integrations. Runs as a Python web application backed by SQL. Live at `team.hanzo.ai` and `hanzo.team`.

### Why Hanzo Suite?

- **Full ERP in one platform**: CRM, accounting, inventory, HR, e-commerce, POS, project management
- **584 addons out of the box**: a mature, battle-tested module ecosystem, not a starter kit
- **AI integration**: Hanzo AI enhancements slot in as ordinary addons
- **Self-hosted**: Docker Compose deployment with SQL, Traefik reverse proxy
- **Modular**: 584 built-in addons, plus custom addons in `/mnt/extra-addons`

### Tech Stack

- **Language**: Python 3.10+ (core framework)
- **Web framework**: Werkzeug + custom Odoo HTTP layer
- **ORM**: Odoo ORM (custom, SQL-backed, declarative field definitions)
...
`hanzoai/suite`. Default branch: `main`. License: LGPL-3.0.
...
The ORM maps Python classes to SQL tables:
...
github.com/hanzoai/suite/
    ...
    odoo/                   # Core framework
        ...
        addons/             # Built-in addon modules (584)
...
HOST=db              # SQL host
USER=odoo            # SQL user
PASSWORD=changeme    # SQL password
...
- `hanzo/hanzo-database.md` - SQL infrastructure
