...
description: "Hanzo Explorer is a self-hostable EVM blockchain explorer for Lux Network and any EVM-compatible chain — full indexing, contract verification, and a REST/GraphQL API, on Elixir/Phoenix with Rust microservices."
---

## Overview

Hanzo Explorer is an **EVM blockchain explorer** for inspecting and analyzing transactions, accounts, smart contracts, and tokens on Lux Network and other EVM-compatible chains. Run it yourself instead of depending on a hosted explorer: it indexes the whole chain, verifies and decompiles contracts, and exposes both REST and GraphQL over the result. Version 8.1.1, built with Elixir/Phoenix (backend) and a separate frontend, with Lux-specific Docker configurations for mainnet deployment.
...
`hanzoai/explorer`. GPL v3.0 License.
...
hanzoai/explorer/
  ...
  docker-compose/               Generic Docker configs
    docker-compose.yml          Standard setup (SQL, KV, backend, frontend, nginx)
...
| `external-db.yml` | Explorer with external SQL |
...
| DB connection error | SQL not ready | Wait for DB startup or check `DATABASE_URL` |
