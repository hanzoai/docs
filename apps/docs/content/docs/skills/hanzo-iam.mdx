---
title: "Hanzo IAM - Identity and Access Management Server"
description: "Hanzo IAM is the identity and access management service for the Hanzo ecosystem — OAuth 2.0, OIDC, SAML, CAS, LDAP, SCIM, WebAuthn, TOTP and RADIUS in one Go binary, serving SSO across hanzo.id, lux.id, zoo.id and pars.id, with a built-in MCP server for agent identity."
---

## Overview

Hanzo IAM is the **server-side identity and access management service** for the Hanzo ecosystem. A single Go binary providing OAuth 2.0/OIDC/SAML/CAS/LDAP/SCIM/WebAuthn/TOTP/RADIUS authentication and authorization. Serves SSO across hanzo.id, lux.id, zoo.id, and pars.id. Includes built-in MCP (Model Context Protocol) server for AI agent identity management.
...
- **Database**: SQL (primary), MySQL, SQLite (embedded)
- **Session store**: KV (auto-discovers `hanzo-kv` or `redis` in K8s)
...
`github.com/hanzoai/iam`. Apache-2.0 licensed.
...
1. **SQL** (or MySQL/SQLite) database
2. **KV** for session storage (optional -- falls back to file sessions)
...
├──────────────┴──────────────┴──────────────┴─────────────────┤
│  SQL/MySQL/SQLite  │  KV                   │  File Storage   │
└───────────────────────────┴────────────────┴─────────────────┘
         ↑                        ↑                    ↑
  ┌──────┴──────┐          ┌──────┴──────┐     ┌──────┴──────┐
...
The server auto-discovers KV in Kubernetes by trying DNS lookups for `hanzo-kv` and `redis` service names. Falls back to file-based sessions if no KV is available.
...
```bash
make dev          # Start local dev with Docker Compose (SQL)
...
| Protocol | Path |
|----------|------|
| SAML | `/v1/iam/saml/redirect/{owner}/{application}`, `/v1/iam/saml/metadata`, `/v1/iam/acs` (ACS, POST only) |
...
Prometheus metrics are on zip's ops listener at `/metrics` — a second port the deployment brings up when it sets `OPS_PORT`, never the public one. A liveness or scrape request must not queue behind public traffic, so `/metrics`, `/healthz` and `/readyz` are not on the served surface at all.
...
- **Build**: Go tests with SQL, frontend build (pnpm), Go build with race detection, gofumpt lint, Cypress E2E
...
| KV not found | No KV endpoint configured | Auto-discovers `hanzo-kv` or `redis` DNS; set `redisEndpoint` explicitly |
