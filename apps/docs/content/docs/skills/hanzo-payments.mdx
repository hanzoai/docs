---
title: "Hanzo Payments - Payment Orchestration Switch"
description: "Hanzo Payments is an open-source payment orchestration switch in Rust — routes across 50+ processors with smart retries, fallback cascading and unified analytics, behind one REST API on port 8080. Apache 2.0."
---

## Overview

Hanzo Payments is an **open-source payment orchestration switch** written in Rust. It routes payments across 50+ processors with smart retries, fallback cascading, and unified analytics, so a processor outage becomes a routing decision rather than an incident. One REST API on port 8080 regardless of how many processors sit behind it. Licensed Apache 2.0.
...
`hanzoai/payments`. Rust, 39 workspace crates.
...
2. **SQL** database
3. **KV** for caching and process tracking
...
| `redis_interface` | KV client wrapper |
