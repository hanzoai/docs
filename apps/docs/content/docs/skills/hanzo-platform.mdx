---
title: "Hanzo Platform - Cloud PaaS for AI Applications"
description: "Hanzo Platform is a Kubernetes-native PaaS (Platform as a Service) for deploying AI applications, APIs, and services."
---
...
- **IAM integrated**: Auth via hanzo.id (Hanzo IAM-based)
- **Secrets via KMS**: Hanzo KMS-backed secret management
...
3. **KMS** at kms.hanzo.ai for secrets (or self-hosted Hanzo KMS)
...
```bash
# IAM Configuration (canonical `IAM_*` contract)
IAM_ENDPOINT=https://hanzo.id
IAM_ISSUER=https://hanzo.id

# IAM Client (sourced from KMS)
IAM_CLIENT_ID=app-hanzo
IAM_CLIENT_SECRET=secret

# Optional: org/app scope and userinfo URL
IAM_ORG=hanzo
IAM_APP=app-hanzo
IAM_USERINFO_URL=https://hanzo.id/oauth/userinfo
```
...
```yaml
# KMSSecret CRD for auto-sync — full field list at
# /docs/services/kms/kmssecret-crd
...
spec:
  ...
  authentication:
    universalAuth:
      credentialsRef:
        secretName: hanzo-paas-iam-creds
