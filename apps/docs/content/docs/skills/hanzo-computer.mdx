...
description: "Hanzo Computer (`hanzoai/computer`) is an ecommerce web application for purchasing AI compute hardware. It sells the DGX Spark instance ($4,000) as the only credit-card purchasable product, with GP..."
---
...
hanzoai/computer/
  ...
  .env.example              # IAM, Commerce API, Upstash KV config
  api/
    _middleware.ts           # Rate limiting middleware (Upstash KV)
...
- **API rate limiting errors**: Configure Upstash KV env vars
