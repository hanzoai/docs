---
title: "Hanzo Reconciliation"
description: "Hanzo Reconciliation is an automated transaction matching engine that compares ledger balances against payment provider data to verify financial consistency and identify discrepancies. It runs as a..."
---

## Overview

Hanzo Reconciliation is an **automated transaction matching engine** that compares ledger balances against payment provider data to verify financial consistency and identify discrepancies. It runs as a standalone HTTP service backed by SQL, with policies that define how and when to reconcile ledger and payment data. Part of the Hanzo treasury stack.
...
- **Language**: Go 1.24
- **HTTP**: go-chi/chi v5
- **ORM**: uptrace/bun (SQL via pgdialect)
- **DI**: uber/fx
- **CLI**: spf13/cobra
- **Auth**: OAuth2 client credentials (Formance SDK)
- **Observability**: OpenTelemetry (traces + metrics)
- **Testing**: testify + gomock + dockertest (SQL integration tests)
...
1. **SQL** (v12+) for policy and reconciliation result storage
...
--postgres-uri        SQL connection string
```
...
github.com/hanzoai/reconciliation/
    ...
    internal/
        ...
        storage/
            ...
            main_test.go             # Test harness (dockertest SQL)
...
|-------|-------|----------|
| Connection refused on startup | SQL not reachable | Check `--postgres-uri` and network |
