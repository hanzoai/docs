...
universe/
  infra/
    k8s/                        # Production Kustomize manifests (main entry point)
      ...
      kv/                        # KV + ZAP (StatefulSet, 2Gi PVC)
      docdb/                     # DocDB -- FerretDB (MongoDB wire protocol over PostgreSQL)
      s3/                        # Hanzo S3 (s3.hanzo.ai)
      s3-csi/                    # Hanzo S3 CSI driver + FUSE mount sidecar
      search/                    # Hanzo Search (search.hanzo.ai, backup CronJob)
      vector/                    # Hanzo Vector (vector.hanzo.ai, backup CronJob)
      nats/                      # NATS core messaging
      stream/                    # Kafka wire protocol gateway over PubSub
      cloud-sql/                 # Serverless SQL (Neon-based: pageserver, safekeeper, compute)
      # --- Identity & Security ---
...
| **docdb** | DocDB | StatefulSet | DocumentDB wire protocol | Via SQL |
| **s3** | `ghcr.io/hanzoai/s3:v1.0.14` | Deployment | S3 API | PVC |
| **search** | Hanzo Search | Deployment | HTTP | PVC + backup CronJob |
| **vector** | Hanzo Vector | Deployment | gRPC + HTTP | PVC + backup CronJob |
| **nats** | NATS | - | 4222 | - |
...
- **Snapshots**: Hourly to S3 (in-cluster Hanzo S3), 5 retained, RLP exports included
- **Gateway**: Hanzo Gateway (`api.lux.network`)
- **Explorer**: Lux Explorer + frontend (`explorer.lux.network`)
...
Cloud SQL (Neon-based) provides serverless SQL with pageserver, safekeeper, storage-broker, and compute nodes.
...
- `hanzo/hanzo-sql.md` -- SQL + ZAP unified image
- `hanzo/hanzo-kv.md` -- KV + ZAP cache
