---
title: "Hanzo Base - Open Source Backend for Any App"
description: "Hanzo Base is an open source Go backend that provides a complete application backend in a single binary. It includes embedded SQLite and SQL databases with realtime subscriptions, built-in f..."
---

## Overview

Hanzo Base is an **open source Go backend** that provides a complete application backend in a single binary. It includes embedded SQLite and SQL databases with realtime subscriptions, built-in file and user management, an Admin dashboard UI, REST and GraphQL APIs, CRDT-based collaboration, and a JavaScript plugin VM. Go module: `github.com/hanzoai/base`.

### Why Hanzo Base?

- **Single binary**: Entire backend compiles to one portable executable (CGO_ENABLED=0)
- **Embedded databases**: In-memory, SQL (SQLite via modernc, SQL via pgx), and vector DB with realtime subscriptions
...
- **Backend**: Go 1.26, cobra CLI, embedded SQLite (modernc.org/sqlite), SQL (pgx/v5)
...
3. No external database required (SQLite embedded); SQL optional via plugins
...
base/
  ...
  plugins/
    ...
    cloudsql/          # Serverless SQL (per-tenant)
...
| `cloudsql` | Serverless SQL per-tenant provisioning |
...
WebSocket-based realtime subscriptions at `/v1/realtime`:
