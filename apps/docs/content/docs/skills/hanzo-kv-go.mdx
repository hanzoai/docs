---
title: "Hanzo KV Go SDK"
description: "Go client library for Hanzo KV, wire compatible with Redis 7.x and Valkey 8.x. Module `github.com/hanzoai/kv-go/v9`; the Go package name is `redis`, so import the Hanzo module and call `redis.NewClient()`."
---

## Overview

Go client library for Hanzo KV, wire compatible with Redis 7.x and Valkey 8.x. Module path `github.com/hanzoai/kv-go/v9`; the Go package name is `redis`, so you import the Hanzo module and call `redis.NewClient()`.

### Repo

`hanzoai/kv-go`, branch: `master`.
...
- Full KV command coverage (strings, hashes, lists, sets, sorted sets, streams, HyperLogLog, geo, bitmaps, JSON, search, time series, probabilistic)
- Automatic connection pooling
- Pub/Sub
- Pipelines and transactions
- Lua scripting
- KV Sentinel and Cluster support
