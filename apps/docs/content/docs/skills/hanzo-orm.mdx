---
title: "Hanzo ORM - Go Generics ORM with Auto-Serialization"
description: "Hanzo ORM is a Go generics-based ORM with automatic serialization, KV caching, and zero code generation."
---

## Overview

Hanzo ORM is a **Go generics-based ORM** with automatic serialization, KV caching, and zero code generation. 130 tests passing, supports SQLite (embedded) and ZAP (SQL/DocumentDB/KV/Datastore via sidecar).
...
| SQL (SQL) | Production RDBMS (direct) | `ormdb.SQLConfig` |
| ZAP-SQL | Production RDBMS (via sidecar) | `ormdb.ZapConfig` |
| ZAP-DocumentDB | Document store (via sidecar) | `ormdb.ZapConfig` |
| ZAP-KV | KV cache layer (via sidecar) | `ormdb.ZapConfig` |
| ZAP-Datastore | Analytics/OLAP (via sidecar) | `ormdb.ZapConfig` |
