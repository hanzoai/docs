---
title: "Hanzo Sentry - Error Tracking & Performance Monitoring"
description: "Hanzo Sentry is the self-hosted Sentry 24.2.0 deployment behind error tracking and performance monitoring for every Hanzo, Lux, Zoo and Pars service — SSO through hanzo.id, running on hanzo-k8s."
---

## Overview

Hanzo Sentry is a **self-hosted Sentry 24.2.0 deployment** for error tracking and performance monitoring across all Hanzo, Lux, Zoo, and Pars services. It runs entirely on our own infrastructure — no events leave the cluster — with a Hanzo-branded image, OIDC SSO through hanzo.id, and manifests deployed to hanzo-k8s.
...
- **Shared infrastructure**: Uses hanzo-sql (SQL) and shared KV

### Tech Stack

- **Backend**: Python 3.11 (Django 5.x, Celery, DRF)
- **Frontend**: React + TypeScript (Webpack, Emotion CSS)
- **Queue**: Celery via KV (or RabbitMQ if configured)
- **Database**: SQL via `hanzo-sql.hanzo.svc`
- **Cache**: KV via `redis.hanzo.svc`
- **Image**: `ghcr.io/hanzoai/sentry:latest`
- **Base version**: Sentry 24.2.0 (FSL-1.0-Apache-2.0 license)

### How it is assembled

This is a **deployment**, not a Hanzo codebase: there is no `hanzoai/sentry` repo. The Sentry distribution is consumed as-is; everything Hanzo lives in the `self-hosted/` overlay and the K8s manifests at `universe/infra/k8s/sentry/`.
...
1. **SQL** via `hanzo-sql.hanzo.svc` (database `sentry`)
2. **KV** via `redis.hanzo.svc` (cache, queues, rate limits, buffers)
...
# Run dependent services (SQL, KV, Kafka, etc.)
...
| Worker not processing | Celery not connected to broker | Check KV connectivity or RabbitMQ config |
...
- `hanzo/hanzo-database.md` - Shared SQL (hanzo-sql)
