---
title: "Hanzo Registry - Private Docker Container Registry"
description: "Hanzo Registry is a private Docker container registry running Docker Distribution (registry:2) on hanzo-k8s, authenticated via Hanzo IAM token-based auth. It provides a self-hosted alternative to D..."
---
...
- **IAM-integrated auth**: Token-based auth via `iam.hanzo.ai/v1/iam/registry/token`
...
| Auth realm | `https://iam.hanzo.ai/v1/iam/registry/token` |
...
2. Registry returns 401 with token realm URL (`iam.hanzo.ai/v1/iam/registry/token`)
...
auth:
  token:
    realm: https://iam.hanzo.ai/v1/iam/registry/token
