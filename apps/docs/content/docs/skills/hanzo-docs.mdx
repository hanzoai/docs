...
**NOTE**: This is a documentation *framework*, not a plain Next.js MDX site. Content enters a build through exactly one of three routes: authored MDX in `content/docs/`, generated reference rendered from `hanzoai/cloud`'s `openapi.yaml`, or ported upstream OSS docs mirrored with attribution.
