---
title: "Hanzo ID - Identity & Authentication"
description: "Hanzo ID is the unified identity platform for the entire Hanzo/Lux/Zoo ecosystem."
---

## Overview

Hanzo ID is the **unified identity platform** for the entire Hanzo/Lux/Zoo ecosystem. Custom login UI built on Hanzo IAM (IAM), serving OAuth2/OIDC for all Hanzo services. White-label-ready with per-org customization.
...
**Hanzo IAM** (IAM backend) + custom **Next.js** login UI (`hanzo/id`).
...
1. **Hanzo IAM** IAM backend running
...
| Full profile | `/v1/iam/get-account` (needs auth) |
| App lookup | `/v1/iam/get-app-login` (source of truth) |
...
| Token | `/oauth/token` | NOT `/api/login/oauth/access_token`. Hanzo alias: `/v1/iam/login/oauth/access_token` |
| Introspect | `/oauth/introspect` | NOT `/api/login/oauth/introspect`. Hanzo alias: `/v1/iam/login/oauth/introspect` |
| Revoke | `/oauth/revoke` | |
| UserInfo | `/oauth/userinfo` | Hanzo alias: `/v1/iam/userinfo` (`/api/userinfo` is legacy Casdoor compat) |
| Device | `/oauth/device` | |
| Logout | `/oauth/logout` | |

Legacy Hanzo IAM paths still work but NEVER use them for new integrations.
...
const appLogin = await fetch("https://hanzo.id/v1/iam/get-app-login", {
  method: "POST",
...
**Token expiry**: Hanzo IAM apps default `expireInHours=0` which means instant expiry. Always set `expireInHours=168` (7 days).
...
- Uses `/v1/iam/get-app-login` as source of truth (NOT hardcoded)

### API Masking Warning

Hanzo IAM's `/v1/iam/get-application` masks fields for unauthenticated requests — appears empty but DB has real values. Always use authenticated requests or `/v1/iam/get-app-login`.
...
| Wrong org after login | Hardcoded `organization: "hanzo"` | Use `/v1/iam/get-app-login` lookup |
| Userinfo only returns `sub` | Normal Hanzo IAM behavior | Use `/v1/iam/get-account` for full profile |
| App appears to have no config | API masking | Use authenticated request |
| Missing cert reference | Wrong client_id for scoped SSO | Use `/v1/iam/get-app-login` |
