...
Hanzo IAM's `/api/get-application` masks fields for unauthenticated requests — appears empty but DB has real values. Always use authenticated requests or `/api/get-app-login`.
