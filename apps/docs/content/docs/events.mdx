...
icon: Radio
title: Events
description: One ingest endpoint that dispatches on the shape of the body, what each credential may write, and the receipt every wire gets back.
---
...
An event is a product fact — somebody did something. You send it to one
endpoint, and you read it back through lenses.

## One endpoint, dispatched by shape
...
ingest endpoint so the tag and the wire cannot drift apart. `/v1/event/replay` takes
