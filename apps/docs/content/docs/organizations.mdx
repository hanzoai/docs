...
| **Secret keys** (`sk-*`) | The member who minted it — one per user |
| **Publishable keys** (`pk-*`) | Organization, or the project it was minted with |
...
A secret key (`sk-*`) resolves to the member who minted it, so requests scope to
that member's organization. A publishable key (`pk-*`) resolves to the
organization itself — or to the project it was minted with. Either way no
additional header is needed.

For administrative operations that span an organization (e.g. listing all projects), use a secret key (`sk-*`) held by a member with the rights to do it:
