...
The model catalog is what the gateway answers from, and `GET /v1/models` takes
a bearer token like every other endpoint. [Mint a key](/docs/api-keys), then:
...
names suggest, so trading up costs less than it sounds like it should.
...
Every model in our catalog answers on that one path.
