...
// The MCP server's tool list, taken from the MCP server.
//
...
// operation?" is a question only the server can answer, and the docs ask it
// rather than assuming.
//
// The WHOLE answer is kept, `inputSchema` included: the tool reference renders
// one page per tool straight from this file, so what the server declares is what
// the page prints. A summary would drift from the server; a verbatim copy cannot.
//
...
//
// The server names operations TWO ways and both are here, because it changed
...
/** The live JSON-RPC endpoint. It is NOT declared in hanzo.yaml — see doorNotice. */
...
export interface McpTool {
  name: string;
  /** The tool's description, verbatim — the server's own prose. */
  description: string;
 ...
 * names itself. One function so no caller has to ask which shape answered, and so
 * "how many operations does this server reach?" has a single meaning across a
...
/** Every operation this server reaches, across every tool. */
export const reach = (tools: McpTool[]): number =>
  tools.reduce((n, t) => n + ops(t).length, 0);

/**
 * A read among a tool's operations, in the SERVER'S vocabulary.
 *
 * `get_`/`list_` is how the server spells a read, so this is its judgement and not
 * ours. Two callers want it and it is stated once: the sync probes a read when
 * it asks the server what an uncredentialed call is refused with, and the tool
...
/** What the server answered `initialize` with — its own account of itself. */
export interface McpHandshake {
  protocolVersion: string;
  serverName: string;
  serverVersion: string;
  /** Capabilities the server advertises, verbatim. */
  capabilities: Record<string, any>;
  /**
   * What ONE `tools/call` answers with NO credential. The server replies 200 with
   * a JSON-RPC result whose `isError` is set, so the docs can state the auth
   * rule from the server's own words instead of asserting it.
   *
...
export interface McpCatalog {
  ...
  meta: {
    ...
    reach: number;
    /** ISO date the list on disk was taken from the server. */
    captured: string;
  };
...
/** One JSON-RPC round trip to the server, with no credential attached. */
async function rpc(method: string, params?: Record<string, any>): Promise<any | null> {
  try {
    ...
    if (!r.ok) {
      console.warn(`[mcp] server answered ${r.status} to ${method}`);
      return null;
    }
    return await r.json();
  } catch (e) {
    console.warn(`[mcp] server unreachable for ${method}: ${(e as Error).message}`);
    return null;
  }
}

/** Ask the server for its tools. Null when it does not answer with a usable list. */
...
/**
 * Ask the server to describe itself, and ask it once WITHOUT a credential so the
...
/**
 * A read the server itself names as a read: a `get_`/`list_` operation on a tool
 * that is not the operator surface. Reading, so a probe that really dispatches
 * cannot change anything; named by the server, so it survives the server renaming
 * everything.
 */
...
export async function syncMcpTools(): Promise<void> {
  ...
  // Say it in the build log, loudly: this build's tool pages describe the server
  ...
  // nothing about the server and buries the changes that do.
  ...
  // A server answering with a fraction of what it had is a server mid-deploy, not a
  // server that lost tools. Keep the copy rather than publishing a reference that
  // silently drops hundreds of pages.
  //
  // MEASURE THE REACH, NOT THE TOOL COUNT. The server regrouped 833 flat per-operation
  ...
  // held it again. Twelve days of tool pages named tools the server answers "unknown
  // tool" to, and the guard could never unstick itself because the number it compared
  // was a fact about the server's SHAPE. Reach is a fact about what the server can do,
  // which is what "mid-deploy" was always trying to ask about.
  if (have && have.meta.reach && reach(tools) < have.meta.reach * 0.8) {
    keep(`server reaches only ${reach(tools)} operations, down from ${have.meta.reach}`);
    return;
  }

  tools.sort((a, b) => a.name.localeCompare(b.name));
  // Probe a READ, never a write. This call really is dispatched at the server, so
  ...
  //
  // The read is chosen in the SERVER'S vocabulary, not ours. This looked for a
  ...
  // a refusal from `get_v1_ads_campaigns` — a tool the server had stopped serving.
  ...
  if (!probe) console.warn('[mcp] no read-only operation to probe — the server states no auth rule this build can quote');
