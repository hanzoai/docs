...
Add API endpoints alongside the auto-generated collection routes. Base serves
everything under `/v1` (set by `BASE_API_PREFIX`), so put custom routes there
too — there is no `/api/` prefix:

```javascript
// hz_hooks/routes.pb.js

// GET /v1/stats
routerAdd('GET', '/v1/stats', (e) => {
...
// Protected route
routerAdd('GET', '/v1/me/dashboard', (e) => {
