...
```bash
curl "https://app.platform.hanzo.ai/v1/organization.one?organizationId=org_abc123" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
```bash
curl -X POST https://app.platform.hanzo.ai/v1/organization.create \
  -H "Authorization: Bearer YOUR_TOKEN" \
...
```bash
curl -X PATCH https://app.platform.hanzo.ai/v1/organization.update \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
