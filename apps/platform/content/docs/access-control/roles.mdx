...
A role belongs to a **membership**, not to a user — the same person can hold a
different role in every organization they belong to — so the write addresses
`memberId`. Every write on this surface is a `POST`; there is no `PATCH`.
...
Owner is nontransferable here: the call refuses any change into or out of Owner,
and refuses to change your own role. Transfer ownership from the dashboard.
