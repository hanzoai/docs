...
Cluster permissions are granted from the dashboard. The `api.hanzo.ai/v1` surface
carries no op that grants, revokes, or reads a per-cluster permission, so there
is nothing to script here — use the steps above.

What it does expose is the clusters themselves:
...
The listing is org-scoped by the calling token and covers both kinds of cluster:
the managed ones and any you attached yourself.
