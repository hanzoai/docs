...
**Ship the winner**: one click in the dashboard, or `POST
/v1/experiments/{id}/decide` with `{"winner": "test"}`. Either rewrites the
assignment flag so that arm serves 100% and every other arm 0%, and stamps who
decided. It takes effect immediately, and it needs an **org admin** — a stricter
gate than the rest of the surface, because promoting is a flag write.
