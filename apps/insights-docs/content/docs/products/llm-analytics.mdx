...
LLM Analytics uses Datastore for trace storage. The `plugin-server` processes and enriches traces async. For high-volume deployments, scale the plugin-server horizontally.
